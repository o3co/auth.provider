...
| Fail-closed grant-policy evaluation — throw → `503`, deny → `400` with the policy's error, `grantedScope` may only narrow the effective scope (CP-18 / CP-15), `grantedAudience` may only narrow the grant's ceiling, and a decision past either ceiling is `500 server_error` — the policy's fault, not the caller's | `packages/core/src/grants/grantPolicy.mts` (`evaluateGrantPolicy`, `boundPolicyAudience`, `policyOutOfBounds`) | `client_credentials`, jwt-bearer and the WebAuthn grant take both halves. `refresh_token` keeps its own scope ceiling (the original grant, RFC 6749 §6) and `/authorize` its redirect shape; both answer an exceeded ceiling through `policyOutOfBounds` / `server_error`. Token exchange bounds the policy against the subject token (`invalid_target`) and predates the home | #441, #520 | yes |
