...
| Shared Redis down — **replay seen-set** (`REPLAY_SEEN_SET_ADAPTER=redis`) | a DPoP proof at `/oauth/token` or at a protected resource; a `private_key_jwt` client assertion | `503 temporarily_unavailable`, with no `WWW-Authenticate` challenge at a protected resource: the proof or assertion is refused unrecorded, not judged invalid, so the client keeps its tokens and retries (`packages/dpop/src/verifier.mts`, `packages/core/src/middleware/tokenBinding.mts`, `protectedResourceBinding.mts`, `packages/oauth/src/middleware/clientAssertion.mts`) | `dpop_replay_store_unavailable` (error) and `token_binding_unavailable` / `protected_resource_binding_unavailable` (warn) for DPoP | `commandTimeout` |
