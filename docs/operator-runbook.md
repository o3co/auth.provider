...
`oauth.dpop.replay-store = "memory"` (the default) and no `dpopReplayStore`
wired, `multi` refuses to boot
...
be replayed once against each replica while its `iat` is within
±`oauth.dpop.iat-window-seconds` of that replica's clock (up to 121 s at the
default 60). Unset warns `dpop_replay_store_not_shared`; `single` is silent.
Wire a shared store and set `oauth.dpop.replay-store = "redis"`, which makes
the slot mandatory in every mode (`packages/dpop/README.md` "Operator
...
- **It does not see state inside a component you build and hand in.** The
  ...
  `AssertionIssuerRegistry` over a shared store. The DPoP replay store is
  another: its check asks only whether `dpopReplayStore` is filled, so a
  per-process store handed into the slot — `createMemoryDPoPReplayStore` is
  exported — counts as wired and boots under `multi` without a word.
...
| `repositories.user.http.authenticateUrl` / `authenticateByTokenUrl` (`CLIENT_USER_AUTHENTICATE_URL`, `CLIENT_USER_AUTHENTICATE_BY_TOKEN_URL`) | absolute `https` (loopback `http` only); `timeout` a positive integer ≤ 2147483647 ms. Not checked at boot but required all the same: each URL is the endpoint that answers — a `3xx` is not followed, so a URL that redirects fails every call ([foundation README](../packages/foundation/README.md#what-the-store-must-enforce-itself)) | `packages/foundation/src/repositories/HttpUserRepository.mts` |
...
| **The Store** (user directory) down or slow, or a configured Store URL answers with a redirect (no request follows one) | `POST /session/login`; federation callback, a `?link=1` link included; jwt-bearer grant | `503 temporarily_unavailable` "User directory temporarily unavailable" (`Session.mts`, `Federation.mts`); `503 temporarily_unavailable` "identity resolution unavailable" from the jwt-bearer grant (`packages/oauth/src/grants/jwtBearer.mts`) | `local login authenticate failed`, `user repository lookup failed`, `federation link: user repository failed` (warn); `jwt_bearer_user_repository_unavailable` (error). A redirect is the case whose logged `err` reads `Unexpected HTTP status 30x from <url>`: set the URL to the endpoint that answers, not one that redirects | `repositories.user.http.timeout` (default 5000 ms) and `maxResponseBytes` (default 1048576) — a timeout is a thrown error, not a `null` user (`packages/foundation/src/repositories/HttpUserRepository.mts`) |
...
| `dpop_replay_store_not_shared` (warn) | `dpop/src/module.mts` | DPoP is enabled under `oauth.dpop.replay-store = "memory"` (the default) with no `dpopReplayStore` wired and `deployment.mode` unset; replay protection is per-process, so a proof can be replayed once against each replica (`"multi"` refuses boot instead, `"single"` is silent) |
