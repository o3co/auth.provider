...
- The standalone's listener: a port it cannot bind (`EADDRINUSE`, `EACCES`) fails boot with that error; the process no longer announces a server it never started. A bound listener logs `server_listening` (info, `port`) once (`templates/standalone/src/listen.mts`).
