...
| **The Store** (user directory) down or slow, a configured Store URL answers with a redirect (no request follows one), or the Store refuses this deployment's `bearerToken` (`401` with `WWW-Authenticate: Bearer`) | `POST /session/login`; federation callback, a `?link=1` link included; jwt-bearer grant | `503 temporarily_unavailable` "User directory temporarily unavailable" (`Session.mts`, `Federation.mts`); `503 temporarily_unavailable` "identity resolution unavailable" from the jwt-bearer grant (`packages/oauth/src/grants/jwtBearer.mts`) | `local login authenticate failed`, `user repository lookup failed`, `federation link: user repository failed` (warn); `jwt_bearer_user_repository_unavailable` (error). A redirect is the case whose logged `err` reads `Unexpected HTTP status 30x from <url>`: set the URL to the endpoint that answers, not one that redirects. A refused token is the case whose logged `err` reads `the Store at <url> refused this deployment's credential (HTTP 401 with a Bearer challenge)`, the same on all four Store calls: set `CLIENT_USER_BEARER_TOKEN` to a token the Store accepts. The token itself is never logged. A Store that refuses the token with a `401` and no `Bearer` challenge is read as "no such user" instead, so a mismatch shows only as every login answering `401 invalid_credentials` | `repositories.user.http.timeout` (default 5000 ms) and `maxResponseBytes` (default 1048576) — a timeout is a thrown error, not a `null` user (`packages/foundation/src/repositories/HttpUserRepository.mts`) |
