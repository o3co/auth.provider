...
| `device_verification_session_liveness_unavailable` (error) | `device-grant/src/verificationEndpoint.mts` | device verification could not check that the cookie's session is still live and answered `503`: no device is looked up, approved or denied while it lasts |
