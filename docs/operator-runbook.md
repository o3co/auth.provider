...
| `error_envelope_code_malformed` (warn, through the console logger) | `core/src/errors/envelope.mts` | a module handed core's `errorEnvelope` an `error` code outside RFC 6749's `1*NQSCHAR` (empty, or carrying `"`, `\`, a control or non-ASCII character). The client got `server_error` with the status the module chose; the logged `error` is the code, sanitised and capped at 200 characters. Nothing in this repository sends such a code, so the module is a contributed one or a custom composition's: fix the code it answers with |
