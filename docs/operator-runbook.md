...
| Shared Redis down — **device-code store** (`redisDeviceCodeStoreModule`, `packages/redis/src/device-code-store.mts`; the in-memory adapter has no outage mode, and `multi` refuses it) | `POST /oauth/device_authorization`; `POST /oauth/device/verification`; the device polling `/oauth/token` | `/oauth/device_authorization`: `503 temporarily_unavailable` "the device authorization store is unavailable; retry later", at once — only the store's own collision signal is re-drawn (`packages/device-grant/src/deviceAuthorizationEndpoint.mts`); `/oauth/device/verification`: `500 server_error` `unexpected_error` (`packages/device-grant/src/module.mts`); `/oauth/token`: the grant's error reaches the host's error handler | `device_authorization_store_unavailable` (error); `device_route_unexpected_error` (error) | `commandTimeout` |
...
| `device_authorization_store_unavailable` (error) | `device-grant/src/deviceAuthorizationEndpoint.mts` | `POST /oauth/device_authorization` could not write the new authorization — the device-code store is down or timed out — and answered `503 temporarily_unavailable` without re-drawing a code. The same outage as the shared-Redis row above; a device retries on its own |
...
| `atdeny:<jti>` | string `"1"` | the revoked access token's **remaining** lifetime plus the five-minute verification clock tolerance (`DEFAULT_CLOCK_SKEW_MS`), the time it could still verify; a token past even that is not written (`packages/oauth/src/routes/revoke.mts`) | `packages/redis/src/access-token-denylist.mts` |
