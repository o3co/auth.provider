...
| `unhandled_request_error` (error) | `core/src/middleware/terminalError.mts` for the composed router's routes; `templates/standalone/src/terminalError.mts` for the template's own host routes (health, readiness, metrics) | a `500` you did not plan for — includes a signer (KMS) failure. A cookie-store failure is no longer one of them: it is `session_middleware_store_unavailable`. A body parser's refusal (`400` / `413` / `415`) is not logged |
