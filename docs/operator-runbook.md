...
| | `grant_type=authorization_code` | `503 temporarily_unavailable` — "authorization code store unavailable" when the code cannot be consumed (it used to reach the terminal handler as a `500`), "session store unavailable" when the code's session cannot be read (`packages/oauth/src/grants/authorization.mts`) | `authorization_grant_store_unavailable` (error, `store: "authorization_code"`, `step: "consume"` / `store: "user_session"`) | `commandTimeout` |
