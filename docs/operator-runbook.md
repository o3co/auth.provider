...
| `400 invalid_request` / `malformed_path` | A grant id in the path that Express could not percent-decode (`%zz`). Refused before the throttle for `token` and `revoke`. | Nothing, server-side: the caller built a bad URL. |
