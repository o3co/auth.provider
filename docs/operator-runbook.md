...
| `adapter_lifecycle_cleanup_failed` (error) with `cleanupIndex` and `err` | `core/src/adapters/AdapterFactory.mts` | an adapter's own cleanup — typically the close of a connection its builder opened — threw while `handle.dispose()` drained them, or while a failed boot did. One line per failed cleanup, through the `logger` component (to stderr when none is wired, or behind `[boot-failure lifecycle drain]` when boot failed); the drain goes on to the rest, and the dispose then rejects, which the standalone reports as `graceful shutdown: cleanup failed` |
