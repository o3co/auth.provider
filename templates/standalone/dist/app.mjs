...
import { AppConfigSchema, createApp, createDefaultFactories, createFederationTokenStoreFactory, createKeyStoreFactory, createUserSessionStoreFactory, registerBuiltinFederationTokenStores, registerBuiltinKeyStores, registerBuiltinUserSessionStores, } from "@o3co/auth-provider-core";
import { registerBuiltinAdapters } from "@o3co/auth-provider-foundation";
import { oauthAuthorizationModule, oauthModule, oauthSessionModule, } from "@o3co/auth-provider-oauth";
import { createSessionStoreFactory, registerBuiltinSessionStores, sessionModule, } from "@o3co/auth-provider-session";
...
const config = validate(parseFile(envConfPath).withFallback(parseFile(applicationConfPath)), AppConfigSchema);
const flattenAdapterConfig = (section) => {
...
await (async () => {
    ...
    const clientConfig = flattenAdapterConfig(config.repositories.client);
    ...
    const userRepository = await userFactory.create(flattenAdapterConfig(config.repositories.user));
    const codeRepository = await codeFactory.create(flattenAdapterConfig(config.repositories.code));
    ...
    const userSessionStore = await userSessionStoreFactory.create(flattenAdapterConfig(config
    ...
    const federationTokenStore = await federationTokenStoreFactory.create(flattenAdapterConfig(config
