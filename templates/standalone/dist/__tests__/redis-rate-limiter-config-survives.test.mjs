...
function resolveShippedConfig() {
    ...
    return validate(parseString(OPERATOR_OVERRIDES, { env: ENV })
...
function buildLimiter(config) {
    const provides = redisRateLimiterModule.provides;
    const provider = provides.rateLimiter;
    if (!provider)
        throw new Error("redisRateLimiterModule no longer provides rateLimiter");
