...
import { AppConfigSchema, createApp, createKeyStoreFactory, defineModule, InMemoryClientRepository, InMemoryUserRepository, registerBuiltinKeyStores, replicaUnsafeReason, } from "@o3co/auth-provider-core";
...
describe('#455: the standalone\'s memory modules are refused under deployment.mode = "multi"', () => {
    ...
    it("each standalone memory module declares its consequence on its own manifest", () => {
        ...
        const standaloneMemoryModules = modulesFor(config).filter((m) => m.name.startsWith("standalone:in-memory-"));
...
describe('#456: federationTokenStore.type = "redis" in the standalone', () => {
    ...
    it("reaches the resolved config from FEDERATION_TOKEN_STORE_TYPE", () => {
        ...
        expect(resolveConfig(without(ALL_REDIS_ENV, "FEDERATION_TOKEN_STORE_TYPE")).federationTokenStore
            ?.type).toBe("memory");
    });
    it("selects the Redis module and the shared clients module, not the memory one", () => {
        ...
        const providers = modules.filter((m) => Object.keys(m.provides ?? {}).includes("federationTokenStore"));
    ...
    it("fails at boot, naming the key, when the encryption key is missing", async () => {
        ...
        const config = resolveConfig(without(ALL_REDIS_ENV, "REDIS_FEDERATION_TOKEN_STORE_ENCRYPTION_KEY"));
        await expect(boot(config)).rejects.toSatisfy((err) => {
            const e = err;
...
describe("#473: the Redis federation store's plaintext guard, booted from the shipped config", () => {
    ...
    afterEach(async () => {
        ...
        if (origInsecure === undefined)
            ...
            process.env.FEDERATION_TOKENS_ALLOW_INSECURE = origInsecure;
        warnSpy.mockRestore();
    ...
    it('is refused under deployment.mode = "multi" in a development environment', async () => {
        ...
        await expect(boot(resolveConfig(PLAINTEXT_ENV), "development")).rejects.toSatisfy((err) => /deployment\.mode is "multi"/.test(messageChain(err)));
    ...
    it("is refused when the config was selected by CONFIG_ENV=production, whatever NODE_ENV says", async () => {
        ...
        await expect(boot(config, "production")).rejects.toSatisfy((err) => /the environment is "production"/.test(messageChain(err)));
    });
