...
import { AppConfigSchema, createApp, createKeyStoreFactory, defineModule, federationGrantAuthorizationRevision, federationGrantIdentityRevision, InMemoryClientRepository, InMemoryUserRepository, memoryRefreshTokenFamilyStoreModule, registerBuiltinKeyStores, } from "@o3co/auth-provider-core";
...
function resolveConfig() {
    ...
    const config = validate(parseFile(envConfPath, { env: ENV })
        .withFallback(parseFile(applicationConfPath, { env: ENV }))
        .withFallback(parseFile(resolveLibraryReferenceConfPath(), { env: ENV })), AppConfigSchema);
    return {
        ...config,
        federations: {
            ...config.federations,
...
describe("#593 AC8: a grant outlives the browser session at both logout endpoints (D14)", () => {
    ...
    async function login(app, config) {
        ...
        return { cookies, csrfToken: decodeURIComponent(reissued), headerName };
    }
    /** The session's `sid`, read off the access token the session grant mints for it. */
    async function sidOf(app, cookies) {
        ...
        const sid = claimsOf(res.body.access_token).sid;
        expect(typeof sid).toBe("string");
        return sid;
    }
    const disclose = (app) => request(app)
     ...
     * still answers.
     */
    async function expectGrantUntouched(app, store) {
    ...
    it("POST /session/logout ends the session's own records and leaves the grant spendable", async () => {
        const { app, config, federationGrantStore, userSessionStore, federationTokenStore } = await boot();
