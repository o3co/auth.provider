...
describe("GET /metrics", () => {
        ...
        try {
            for (const method of ["FOO", "M000001", "PURGE"]) {
                await new Promise((resolve) => {
                    const req = httpRequest({ host: "127.0.0.1", port, path: "/api/widgets/1", method }, (res) => {
                        res.resume();
    ...
    it("re-samples dependencies on every scrape instead of caching a verdict", async () => {
        ...
        expect((await request(app).get("/metrics")).text).toContain('auth_dependency_up{dependency="redis"} 1');
        healthy = false;
        expect((await request(app).get("/metrics")).text).toContain('auth_dependency_up{dependency="redis"} 0');
    });
