 ...
 */
import { createApp, createFederationTokenStoreFactory, createKeyStoreFactory, createUserSessionStoreFactory, generateToken, InMemoryClientRepository, InMemoryCodeRepository, InMemoryUserRepository, registerBuiltinFederationTokenStores, registerBuiltinKeyStores, registerBuiltinUserSessionStores, } from "@o3co/auth-provider-core";
...
describe("standalone smoke test", () => {
    ...
    it("POST /oauth/introspect returns iat in active token response", async () => {
        ...
        const { token } = await generateToken({}, { keyStore, subject: "u1", expiresIn: 3600, tokenType: "at+jwt" });
        const res = await request(app)
