...
const DELIBERATELY_UNSET = {
    OAUTH_AUTHORIZE_ALLOW_UNMARKED_CLIENTS: "#330 tombstone — any value must fail boot with migration instructions",
};
/**
...
function buildResolvedConfig(env, configEnv = "production") {
    ...
    return validate(parseFile(envConfPath, { env })
        ...
        .withFallback(parseFile(libraryReferencePath, { env })), AppConfigSchema);
}
/** Every `${?VAR}` in a HOCON layer, ignoring commented-out lines. */
function substitutionsIn(path) {
    const found = new Set();
 ...
 */
function documentedInReadme() {
    const found = new Set();
    for (const match of readFileSync(readmePath, "utf8").matchAll(/^\|\s*`([A-Z][A-Z0-9_]*)`\s*\|/gm)) {
        const name = match[1];
        if (name.includes("_"))
            found.add(name);
    }
...
describe("#288: the shipped config boots with every documented override supplied as a string", () => {
    ...
    describe("boolean overrides accept the spellings an operator writes", () => {
        ...
        it("fails boot on a spelling it does not recognise rather than guessing", () => {
            expect(() => buildResolvedConfig({ ...DOCUMENTED_ENV, SESSION_SECURE: "ture" })).toThrow(/true.*false/s);
        });
    ...
    describe("the documented surface and this suite cannot drift apart", () => {
        it("covers every substitution the shipped config layers declare", () => {
            const uncovered = [...liveSubstitutions()].filter((name) => !(name in DOCUMENTED_ENV) && !(name in DELIBERATELY_UNSET));
            expect(uncovered).toEqual([]);
        });
        it("covers every environment variable the README documents", () => {
            const uncovered = [...documentedInReadme()].filter((name) => !(name in DOCUMENTED_ENV) && !(name in DELIBERATELY_UNSET));
            expect(uncovered).toEqual([]);
        });
    ...
    describe("variables whose documented behaviour is to fail boot", () => {
        ...
        it("still refuses an empty SESSION_CSRF_TTL_SECONDS (#272)", () => {
            ...
            expect(() => buildResolvedConfig({ ...DOCUMENTED_ENV, SESSION_CSRF_TTL_SECONDS: "" })).toThrow();
        });
    });
