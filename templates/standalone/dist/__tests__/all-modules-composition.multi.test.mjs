...
import { compose, composedModules, MULTI_ENV, resolveConfig, } from "./all-modules-composition.fixture.mjs";
vi.mock("redis", () => ({
    createClient: vi.fn(() => ({
...
const MULTI = { env: MULTI_ENV, shippedRefreshTokenFamilyStore: true };
...
describe('every module on, every shared store on Redis, deployment.mode = "multi"', () => {
    ...
    it.each(MEMORY_SWITCHES)("%s=memory is refused at boot, naming %s", async (variable, module) => {
        await expect(compose({ ...MULTI, env: { ...MULTI_ENV, [variable]: "memory" } })).rejects.toMatchObject({
            name: "BootError",
    ...
    it("names every memory store together when every switch is memory", async () => {
        const env = { ...MULTI_ENV };
        for (const [variable] of MEMORY_SWITCHES)
            env[variable] = "memory";
...
describe("the shared Redis socket can back every store the Redis package ships", () => {
     ...
     */
    const storeModules = Object.entries(redisPackage).flatMap(([name, value]) => {
    ...
    it.each(storeModules.map((m) => [m.name, m]))("%s: its client slot is provided by standalone:redis-clients", (_name, module) => {
        ...
        for (const slot of clients)
            expect(provided, slot).toContain(slot);
    });
