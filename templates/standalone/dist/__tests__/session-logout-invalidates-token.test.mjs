...
import { createApp, createKeyStoreFactory, createReadinessRouter, defineModule, InMemoryClientRepository, InMemoryUserRepository, memoryRefreshTokenFamilyStoreModule, registerBuiltinKeyStores, } from "@o3co/auth-provider-core";
...
const testRepositoriesModule = defineModule({
    ...
    provides: {
        ...
        userRepository: () => new InMemoryUserRepository(new Map([[USERNAME, { id: "u-alice", password: PASSWORD, email: "alice@example.com" }]])),
    },
});
...
describe("POST /session/logout invalidates the session grant's access token", () => {
    ...
    async function login(app) {
        ...
        return {
            ...
            csrfToken: decodeURIComponent(reissued),
            headerName,
        };
    }
