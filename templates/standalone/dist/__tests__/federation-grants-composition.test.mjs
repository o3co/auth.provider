...
function resolveConfig(env) {
    ...
    const config = validate(parseFile(envConfPath, { env })
...
const modulesFor = (config, memoryOnly = false, environment) => buildModules(config, {
...
const boot = (config, memoryOnly = false, environment) => createApp({
    modules: modulesFor(config, memoryOnly, environment),
    bootstrapComponents: { config, pathResolver: (s) => s },
});
const names = (config, memoryOnly = false) => modulesFor(config, memoryOnly).map((m) => m.name);
const GRANT_MODULES = [
    "federation-grants",
...
describe("#593 slice 7: the standalone composes federation grants from its config", () => {
    ...
    it("installs nothing of the feature while it is off, whatever the switches say", async () => {
        ...
        for (const name of GRANT_MODULES)
    ...
    it("selects the Redis stores under the all-Redis environment, provides their clients, and boots under multi", async () => {
        ...
        expect(Object.keys(clients?.provides ?? {})).toEqual(expect.arrayContaining(["federationGrantStoreClient", "federationGrantIntentStoreClient"]));
