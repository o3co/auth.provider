...
import { ALICE, AS_LISTED, authorize, basic, codeFrom, compose, contributionNames, cookiesOf, DISCOVERY_PATHS, describeOutages, expectValidMetadata, FEDERATION_LANDING, FORM_TYPE, federatedCallback, ISSUER, inMonorepo, JSON_TYPE, KIB, knownDefect, lodgeGrant, login, M2M, padForm, padJson, REVERSED, redeem, SINGLE_ENV, TEMPLATE_DEPENDENCIES, THIRD, TOO_LARGE, TRANSFERS, WEB, WORKER, webTokens, withLength, } from "./all-modules-composition.fixture.mjs";
let current;
afterEach(async () => {
...
const TEMPLATE_PACKAGES = {
    "@o3co/auth-provider-core": "jwksModule, the memory stores, the refresh-token family rotation and revocation",
    ...
    "@o3co/auth-provider-oauth": "oauthModule, oauthSessionModule, oauthAuthorizationModule, subjectRevocationServiceModule",
    ...
    describe.runIf(inMonorepo)("in the monorepo", () => {
        it("places every workspace package: composed here, or listed as not in the template", () => {
            ...
            const workspace = readdirSync(packagesDir, { withFileTypes: true })
                ...
                .map((entry) => JSON.parse(readFileSync(new URL(`${entry.name}/package.json`, packagesDir), "utf8")).name)
                .sort();
            expect(workspace.length).toBeGreaterThan(0);
            expect(workspace).toEqual([...Object.keys(TEMPLATE_PACKAGES), ...Object.keys(NOT_IN_TEMPLATE)].sort());
        });
    });
});
// ---------------------------------------------------------------------------
...
describe("every module the template can turn on boots together", () => {
    ...
    it("mounts every route a module contributes", async () => {
        ...
        for (const module of contributing)
            expect(mountedBy, module.name).toContain(module.name);
    ...
    it("hands the deployment's logger to every module that answers a request", async () => {
        ...
        const { modules } = await boot();
        const answering = modules.filter((m) => contributionNames(m, "routes").length + contributionNames(m, "grants").length > 0);
        expect(answering.length).toBeGreaterThan(5);
        for (const module of answering) {
            expect([...(module.requires ?? []), ...(module.optional ?? [])], module.name).toContain("logger");
        }
    });
    it("materializes every slot a module provides and another reads", async () => {
        ...
        const expected = provided.filter((slot) => read.has(slot));
        expect(expected.length).toBeGreaterThan(10);
        const components = handle.components;
        for (const slot of expected)
            expect(components[slot], slot).toBeDefined();
    });
});
// ---------------------------------------------------------------------------
// Discovery
...
describe("discovery", () => {
    ...
    it("advertises only endpoints the composed app answers", async () => {
        ...
        const advertised = Object.entries(doc).filter(([field]) => field.endsWith("_endpoint") || field === "jwks_uri");
        ...
        for (const [field, url] of advertised) {
            ...
            expect([get.status, post.status].some((status) => status !== 404), field).toBe(true);
        }
    });
    /**
    ...
    it.each(GRANT_SWITCHES)("%s off: absent from grant_types_supported and refused at the token endpoint", async (grant, variable) => {
        const { app } = await boot({ env: { ...SINGLE_ENV, [variable]: "false" } });
        const doc = (await request(app).get(DISCOVERY_PATHS[0])).body;
        expectValidMetadata(doc);
        expect([...doc.grant_types_supported].sort()).toEqual(ENABLED_GRANTS.filter((g) => g !== grant));
    ...
    const withoutConnections = (config) => ({ ...config, federationGrants: { ...config.federationGrants, connections: {} } });
    ...
    it.each(FEATURE_SWITCHES)("%s off: its route answers 404 and the document does not change", async (_feature, variable, path, config) => {
    ...
    it("the OIDC federation off while a grant connection names it is refused at boot, naming both", async () => {
        await expect(compose({ env: { ...SINGLE_ENV, FEDERATIONS_OIDC_ENABLED: "false" } })).rejects.toSatisfy((err) => {
            const e = err;
            return (e.name === "BootError" &&
                /connections\.calendar: federation "oidc" is configured but disabled/.test(e.cause?.message ?? ""));
        });
    });
});
// ---------------------------------------------------------------------------
...
describe("every module's primary route answers in the one app", () => {
    ...
    ])("a login through %s (%s) ends in a session /authorize accepts", async (name, _federation) => {
    ...
    // completing.
    knownDefect("a federation enabled from the documented variables alone either refuses to boot or completes a login", async () => {
        const withoutLanding = (config) => {
            const federations = config.federations;
            const { clientUrl: _dropped, ...oidc } = federations.oidc ?? {};
            return { ...config, federations: { ...federations, oidc } };
        };
        let composed;
        try {
        ...
        const callback = await (await federatedCallback(composed.app, "oidc", composed.upstreams.oidc))();
        expect(callback.status).toBe(302);
    ...
    it("a federation grant is lodged, beside oauthModule under /oauth", async () => {
        ...
        expect(res.body.connect_uri).toMatch(new RegExp(`^${ISSUER}/session/federation-grants/connect\\?request=`));
    });
});
// ---------------------------------------------------------------------------
// Bodies and content types
// ---------------------------------------------------------------------------
describe.each([AS_LISTED, REVERSED])("bodies and content types, modules %s", (order) => {
    ...
    it("mounts in a different order from the other case, under /oauth and under /session", () => {
        ...
        for (const [neighbour, own] of [
            ...
            expect(ids.indexOf(neighbour) < ids.indexOf(own), `${neighbour} / ${own}`).toBe(order === AS_LISTED);
        }
    });
    it.each(TRANSFERS)("federation grants keep their 16 KiB bound beneath oauthModule's /oauth, a body sent %s", async (_transfer, send) => {
        for (const type of [JSON_TYPE, FORM_TYPE]) {
            const body = type === JSON_TYPE ? padJson(40 * KIB) : padForm(40 * KIB, "sub=local-subject");
    ...
    it.each(TRANSFERS)("the grants consent route keeps its 16 KiB bound beneath sessionModule's /session, a body sent %s", async (_transfer, send) => {
        const res = await send(composed.app, "/session/federation-grants/consent", JSON_TYPE, padJson(40 * KIB));
        expect(res.status).toBe(413);
        expect(res.body).toEqual({ error: "invalid_request", error_description: "body_too_large" });
    });
    it.each(TRANSFERS)("the token endpoint parses past a neighbour's 16 KiB and stops at its own 100 KiB, a body sent %s", async (_transfer, send) => {
        const headers = { authorization: basic(M2M) };
        const fields = "grant_type=client_credentials";
        const within = await send(composed.app, "/oauth/token", FORM_TYPE, padForm(50 * KIB, fields), headers);
        ...
        const over = await send(composed.app, "/oauth/token", FORM_TYPE, padForm(150 * KIB, fields), headers);
        expect(over.status).toBe(413);
        expect(over.body).toEqual(TOO_LARGE);
    });
    it.each(TRANSFERS)("the login route parses past a neighbour's 16 KiB and stops at its own 100 KiB, a body sent %s", async (_transfer, send) => {
        const fields = `username=${ALICE.username}&password=x`;
        const within = await send(composed.app, "/session/login", FORM_TYPE, padForm(50 * KIB, fields));
        ...
        const over = await send(composed.app, "/session/login", FORM_TYPE, padForm(150 * KIB, fields));
        expect(over.status).toBe(413);
        expect(over.body).toEqual(TOO_LARGE);
    });
    it("each module answers a body it cannot read with its own refusal", async () => {
        ...
        expect(await withLength(app, "/oauth/federation-grants", "text/plain", "hello", grants)).toEqual({
            status: 415,
        ...
        expect(await withLength(app, "/session/federation-grants/consent", JSON_TYPE, "{nope")).toEqual({
            status: 400,
            body: { error: "invalid_request", error_description: "malformed_body" },
        });
...
describe("a request body the OAuth endpoints do not parse", () => {
    ...
    ])("%s with %s is the client's mistake: 400 invalid_request, nothing logged", async (path, type) => {
...
// Outages
// ---------------------------------------------------------------------------
const tokenRequest = (app, client) => request(app).post("/oauth/token").set("Authorization", basic(client)).type("form");
/** The code exchange, with the store taken down just before the redeem. */
const codeExchange = async (app, outage) => {
    const { cookies } = await login(app);
...
const refresh = async (app, outage) => {
...
const oidcCallback = async (app, outage, c) => {
    const callback = await federatedCallback(app, "oidc", c.upstreams.oidc);
    outage.down = true;
    return callback();
};
const VERIFIER_WARN = "core's verifier (`verifyJwt`, `packages/core/src/jwt/verify.mts`) writes its own `jwt_verify_rejected` warn (`reason: \"revocation_unavailable\"`) beside the route's error line: two lines for one outage (the runbook's outage table documents both)";
const OUTAGES = [
    {
    ...
    },
    {
        ...
        defects: {
            "store-field": "packages/oauth `routes/authorize.mts`: `authorize_consent_store_unavailable` carries `clientId` and `err` but no `store` / `step` / `site` field",
        },
    },
    {
        module: "oauth (consent step)",
        ...
        defects: {
            "store-field": "packages/oauth `routes/authorize.mts`: `authorize_pending_consent_store_unavailable` carries `clientId` and `err` but no `store` / `step` / `site` field",
        },
    },
    {
        module: "session",
    ...
    },
    {
        ...
        storeFieldNotRequired: "the login succeeds; the line records a missed best-effort write, named by its event, with the subject and the session",
    },
    {
        module: "session",
    ...
    },
    {
        ...
        defects: {
            "store-field": "core's rate-limit guard (`packages/core/src/ratelimit/guard.mts`): `rate_limiter_failed_closed` names the limiter by `tag`, not `store` / `step` / `site`",
            projection: "core's rate-limit guard (`packages/core/src/ratelimit/guard.mts`): the limiter's error is flattened to a string under `error` (the projection's `detail` or `name`), not the projection under `err`",
        },
    },
    {
        ...
        run: async (app, outage) => {
            ...
            outage.down = true;
            return request(app)
                .post(`/oauth/federation-grants/${lodged.body.grant_id}/status`)
    ...
    },
    {
        ...
        defects: {
            "store-field": "core's JWKS route (`packages/core/src/jwks/router.mts`): `jwks_unavailable` carries `algorithm` and `err` but no `store` / `step` / `site` field naming the key store",
        },
    },
];
describeOutages("a store or repository outage answers 503 and is logged once, at error (#685)", OUTAGES, (outage) => compose({ outage }));
