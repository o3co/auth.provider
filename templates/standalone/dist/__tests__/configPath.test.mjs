...
describe("resolveConfigPaths", () => {
    it("accepts a configDirPath with a trailing slash (regression: fileURLToPath preserves trailing /)", () => {
        const { applicationConfPath, envConfPath } = resolveConfigPaths("/home/node/templates/standalone/config/", "production");
    ...
    it("accepts a configDirPath without a trailing slash", () => {
        const { applicationConfPath, envConfPath } = resolveConfigPaths("/home/node/templates/standalone/config", "development");
        expect(applicationConfPath).toBe("/home/node/templates/standalone/config/application.conf");
    ...
    it("rejects env names that resolve outside configDirPath (path traversal)", () => {
        expect(() => resolveConfigPaths("/home/node/templates/standalone/config/", "../secrets")).toThrow(/resolves outside/);
    });
    it("rejects env names containing a path separator", () => {
        expect(() => resolveConfigPaths("/home/node/templates/standalone/config/", "nested/env")).toThrow(/resolves outside/);
    });
});
