...
describe("POST /oauth/federation/google/token: an upstream outage never ends the session's Google tokens", () => {
	 ...
	 * and `body`.
	 */
	async function refreshAnswered(status: number, body: Record<string, unknown>) {
		const idp: FakeIdp = await createFakeIdp({ ...GOOGLE, clientId: GOOGLE_CLIENT_ID });
		...
		expect(
			idp.requests.filter(
				(r) =>
					`${r.url.origin}${r.url.pathname}` === GOOGLE.tokenEndpoint &&
					r.body?.get("grant_type") === "refresh_token",
			),
		).toHaveLength(1);
		return { res, ...session };
	}
