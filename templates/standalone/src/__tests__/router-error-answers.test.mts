...
const boot = async (): Promise<Composition> => {
...
const errorLines = (c: Composition) => c.logger.lines.filter((line) => line.level === "error");

describe("the composed router, mounted alone, answers a body parser's refusal", () => {
	it.each(PARSING_ROUTES)(
		"%s: malformed JSON is 400 in the envelope, nothing logged",
		async (path, headers) => {
			const c = await boot();
	...
	it.each(PARSING_ROUTES)(
		"%s: a body over the parser's limit is 413 in the envelope, nothing logged",
		async (path, headers) => {
			const c = await boot();
			const res = await request(c.app)
				.post(path)
