...
function loadEnv(opts = {}) {
  ...
  const source = opts.env ?? globalThis.process?.env ?? {};
...
function parseDotEnv(input, opts = {}) {
...
function dotEnvValue(v, origin, line, name) {
  const fail = (msg) => {
  ...
  if (v.startsWith('"')) {
    ...
    for (let i = 1; i < v.length; i++) {
      ...
      if (c === "\\") {
        i++;
        const e = v[i];
        if (e === void 0) fail("dangling \\ at end of line");
        else if (e === "n") out += "\n";
        else if (e === "r") out += "\r";
        else if (e === "t") out += "	";
        else if (e === "\\") out += "\\";
...
function checkName(name, origin, line) {
  const bad = /\p{White_Space}|#/u.exec(name);
  if (bad) {
    const what = bad[0] === "#" ? "'#'" : "whitespace";
    throw new ConfigError(
      `${origin}:${line}: variable name "${name}" contains ${what}; the line is not NAME=value (spec F1.7)`,
      name
    );
  }
}
