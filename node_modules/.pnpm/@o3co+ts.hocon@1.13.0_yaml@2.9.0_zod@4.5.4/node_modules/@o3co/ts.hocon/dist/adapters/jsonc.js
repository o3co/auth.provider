...
// src/adapters/jsonc.ts
function parseJsonc(input, originDescription) {
  const doc = guardStackDepth(
    () => JSON.parse(stripTrailingCommas(stripComments(stripBom(input))), bigIntReviver),
...
function isIntegerLiteral(source) {
  return !/[.eE]/.test(source);
}
var HAS_SOURCE_TEXT_ACCESS = (() => {
  let seen;
  JSON.parse("1", function(_k, v, context) {
    seen = context?.source;
    return v;
  });
  return seen !== void 0;
...
function bigIntReviver(key, value, context) {
  ...
  if (source === void 0) {
    ...
    throw new ConfigError(
      `jsonc: the integer at "${key || "<root>"}" needs more precision than a JS number holds, and this runtime does not expose JSON.parse source text (${HAS_SOURCE_TEXT_ACCESS ? "unexpected \u2014 the capability probe said otherwise" : "requires Node >= 22 or an engine with JSON.parse source access"}), so it cannot be read losslessly (spec F0.5)`,
      key
    );
  }
