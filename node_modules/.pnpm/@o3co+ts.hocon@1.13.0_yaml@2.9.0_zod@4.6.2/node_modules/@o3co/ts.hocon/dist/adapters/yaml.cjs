...
// src/adapters/yaml.ts
var yaml_exports = {};
__export(yaml_exports, {
  fromYamlValue: () => fromYamlValue,
  parseYaml: () => parseYaml
});
...
// src/adapters/yaml.ts
function parseYaml(input, originDescription) {
  const docs = import_yaml.default.parseAllDocuments(stripBom(input), { version: "1.2", merge: true, intAsBigInt: true });
  if (docs.length > 1) {
    throw new ConfigError(
  ...
  const doc = docs[0];
  if (doc === void 0) return fromMap({}, originDescription);
...
function fromYamlValue(value, originDescription) {
  if (value === null || value === void 0) return fromMap({}, originDescription);
  ...
  return fromMap(normalized, originDescription);
}
function convert(v, atPath) {
  ...
  if (v instanceof Map) {
    ...
    for (const [k, e] of v) {
      ...
      if (seen.has(key)) {
        throw new ConfigError(
          `yaml: sibling mapping keys ${keyForm(seen.get(key))} and ${keyForm(k)} both give the key "${key}"${atPath === "" ? "" : ` at "${atPath}"`}; rename one of them, because one of the two values would otherwise be lost. Quoting a non-string key helps only where that changes the key text, as 0x10 does and 1 does not (spec F5.3)`,
          atPath
        );
      }
      seen.set(key, k);
  ...
  if (v !== null && typeof v === "object") {
    return Object.fromEntries(
      Object.entries(v).map(
        ([k, e]) => [k, convert(e, atPath === "" ? k : `${atPath}.${k}`)]
      )
    );
  }
...
function isSafeIntegerBigInt(v) {
