...
__export(zod_exports, {
  ...
  parseFileWithSchema: () => parseFileWithSchema,
...
function getValidated(config, path2, schema) {
  const val = config.get(path2);
...
function getObjectShape(schema) {
  ...
  return typeof shape === "object" && shape !== null ? shape : void 0;
}
function getArrayElement(schema) {
  return schema._zod?.def?.element;
}
function parseWithSchema(input, schema, opts) {
  const config = parse(input, opts);
  return validate(config, schema);
}
function parseFileWithSchema(filePath, schema, opts) {
  ...
  switch (defType) {
    case "boolean":
      if (typeof value === "string") {
        ...
        return coerced !== void 0 ? coerced : value;
      }
      return value;
    case "number":
      if (typeof value === "string") {
