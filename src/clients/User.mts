...
export class UserClient extends RESTClient {
	...
	async authenticate({
		...
		return await this.post("/user/authenticate", {
			body: {
				email: username,
				password,
			},
	...
	async authenticateByToken(token: string): Promise<Record<string, unknown>> {
		return await this.post("/user/authenticate/token", {
			body: { token },
		});
	}
}
