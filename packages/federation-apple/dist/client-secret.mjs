...
export function createAppleClientSecret(options) {
    ...
    if (lifetimeSeconds > APPLE_CLIENT_SECRET_MAX_LIFETIME_SECONDS) {
        throw new Error(`Apple client secret lifetime must not exceed six months (${APPLE_CLIENT_SECRET_MAX_LIFETIME_SECONDS}s); Apple rejects a longer-lived assertion`);
    }
    const now = options.now ?? Date.now;
    let signingKey;
    const importSigningKey = () => {
        if (!signingKey) {
            signingKey = importPKCS8(options.privateKey, "ES256").catch((err) => {
