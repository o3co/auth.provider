...
import { clientAuthFor } from "./client-auth.mjs";
/**
...
export function checkFederationName(name) {
    if (typeof name !== "string" || !NAME_PATTERN.test(name)) {
        throw new Error(`OIDC federation name must be one URL path segment of letters, digits, ".", "_" or "-", got ${JSON.stringify(name)}`);
    }
}
...
const stringArray = (value) => Array.isArray(value) && value.every((entry) => typeof entry === "string")
    ? value
    : undefined;
function parseIssuer(label, issuer) {
    ...
    if (url.protocol !== "https:" &&
        ...
        throw new Error(`${label}: issuer must be an https URL (plain http is accepted only on a loopback host), got ${issuer}`);
    }
    return url;
}
function checkScopes(label, scopes) {
    ...
    if (!scopes.includes("openid")) {
        throw new Error(`${label}: scopes must include "openid" — without it the IdP issues no id_token`);
    }
    return [...scopes];
}
const REQUIRED_METADATA = ["authorization_endpoint", "token_endpoint", "jwks_uri"];
const OVERRIDE_KEYS = [
...
async function resolveServerMetadata(label, issuerUrl, config, clientMetadata, clientAuth, insecure) {
    ...
    if (config.discovery === false) {
        ...
        try {
            ...
            throw new Error(`${label}: discovery of ${config.issuer} failed and the provider cannot start without the issuer's metadata ` +
                `(set discovery = false and the endpoints to run from hand-typed values): ${message(err)}`, { cause: err });
        }
        // The helper methods on the discovered object do not survive a spread; the data does.
        const base = JSON.parse(JSON.stringify(discovered.serverMetadata()));
        metadata = { ...base, ...mapped };
    }
    for (const key of REQUIRED_METADATA) {
        if (typeof metadata[key] !== "string") {
            throw new Error(`${label}: the issuer's metadata has no ${key}; the provider cannot ${key === "jwks_uri" ? "verify id_tokens" : "run"} without it (set endpoints to override)`);
        }
    }
    return metadata;
}
export async function createOidcProvider(name, config) {
    ...
    const metadata = await resolveServerMetadata(label, issuerUrl, config, clientMetadata, clientAuth, insecure);
    const configuration = new oidc.Configuration(metadata, config.clientId, clientMetadata, clientAuth);
    if (config.fetch)
        configuration[oidc.customFetch] = config.fetch;
    if (insecure)
    ...
    if (config.userInfo === true && !hasUserInfo) {
        throw new Error(`${label}: userInfo = true but the issuer publishes no userinfo_endpoint (set endpoints.userinfoEndpoint, or drop userInfo)`);
    }
    ...
    const requireNonce = (nonce) => {
        if (typeof nonce !== "string" || nonce.length === 0) {
            throw new Error(`${label} requires a non-empty nonce — OIDC Core §3.1.3.7 binds the id_token to the session through it.`);
        }
        return nonce;
    };
    ...
    return {
        ...
        async exchangeCode(params) {
            ...
            if (!claims)
                throw new Error(`${label}: the token response carried no id_token`);
