...
import { createFederationRedirectPolicy, extractFederationSection, } from "@o3co/auth-provider-session";
import { checkFederationName, createOidcProvider, } from "./oidc.mjs";
/** The `type` a `federations.<name>` section names to select this provider. */
export const OIDC_FEDERATION_TYPE = "oidc";
function entryFor(configs, name) {
    const entry = configs?.[name];
    if (entry === undefined) {
        throw new Error(`oidcFederationConfigs has no entry for "${name}" — federation:oidc:${name} is in the manifest, so the composition root must supply its config under that name (readOidcFederationConfigs builds the slot from config.federations)`);
    }
    return entry;
}
/**
 ...
 */
export function oidcFederationModule(name) {
    checkFederationName(name);
    return defineModule({
        name: `federation:oidc:${name}`,
        requires: ["oidcFederationConfigs"],
        contributes: {
            federations: {
                [name]: (deps) => createOidcProvider(name, entryFor(deps.oidcFederationConfigs, name)),
            },
            federationRedirectPolicies: {
                [name]: (deps) => createFederationRedirectPolicy(entryFor(deps.oidcFederationConfigs, name)),
            },
        },
    });
}
/** Names of every enabled `federations.<name>` section of type `oidc`, sorted. */
export function oidcFederationNames(federations) {
    if (!federations)
        return [];
...
function readSection(name, slice) {
    const at = (field) => `federations.${name}.${field}`;
    const present = (field) => slice[field] !== undefined && slice[field] !== null;
    ...
    const optionalString = (field) => {
        ...
        if (typeof value !== "string")
            throw new Error(`${at(field)} must be a string when present`);
        return value;
    };
    ...
    const optionalStringList = (field) => {
        if (!present(field))
            return undefined;
        const value = slice[field];
    ...
    if ((clientSecret === undefined) === (privateKey === undefined)) {
        throw new Error(`federations.${name} must set exactly one of clientSecret (client_secret_basic) or privateKey (private_key_jwt)`);
    }
    let endpoints;
    if (present("endpoints")) {
        const raw = slice.endpoints;
        if (!isObject(raw))
            throw new Error(`${at("endpoints")} must be an object of endpoint URLs`);
        const out = {};
        for (const key of Object.keys(raw)) {
            if (!ENDPOINT_KEYS.includes(key)) {
                throw new Error(`${at("endpoints")} has an unknown key "${key}" (expected ${ENDPOINT_KEYS.join(", ")})`);
            }
            const value = raw[key];
            if (value === undefined || value === null)
                continue;
            if (typeof value !== "string")
                throw new Error(`${at(`endpoints.${key}`)} must be a string`);
            out[key] = value;
        }
        endpoints = out;
    }
 ...
 */
export function readOidcFederationConfigs(federations) {
