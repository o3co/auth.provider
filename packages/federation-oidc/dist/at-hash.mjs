...
export function computeAtHash(accessToken, alg) {
...
export function verifyAtHash(label, idToken, accessToken, atHash) {
    ...
    if (presented.length !== wanted.length || !timingSafeEqual(presented, wanted)) {
        throw new Error(`${label}: id_token at_hash does not match the access token it was issued with (OIDC Core §3.3.2.11)`);
    }
}
