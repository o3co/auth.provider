...
import { checkAlgorithmPolicy, checkSignatureAlgorithm, } from "./algorithms.mjs";
import { checkCriticalExtensions, checkOcspCriticalExtensions, extensionValueParsed, } from "./criticalExtensions.mjs";
 ...
 */
export const ocspResponders = (certificate) => {
    ...
    const urls = (parsed?.accessDescriptions ?? [])
        ...
        .filter((value) => typeof value === "string" && isHttpUrl(value));
    if (urls.length === 0) {
        return {
            ok: false,
            reason: "no_responder",
...
const equalBytes = (a, b) => a.byteLength === b.byteLength && a.every((byte, index) => byte === b[index]);
...
const issuerKeyId = (issuer) => createHash("sha256")
    ...
    .digest("hex");
const serialHex = (certificate) => Buffer.from(certificate.serialNumber.valueBlock.valueHexView).toString("hex");
/** Node's view of a certificate — what `checkAlgorithmPolicy` reads the key size from. */
const toNode = (certificate) => new X509Certificate(Buffer.from(certificate.toSchema(true).toBER(false)));
const buildRequest = async (certificate, issuer, crypto) => {
...
const isNamedResponder = async (candidate, responderId, crypto) => {
    ...
    if (responderId instanceof asn1js.OctetString) {
        const hash = await crypto.digest({ name: "SHA-1" }, 
 ...
 */
const checkDelegatedResponder = async (candidate, issuer, now, crypto, algorithms) => {
    const notIssued = "the responder certificate was not issued by the certificate's issuing CA (RFC 6960 §4.2.2.2)";
    ...
    if (!issued)
        return { ok: false, reason: "bad_signature", detail: notIssued };
    if (candidate.notBefore.value.getTime() > now.getTime() ||
    ...
    if (eku === undefined ||
        ...
        return {
            ok: false,
            reason: "bad_signature",
            detail: "the responder certificate does not carry id-kp-OCSPSigning in extendedKeyUsage " +
                "(RFC 6960 §4.2.2.2)",
        };
    }
    ...
    const algorithm = checkAlgorithmPolicy(toNode(candidate), candidate.signatureAlgorithm.algorithmId, algorithms);
    if (!algorithm.ok) {
        return {
            ok: false,
...
const identifySigner = async (basic, issuer, now, crypto, algorithms) => {
    const responderId = basic.tbsResponseData.responderID;
    if (await isNamedResponder(issuer, responderId, crypto))
        return { ok: true, signer: issuer };
    for (const candidate of basic.certs ?? []) {
        if (!(await isNamedResponder(candidate, responderId, crypto)))
            continue;
    ...
    return {
        ok: false,
        reason: "bad_signature",
        detail: "the response names a responder that is neither the issuing CA nor a certificate " +
            "attached to the response",
    };
};
const verifySignature = async (basic, signer, crypto) => {
    let verified = false;
    try {
        verified = await crypto.verifyWithPublicKey(basic.tbsResponseData.tbsView, basic.signature, signer.subjectPublicKeyInfo, basic.signatureAlgorithm);
    }
    catch (err) {
...
const freshness = (single, now, cacheTtlSeconds) => {
...
export const createOcspResolver = (options) => {
    const cache = new Map();
    ...
    const remember = (key, reason, detail, now) => store(key, {
        kind: "unavailable",
        reason,
        detail,
    ...
    const query = async (url, certificate, issuer, now) => {
        ...
        if (!signer.ok)
            return { ok: false, reason: signer.reason, detail: signer.detail };
        ...
        if (!decoded.ok)
            return { ok: false, reason: "unparseable", detail: decoded.detail };
    ...
    const lookup = async (url, certificate, issuer, issuerId, serial, now) => {
        ...
        if (RESPONDER_FAILURES.has(answer.reason)) {
            remember(responderDownKey(url), answer.reason, answer.detail, now);
        }
        else if (CERTIFICATE_FAILURES.has(answer.reason)) {
            remember(certificateDownKey(url, issuerId, serial), answer.reason, answer.detail, now);
        }
    ...
    return {
        ...
        resolve: async (certificate, issuer, now) => {
            ...
            for (const url of responders.urls) {
                ...
                if (answer.ok)
                    return { ok: true, responder: url, status: answer.status };
