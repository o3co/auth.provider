    ...
    for (let i = 1; i < path.length; i++) {
        ...
        if (intermediatesBelow > limit) {
            return {
                ...
                detail: `a CA at depth ${i} permits ${limit} intermediate CA(s) below it, ` +
...
export const createFullPkiValidator = (options) => {
    const trustedCerts = options.trustedCas.map(toPkijs);
    const resolver = options.revocation.mode === "crl"
        ? createCrlResolver({
    ...
    return {
        ...
        validate: async (leaf, chain, now) => {
            ...
            if (Buffer.compare(Buffer.from(path[0]?.tbsView ?? []), Buffer.from(certs[certs.length - 1]?.tbsView ?? [])) !== 0) {
                return {
                    ok: false,
                    step: "path validation failed",
            ...
            if (!critical.ok)
                return critical;
            const leafKeyUsage = checkLeafKeyUsage(path[0]);
            if (!leafKeyUsage.ok)
                return leafKeyUsage;
            ...
            for (const certificate of path) {
                const check = checkAlgorithmPolicy(toNode(certificate), certificate.signatureAlgorithm.algorithmId, options.algorithms);
                if (!check.ok)
                    return { ok: false, step: check.step, detail: check.detail };
            }
            if (resolver === null || options.revocation.mode === "disabled")
                return { ok: true };
            ...
            const lookups = await Promise.all(subjects.map((certificate, index) => resolver.resolve(certificate, path[index + 1], now)));
            for (const [index, certificate] of subjects.entries()) {
                ...
                if (!lookup.ok) {
                    ...
                    if (options.revocation.onUnavailable === "reject") {
                        options.logger?.warn({ subject, reason: lookup.reason, detail: lookup.detail }, "mtls_revocation_unavailable_rejected");
                        return {
                            ok: false,
                    ...
                    // configuration.
                    options.logger?.warn({ subject, reason: lookup.reason, detail: lookup.detail }, "mtls_revocation_unavailable_allowed");
                    continue;
                }
                ...
                if (lookup.crls.some((crl) => crl.isCertificateRevoked(certificate))) {
                    return {
                        ...
                        detail: `${toNode(certificate).subject}: listed on the CRL published by ` +
