 ...
 * treated as literal.
 */
const isUrlEncoded = (value) => /%[0-9A-Fa-f]{2}/.test(value);
/**
 ...
 */
const safeDecodeURIComponent = (value, field) => {
    try {
 ...
 */
const unquoteXfccField = (raw, fieldName) => {
    if (raw.length === 0)
        return raw;
 ...
 */
export const parseEnvoyXfccHeader = (value) => {
    ...
    if (rawByteLen > MAX_RAW_HEADER_BYTES) {
        throw new Error(`XFCC header value exceeds size cap (${rawByteLen} > ${MAX_RAW_HEADER_BYTES} bytes)`);
    }
    ...
    if (certByteLen > MAX_DECODED_PAYLOAD_BYTES) {
        throw new Error(`XFCC Cert= decoded payload exceeds size cap (${certByteLen} > ${MAX_DECODED_PAYLOAD_BYTES} bytes)`);
    }
    const rawChain = fields.get("Chain");
    const chainPem = rawChain !== undefined
    ...
    if (chainPem !== undefined) {
        ...
        if (chainByteLen > MAX_DECODED_PAYLOAD_BYTES) {
            throw new Error(`XFCC Chain= decoded payload exceeds size cap (${chainByteLen} > ${MAX_DECODED_PAYLOAD_BYTES} bytes)`);
        }
    }
 ...
 */
export const parsePlainPemHeader = (value) => {
    ...
    const rawByteLen = Buffer.byteLength(value, "utf8");
    if (rawByteLen > MAX_RAW_HEADER_BYTES) {
        throw new Error(`plain-pem header value exceeds size cap (${rawByteLen} > ${MAX_RAW_HEADER_BYTES} bytes)`);
    }
    ...
    if (decodedByteLen > MAX_DECODED_PAYLOAD_BYTES) {
        throw new Error(`plain-pem decoded payload exceeds size cap (${decodedByteLen} > ${MAX_DECODED_PAYLOAD_BYTES} bytes)`);
    }
    ...
    if (beginCount > 1) {
        throw new Error("plain-pem header contains multiple PEM blocks; use the envoy dialect for chain transport");
    }
