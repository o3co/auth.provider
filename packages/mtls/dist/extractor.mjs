...
const DEFAULT_DIALECT = "envoy";
const isTlsLikeSocket = (s) => typeof s === "object" &&
    s !== null &&
    typeof s.getPeerCertificate === "function";
// ---------------------------------------------------------------------------
// Factory
// ---------------------------------------------------------------------------
/**
 ...
 * Per spec §8 + §11.2.
 */
export const createMtlsMechanism = (options) => {
    const certHeader = options.certHeader ?? DEFAULT_CERT_HEADER;
    const dialect = options.certHeaderDialect ?? DEFAULT_DIALECT;
    ...
    if (mode === "pki") {
        ...
        if (!trustedCas || trustedCas.length === 0) {
            throw new Error('createMtlsMechanism: mode = "pki" requires a non-empty trustedCas list. ' +
                "Without trusted CAs, chain validation cannot proceed.");
        }
        if (source === "tls-layer") {
            throw new Error('createMtlsMechanism: mode = "pki" with source = "tls-layer" is not supported in Phase 3. ' +
        ...
        ? // biome-ignore lint/style/noNonNullAssertion: boot-time check above guarantees defined when mode === "pki"
            options.trustedCas.map((entry, index) => {
                ...
                try {
                    ...
                    throw new Error(`createMtlsMechanism: trustedCas[${index}] is not a parseable X.509 certificate: ${err.message}`);
                }
            })
        : [];
    return {
        ...
        extract: async (req) => {
            ...
            if (source === "header") {
                ...
                try {
                    ...
                    throw new MtlsError("malformed_header", `${dialect} header parse failure: ${err.message}`, { dialect });
                }
                ...
                if (!isTlsLikeSocket(socket) || socket.getPeerCertificate === undefined) {
                    throw new MtlsError("tls_peer_unavailable", "request socket does not expose getPeerCertificate() — mTLS requires a TLS-terminated connection");
                }
            ...
            if (certPem !== undefined) {
                try {
                    ...
                    throw new MtlsError("cert_decode_failed", `PEM decode failed: ${err.message}`);
                }
            }
            ...
            try {
                ...
                throw new MtlsError("cert_decode_failed", `DER parse failed: ${err.message}`);
            }
            if (chainPem !== undefined) {
                ...
                try {
                    ...
                    throw new MtlsError("cert_decode_failed", `Chain= entry DER parse failed: ${err.message}`);
                }
            }
            // --- Step 4: Validity window ---
            const now = new Date();
            if (now < new Date(x509.validFrom)) {
                throw new MtlsError("cert_not_yet_valid", `client certificate notBefore (${x509.validFrom}) is in the future`, { notBefore: x509.validFrom });
            }
            if (now > new Date(x509.validTo)) {
                throw new MtlsError("cert_expired", `client certificate notAfter (${x509.validTo}) is in the past`, { notAfter: x509.validTo });
            }
            ...
            if (mode === "pki") {
                ...
                if (!result.ok) {
                    ...
                    throw new MtlsError("chain_validation_failed", `client certificate failed PKI chain validation: ${result.step}`, { step: result.step });
                }
            }
 ...
 */
const resolveTrustedCaEntry = (entry, index) => {
    if (entry.startsWith("file:")) {
        ...
        try {
            ...
            throw new Error(`createMtlsMechanism: trustedCas[${index}] = "${entry}": failed to read file at ${path}: ${err.message}`);
        }
    }
    return entry;
};
