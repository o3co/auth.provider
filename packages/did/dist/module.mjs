 ...
 */
export const oauthDidModule = (options) => ({
    name: "oauth-did",
    async init(context) {
        const grantConfig = context.config.oauth.grants.did;
        if (grantConfig?.enabled === false)
            return;
        const resolver = "resolver" in options ? options.resolver : options.resolverFactory(grantConfig ?? {});
