 ...
 */
export function detectAlgorithm(body) {
    // Explicit algorithm field takes precedence over shape-based detection
    if (typeof body.algorithm === "string" && body.algorithm)
        return body.algorithm;
    if (body.signature && body.message) {
        if (body.prehash === "sha256")
            return "ed25519_prehash";
        return "ed25519_raw";
    }
    if (body.jws) {
        try {
            const header = decodeProtectedHeader(body.jws);
            switch (header.alg) {
                case "EdDSA":
                    return "ed25519_jws";
                case "ES256":
