 ...
 */
function buildResolver(did, publicKeyBytes) {
 ...
 */
async function makeSignedCtx(did, overrides = {}) {
...
describe("createDidGrant", () => {
    ...
    describe("config defaults", () => {
        it("uses default messageMaxAgeSec and algorithm when did config is absent", async () => {
            ...
            const handler = createDidGrant({ config: noDIDConfig, keyStore: createSymmetricKeyStore("test-secret") }, { resolver });
        ...
        it("uses defaults when messageMaxAgeSec and algorithm are missing from did config", async () => {
            ...
            const handler = createDidGrant({ config: partialConfig, keyStore: createSymmetricKeyStore("test-secret") }, { resolver });
            expect(typeof handler.handle).toBe("function");
        });
    });
    describe("cleanup", () => {
    ...
    describe("handle – multi-algorithm support", () => {
         ...
         */
        async function makeJwsCtx(did, overrides = {}) {
        ...
        it("accepts ed25519_raw when supportedAlgorithms includes it", async () => {
            ...
            const handler = createDidGrant({ config, keyStore: createSymmetricKeyStore("test-secret") }, { resolver });
            const { result } = await handler.handle(ctx);
            expect(result.status).toBe(200);
        });
    ...
    describe("handle – custom verifierRegistry", () => {
        it("uses injected verifierRegistry instead of default", async () => {
            ...
            const mockVerifier = {
                async verify(ctx) {
                    verifyCalled = true;
                    const parsedMessage = JSON.parse(ctx.body.message);
                    return {
                        valid: true,
                        subject: ctx.did,
            ...
            const handler = createDidGrant({ config, keyStore: createSymmetricKeyStore("test-secret") }, { resolver, verifierRegistry: customRegistry });
            const message = JSON.stringify({
                did,
                timestamp: new Date().toISOString(),
    ...
    describe("handle – audience allowlist", () => {
        function makeConfigWithAllowedAudiences(allowedAudiences) {
            return {
                oauth: {
                    jwt: { secret: "test-secret" },
                    accessToken: { expiresIn: 3600 },
