...
export const createDidGrant = (deps, options) => {
    const { config, keyStore } = deps;
    const { resolver } = options;
    const DEFAULT_MESSAGE_MAX_AGE_SEC = 300;
    const DEFAULT_ALGORITHM = "ed25519_raw";
    const didConfig = config.oauth.grants
        .did;
    const messageMaxAgeMs = (didConfig?.messageMaxAgeSec ?? DEFAULT_MESSAGE_MAX_AGE_SEC) * 1000;
    const allowedAudiences = didConfig?.allowedAudiences ?? [];
    ...
    const rawSupported = didConfig?.supportedAlgorithms;
    const rawAlgorithm = didConfig?.algorithm;
    const supportedAlgorithms = rawSupported ?? (rawAlgorithm ? [rawAlgorithm] : [DEFAULT_ALGORITHM]);
    // Validate all configured algorithms are registered
    for (const alg of supportedAlgorithms) {
        if (!verifierRegistry.has(alg)) {
            throw new Error(`Invalid DID grant algorithm: "${alg}". Supported: ${verifierRegistry.algorithms().join(", ")}`);
        }
    }
    ...
    const cleanupInterval = setInterval(() => {
        ...
        for (const [key, time] of nonceStore) {
            if (now - time > messageMaxAgeMs)
                nonceStore.delete(key);
        }
    }, 60 * 1000);
    // Per-algorithm verifier cache: created lazily on first use
    const verifierCache = new Map();
    const verifierErrorCache = new Map();
    const getVerifier = async (algorithm) => {
    ...
    return {
        async handle(ctx) {
            ...
            const detectedAlgorithm = detectAlgorithm(body);
