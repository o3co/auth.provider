...
| `POST /oauth/device/verification` — `findPendingByUserCode` (`lookup`), `approve`, `deny` | anything | `503 temporarily_unavailable`, the same description; nothing is decided | `device_verification_store_unavailable` (error, with the `action`) |
| `POST /oauth/token`, the device-code grant — `poll` | anything | `503 temporarily_unavailable`, the same description, written by the token endpoint as every grant's error is | `device_code_grant_store_unavailable` (error, with the `clientId`) |
