...
const error = (status, code, description) => ({
    result: { status, error: code, errorDescription: description },
});
export const createDeviceCodeGrant = (options) => {
    ...
    return {
        async handle(ctx) {
            ...
            switch (outcome.status) {
                ...
                case "expired":
                    return error(400, "expired_token", "the device_code has expired; start a new device authorization request");
                case "denied":
                    return error(400, "access_denied", "the end user denied this authorization request");
                case "pending":
                    return error(400, "authorization_pending", "the end user has not yet completed the authorization");
                case "slow_down":
                    return error(400, "slow_down", `polling too frequently; the interval is now ${outcome.intervalSeconds} seconds`);
                case "approved":
                    break;
            }
            ...
            if (authorization.clientId !== client.clientId) {
                options.logger?.warn({ expected: authorization.clientId, presented: client.clientId }, "device_code_client_mismatch");
