...
- **`400 invalid_target`** for an `audience` outside the client's allowlist (note 3) or outside the subject token's audience (`audience_widening_not_allowed`, note 5), and a `resource` that is not the issued audience (note 6).
...
A `GrantPolicyHook` deny carries the hook's own `error` (`403` for `access_denied`). A decision the policy was not entitled to make is `500 server_error`: a `grantedScope` past the subject token or the registration (`scope_widening_not_allowed`, note 5) or a non-array decision. That is core's `policyOutOfBounds`, the answer every grant gives a policy that exceeds its ceiling — the caller did nothing wrong, and the request's own `scope` past either ceiling is `invalid_scope` before the policy runs.
