...
A `GrantPolicyHook` deny carries the hook's own `error` (`403` for `access_denied`). A decision the policy was not entitled to make is `500 server_error`: a `grantedScope` past the subject token or the registration (`scope_widening_not_allowed`, note 5), a `grantedAudience` past the subject token's audience (`policy returned audiences outside the subject_token audience: …`, note 5), or a non-array decision. That is core's `policyOutOfBounds`, the answer every grant gives a policy that exceeds its ceiling — the caller did nothing wrong. The request's own `scope` past either ceiling is `invalid_scope` and its own `audience` past the subject token's is `invalid_target`, so who asked for more decides the answer.

Because `invalid_request` covers both a malformed request and a refused token, the `error_description` is what tells a client which check refused it. The descriptions quoted in this README are part of the contract. Every `invalid_request` goes through one function, `invalidRequest` in [`grant.mts`](./src/grant.mts). The `refresh_token` grant answers the same binding and family rows `invalid_grant`, which is RFC 6749's code for a refresh token.
...
3. **Audience allowlist (client-scoped).** The `audience` request parameter must be in `client.allowedAudiences ∪ { client.clientId }`, and refused with `400 invalid_target` / `audience "<aud>" is not allowed for this client` otherwise. The handler enforces this before the policy hook runs. Empty `allowedAudiences` means only the client's own `clientId` is a valid exchange audience. The allowlist bounds the request only; a policy's `grantedAudience` is bounded by the subject token's audience instead (note 5).
...
5. **Policy hook widening is always rejected.** The `GrantPolicyHook.evaluate()` result's `grantedScope` and `grantedAudience` are still allowed to override the request-derived values, but the handler always re-checks them before minting: `grantedScope` against **both** the validated `subject_token` scope and `client.allowedScopes`, `grantedAudience` against the subject token's audience boundary. A scope widening is `500 server_error` / `scope_widening_not_allowed: <scopes>`: only the policy can reach it, since the request's own `scope` is held to both ceilings first (note 2), so it is the deployment's policy exceeding its authority, answered with core's `policyOutOfBounds` as every other grant answers it. An audience widening is answered by who named the audience. A `grantedAudience` is checked before it replaces the request's audience, and one past the subject token's is `500 server_error` / `policy returned audiences outside the subject_token audience: <audiences>`: core's `policyOutOfBounds`, which every other grant gives a policy audience past its ceiling. The request's own `audience` past the subject token's — one the client's allowlist names but the subject token does not carry — is `400 invalid_target` / `audience_widening_not_allowed: <audiences>`, the code RFC 8693 §2.2.2 gives a target the server will not issue for. There is no opt-in to bypass either check.
...
12. **Missing subject claim rejection.** Self-issued access_tokens without a `sub` claim (or with an empty-string `sub`) are rejected with `invalid_request` / `subject_token validation failed`. This prevents a silently-anonymous token from reaching downstream services.
