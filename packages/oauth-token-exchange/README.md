...
- **`400 invalid_request` when the `subject_token` or the `actor_token` is refused.** [RFC 8693 §2.2.2](https://datatracker.ietf.org/doc/html/rfc8693#section-2.2.2) requires this code for a token that is "invalid for any reason, or [...] unacceptable based on policy", whichever check refused it: validation (`subject_token validation failed`, note 20), the DPoP and mTLS binding matrices (`subject_token requires a DPoP proof`, `DPoP proof does not match subject_token binding`, `subject_token has compound cnf binding which is not supported (Stage 1)` and their `client certificate` and `actor_token` forms), the family rule (note 1), `may_act` (note 8), the actor-chain bound (note 9) and the subject's expiry (note 16). A malformed request is `invalid_request` too: a missing parameter, `actor_token` and `actor_token_type` not sent together, a `client_id` or `client_secret` that is not one string, a malformed `expires_in`.
- **`400 invalid_target`** for an `audience` outside the client's allowlist (note 3), a `resource` that is not the issued audience (note 6), and a policy decision that widens scope or audience (note 5).
- **`400 invalid_scope`** for a requested `scope` outside the subject token's or the client's (note 2).
- **`400 unsupported_token_type`** for a `subject_token_type` or `actor_token_type` no validator is registered for, and for a `requested_token_type` other than `access_token`.
...
A `GrantPolicyHook` deny carries the hook's own `error` (`403` for `access_denied`), and a malformed decision is `500 server_error`.

Because `invalid_request` covers both a malformed request and a refused token, the `error_description` is what tells a client which check refused it. The descriptions quoted in this README are part of the contract. Every token refusal goes through one function, `tokenRefusal` in [`grant.mts`](./src/grant.mts). The `refresh_token` grant answers the same binding and family rows `invalid_grant`, which is RFC 6749's code for a refresh token.
