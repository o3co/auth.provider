 ...
 */
export function createSelfIssuedAccessTokenValidator(options) {
    const { keyStore, refreshTokenFamilyRevocation, issuer } = options;
    if (typeof issuer !== "string" || issuer.length === 0) {
        throw new Error("createSelfIssuedAccessTokenValidator: issuer is required (a non-empty string). Without an issuer to compare against, an at+jwt signed by the same KeyStore but with a different `iss` claim could be accepted.");
    }
    return {
        async validate(token, _context) {
            let payload;
            let header;
            try {
