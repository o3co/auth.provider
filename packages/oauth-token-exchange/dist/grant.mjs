...
export function createTokenExchangeGrant(deps) {
    ...
    return {
        async handle(ctx) {
            ...
            const actorValidator = actorToken !== null && actorTokenType !== null
            ...
            if (subjectValidated.familyId &&
                ...
                return {
                    result: {
                        status: 400,
                        error: "invalid_grant",
                        errorDescription: "refresh token family revocation not configured (revocation cannot be verified)",
                    },
                };
            }
            ...
            if (subjectValidated.familyId &&
                ...
                let revoked;
                try {
                    revoked = await deps.refreshTokenFamilyRevocation.isFamilyRevoked(subjectValidated.familyId);
                }
                catch {
                    return {
                        result: {
            ...
            const requestedScope = requestedScopeRaw !== null && requestedScopeRaw.length === 0 ? null : requestedScopeRaw;
            if (requestedScope) {
            ...
            let grantedScope = requestedScope ?? subjectScope;
            let grantedAudience = requestedAudience ?? undefined;
            if (deps.grantPolicy) {
                const policyRequest = {
                    ...
                    actorTokenType: actorValidated !== null && actorTokenType !== null ? actorTokenType : undefined,
            ...
            const audienceForToken = (() => {
                ...
                const single = typeof subjectAud === "string"
                    ? subjectAud
...
function getExpiresIn(deps) {
    ...
    const top = deps.config.oauth.accessToken?.expiresIn;
