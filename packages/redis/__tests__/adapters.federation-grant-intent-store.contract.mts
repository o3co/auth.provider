...
/**
 * Conformance suite for `FederationGrantIntentStore` (#593, D16) — the copy
 * `@o3co/auth-provider-redis` runs against its adapter.
 *
 * Duplicated from
 * `packages/core/src/federation-grants/__tests__/intentStore.contract.mts`,
 * differing only in how it imports the port: a contract file cannot be
 * imported across a package boundary (see `docs/adapter-surface.md`,
 * "Proving an implementation").
 *
 * The two are held in step MECHANICALLY, by
 * `__tests__/federation-grant-intent-contract-parity.test.mts`. Nothing but the
 * import block may differ.
 */
