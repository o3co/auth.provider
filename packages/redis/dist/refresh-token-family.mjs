 ...
 */
import { defineModule, RefreshTokenStorageError, } from "@o3co/auth-provider-core";
import { z } from "zod";
const serialize = (fam) => JSON.stringify(fam);
const deserialize = (raw) => Object.freeze(JSON.parse(raw));
/**
 ...
 * Per A3 §7.2.
 */
export function createRedisRefreshTokenFamilyStore(opts) {
    const { client, keyPrefix } = opts;
    const casRetryLimit = opts.casRetryLimit ?? 3;
    const fullKey = (familyId) => `${keyPrefix}${familyId}`;
    return {
        kind: "redis",
        async registerFamily(family) {
            ...
            const result = await client.set(fullKey(family.familyId), serialize(family), "PX", ttlMs, "NX");
            if (result === null) {
        ...
        async findFamily(familyId) {
            ...
            if (pttl <= 0)
                return null; // -2 nonexistent, -1 no-TTL (defensive), 0 expired
        ...
        async updateFamily(familyId, updater) {
            const key = fullKey(familyId);
 ...
 */
export const redisRefreshTokenFamilyStoreBuilder = (config, _ctx) => {
    const c = config;
...
export const redisRefreshTokenFamilyStoreModule = defineModule({
    ...
    requires: ["refreshTokenFamilyClient", "config"],
    ...
    provides: {
        refreshTokenFamilyStore: (deps) => {
            const cfg = deps.config.redisRefreshTokenFamilyStore;
            return createRedisRefreshTokenFamilyStore({
