 ...
 */
import { canonicalChallengeKey, defineModule, PENDING_CONSENT_PER_SESSION_LIMIT, } from "@o3co/auth-provider-core";
import { z } from "zod";
/**
...
const safetyNetTtlMs = (expiresAt, nowMs) => Math.ceil(expiresAt - nowMs) + CONSENT_EXPIRY_SLACK_MS;
/**
...
const toConsentRecord = (sub, clientId, fields) => {
    if (!isString(fields.scopes) || !isString(fields.grantedAt))
        return null;
    let scopes;
    try {
    ...
    if (fields.expiresAt === undefined)
        return { sub, clientId, scopes, grantedAt };
 ...
 */
const toPendingRecord = (json, challenge) => {
    let parsed;
    try {
...
export function createRedisConsentStore(opts) {
    const { client, keyPrefix } = opts;
    const key = (sub, clientId) => `${keyPrefix}rec:${canonicalChallengeKey(sub, clientId)}`;
    return {
        kind: "redis",
...
export function createRedisPendingConsentStore(opts) {
    const { client, keyPrefix } = opts;
    const keys = {
 ...
 */
export const redisConsentStoreBuilder = (config, _ctx) => {
    const c = config;
 ...
 */
export const redisPendingConsentStoreBuilder = (config, _ctx) => {
    const c = config;
    if (!c.client) {
...
export const redisConsentStoreModule = defineModule({
    ...
    requires: ["consentStoreClient", "pendingConsentStoreClient", "config"],
