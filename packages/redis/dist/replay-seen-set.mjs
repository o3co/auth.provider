 ...
 * Per A1 §7.2.
 */
export function createRedisReplaySeenSet(opts) {
    const { client, keyPrefix } = opts;
    const fullKey = (scope, key) => `${keyPrefix}${canonicalChallengeKey(scope, key)}`;
    return {
        kind: "redis",
 ...
 */
export const redisReplaySeenSetBuilder = (config, _ctx) => {
    const c = config;
...
export const redisReplaySeenSetModule = defineModule({
    ...
    requires: ["replaySeenSetClient", "config"],
