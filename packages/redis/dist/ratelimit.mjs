...
export function createRedisRateLimiter(opts) {
    ...
    const defaultLimit = opts.defaultLimit ?? { limit: 60, windowSeconds: 60 };
 ...
 */
export const redisRateLimiterBuilder = (config, _ctx) => {
    const cfg = config;
    if (!cfg.client) {
        throw new Error('Rate limiter "redis" requires config.client; the built-in limiter does not create its own redis client because RateLimiterBase has no disposal hook.');
    }
...
export const redisRateLimiterModule = defineModule({
    ...
    requires: ["rateLimiterClient", "config"],
    ...
    provides: {
        rateLimiter: (deps) => {
            const cfg = deps.config.redisRateLimiter;
            return createRedisRateLimiter({
