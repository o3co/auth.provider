 ...
 */
import { DEFAULT_FEDERATION_GRANT_TOMBSTONE_RETENTION_MS, defineModule, withinFederationGrantLifetimeCeiling, } from "@o3co/auth-provider-core";
import { z } from "zod";
import { openSealedCredential, sealCredential, } from "./internal/crypto.mjs";
import { validateEncryptionMode, } from "./internal/encryption-mode.mjs";
import { canonicalAuthorization, credentialAad, decodeCredentials, encodeCredentials, parseCanonicalAuthorization, } from "./internal/federation-grant-codec.mjs";
...
const RANGE = (what) => new RangeError(`federation grant store: ${what}`);
...
const parseBase = (text) => {
    ...
    if (!Array.isArray(parsed) || parsed.length !== 5)
        return undefined;
    const [id, subject, clientId, connection, createdAtMs] = parsed;
    if (typeof id !== "string" ||
...
const parseMarker = (text) => {
    ...
    if (!Array.isArray(parsed) || parsed.length !== 3)
        return undefined;
    const [reason, atMs, judgedAgainst] = parsed;
...
const encodeMarker = (marker) => JSON.stringify([marker.reason, String(marker.at.getTime()), String(marker.judgedAgainst)]);
/**
...
grantId) {
    ...
    if (base === undefined || version === undefined)
        return undefined;
    if (grantId !== undefined && base.id !== grantId)
        return undefined;
    ...
    const intent = fields.intentHandle !== undefined && intentExpiresAtMs !== undefined
        ? { handle: fields.intentHandle, expiresAtMs: intentExpiresAtMs }
        : undefined;
    const revocation = status === "revoked" && revokedAt !== undefined && revokedBy !== undefined
        ? {
            status: "revoked",
            revocation: { by: revokedBy, at: revokedAt },
        }
        : undefined;
    if (status === "revoked" && revocation === undefined)
        return undefined;
    if (fields.authorization === undefined) {
        // Never authorized: `pending`, or revoked before it ever was.
        if (status !== "pending" && status !== "revoked")
            return undefined;
        const horizonMs = status === "pending"
            ? (intentExpiresAtMs ?? Number.NaN)
            : revokedAt.getTime() + retentionMs;
        const pending = {
            ...base,
            version,
            ...(status === "pending" ? { status: "pending" } : revocation),
        };
        return {
            grant: pending,
            horizonMs,
    ...
    const refreshFailure = failureAt !== undefined && fields.failureKind !== undefined && failureCount !== undefined
        ? {
            at: failureAt,
    ...
    return {
        ...
        guardsAgree: fields.identityRevision === authorization.identityRevision &&
 ...
 */
export function createRedisFederationGrantStore(options) {
    ...
    const retentionMs = options.tombstoneRetentionMs ?? DEFAULT_FEDERATION_GRANT_TOMBSTONE_RETENTION_MS;
    if (!Number.isFinite(retentionMs) || retentionMs < 0) {
        throw new Error("federation grant store: tombstoneRetentionMs must be a non-negative finite number");
    }
    ...
    if (!Number.isFinite(allowanceMs) || allowanceMs < 0) {
        throw new Error("federation grant store: listingAllowanceMs must be a non-negative finite number");
    }
    ...
    const ring = options.encryption.mode === "required"
    ...
    const grantKey = (id) => `${keyPrefix}{${segment(id)}}:grant`;
    const credKey = (id) => `${keyPrefix}{${segment(id)}}:cred`;
    const lockKey = (id) => `${keyPrefix}{${segment(id)}}:lock`;
    const indexKey = (subject) => `${keyPrefix}sub:${segment(subject)}`;
    const member = (id) => segment(id);
    const lock = createFederationGrantLock({ client, lockKey });
    const aadFor = (decoded, authorizationText, grantId) => credentialAad({
    ...
    const seal = (credentials, binding) => {
        ...
        return sealCredential(payload, ring, credentialAad({ credentialKey: credKey(binding.id), ...binding }));
    };
    /** What `open` and `inspect` say about the credential they read beside the record. */
    const openCredential = (decoded, credential, nowMs, grantId) => {
        ...
        if (decoded.grant.status !== "active")
            return { state: "absent" };
        const authorization = decoded.authorization;
        if (authorization === undefined)
            return { state: "absent" };
        if (!(nowMs < authorization.expiresAt.getTime()))
            return { state: "absent" };
        if (credential === null)
            return { state: "absent" };
        ...
        if (!decoded.guardsAgree)
            return { state: "unreadable" };
        const text = decoded.authorizationText;
        let payload;
        if (options.encryption.mode === "allow-plaintext") {
            if (!credential.startsWith(PLAINTEXT_PREFIX))
                return { state: "unreadable" };
            payload = Buffer.from(credential.slice(PLAINTEXT_PREFIX.length), "base64url").toString("utf8");
        }
        else {
    ...
    const visible = (decoded, nowMs) => decoded !== undefined && nowMs < decoded.horizonMs ? decoded : undefined;
    const read = async (grantId, nowMs) => {
    ...
    const horizonOf = (status, ms, recordRetentionMs = retentionMs) => (status === "pending" ? ms : ms + recordRetentionMs);
    return {
        kind: "redis",
        async createPending(input) {
            ...
            if (!isDate(input.intent.expiresAt))
                return { ok: false };
            ...
            await client.reserve(indexKey(input.subject), member(input.id), horizonOf("pending", intentExpiresAtMs), allowanceMs);
            return written(await client.createPending(grantKey(input.id), credKey(input.id), {
                nowMs,
                base: JSON.stringify([
                    input.id,
                    input.subject,
        ...
        async nameIntent(input) {
            ...
            if (current === undefined || !current.guardsAgree)
                return { ok: false };
            return written(await client.nameIntent(grantKey(input.grantId), {
                nowMs,
                handle: input.intent.handle,
        ...
        async isCurrentIntent(grantId, handle, now) {
            ...
            if (found === null)
                return false;
            const { decoded } = found;
            if (decoded.grant.status === "revoked")
                return false;
        ...
        async open(grantId, now) {
            ...
            return {
                ...
                credentials: opened.state === "ok" ? { state: "ok", value: opened.value } : { state: opened.state },
            };
        },
        async activate(input) {
            ...
            if (!withinFederationGrantLifetimeCeiling(authorization.consent.at, authorization.expiresAt)) {
                return { ok: false };
            }
            // Not dated after this write, with no allowance: a revocation boundary
            // stamped from now on must always cover the consent (D13).
            if (!(authorization.consent.at.getTime() <= nowMs))
                return { ok: false };
            if (!(authorization.authorizedAt.getTime() <= nowMs))
                return { ok: false };
            if (!isDate(authorization.expiresAt))
                return { ok: false };
            const access = input.credentials.accessToken;
            if (access !== undefined && !isDate(access.obtainedAt))
                return { ok: false };
            ...
            await client.reserve(indexKey(current.base.subject), member(current.base.id), horizonOf("authorized", authorization.expiresAt.getTime(), current.retentionMs), allowanceMs);
            return written(await client.activate(grantKey(input.grantId), credKey(input.grantId), {
                nowMs,
        ...
        async replaceCredentials(input) {
            ...
            if (access !== undefined && !isDate(access.obtainedAt))
                return { ok: false };
            if (input.ineligible !== null && !isDate(input.ineligible.at))
                return { ok: false };
            const snapshot = await client.snapshot(grantKey(input.grantId), credKey(input.grantId));
            if (snapshot === null)
                return { ok: false };
            const current = decode(snapshot.fields, input.grantId);
            if (current === undefined || current.authorizationText === undefined)
                return { ok: false };
            if (!current.guardsAgree)
                return { ok: false };
            ...
            return written(await client.replaceCredentials(grantKey(input.grantId), credKey(input.grantId), {
                nowMs,
        ...
        async requireReauthorization(input) {
            return written(await client.requireReauthorization(grantKey(input.grantId), credKey(input.grantId), {
                nowMs: instant(input.now, "now"),
                expectedVersion: input.expectedVersion,
            }), input.grantId);
        },
        async revoke(grantId, by, at) {
            ...
            if (current !== undefined && current.grant.status === "pending") {
                await client.reserve(indexKey(current.base.subject), member(current.base.id), horizonOf("revoked", atMs, current.retentionMs), allowanceMs);
            }
            return written(await client.revoke(grantKey(grantId), credKey(grantId), { atMs, by }), grantId);
        },
        async noteRefreshFailure(input) {
            const nowMs = instant(input.now, "now");
            if (!isDate(input.failure.at))
                return { ok: false };
            return written(await client.noteRefreshFailure(grantKey(input.grantId), {
                nowMs,
                expectedVersion: input.expectedVersion,
        ...
        async acquireRefreshLock(grantId, bounds) {
 ...
 */
const durationFromEnv = (bounds) => z.preprocess((value) => {
 ...
 * already consented.
 */
const keyMaterial = (id, encoded) => {
    const refuse = () => {
        throw new Error(`federation grant store: encryption key "${id}" must be canonical base64 of ${KEY_BYTES} bytes`);
    };
 ...
 * days of them.
 */
export function resolveRedisFederationGrantStoreOptions(rawConfig, moduleOptions) {
 ...
 * configuration file.
 */
export function redisFederationGrantStoreModuleFor(options = {}) {
    return defineModule({
        name: "redis-federation-grant-store",
        requires: ["federationGrantStoreClient", "config"],
        configSchema: moduleConfigSchema,
        provides: {
            federationGrantStore: (deps) => createRedisFederationGrantStore({
