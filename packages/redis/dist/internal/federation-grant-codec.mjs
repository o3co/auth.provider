...
const ms = (value) => String(value.getTime());
...
const stringFrom = (value) => typeof value === "string" ? value : undefined;
const scopesFrom = (value) => Array.isArray(value) && value.every((entry) => typeof entry === "string")
    ? value
    : undefined;
 ...
 */
export function canonicalAuthorization(authorization) {
 ...
 */
export function parseCanonicalAuthorization(text) {
    let parsed;
    try {
...
export function decodeCredentials(text) {
    let parsed;
    try {
        parsed = JSON.parse(text);
    }
    ...
    if (!Array.isArray(parsed) || parsed.length !== 3 || parsed[0] !== 1)
        return undefined;
    const refreshToken = stringFrom(parsed[1]);
    if (refreshToken === undefined || refreshToken.length === 0)
        return undefined;
    if (!Array.isArray(parsed[2]) || parsed[2].length > 1)
        return undefined;
    if (parsed[2].length === 0)
        return { refreshToken };
    const token = parsed[2][0];
