 ...
 */
import { AUDIT_SINK_ABSENCE_POLICY, defineModule, fullSectionsSchema, requireFederationGrantSubjectRevocation, resolveFederationGrantAcquisitionLimits, resolveFederationGrantRetrievalLimits, } from "@o3co/auth-provider-core";
import { supportsDelegatedAuthorization } from "@o3co/auth-provider-session";
import { z } from "zod";
import { requireFederationGrantIdentityLookup, requireFederationGrantIntentStore, resolveFederationGrantAcquisitionSettings, } from "./acquisitionSettings.mjs";
import { createFederationGrantBackground } from "./background.mjs";
import { createDisabledFederationGrantBrowserRouter, createFederationGrantBrowserRouter, FEDERATION_GRANTS_BROWSER_MOUNT_PATH, } from "./browserRoutes.mjs";
...
const isEnabled = (deps) => deps.config.federationGrants?.enabled === true;
/**
...
const requireStore = (deps) => {
    if (deps.federationGrantStore === undefined) {
        throw new Error("federationGrantsModule: federationGrants.enabled = true requires a " +
 ...
 */
const requireLimiter = (deps) => {
    if (deps.rateLimiter === undefined) {
        throw new Error("federationGrantsModule: federationGrants.enabled = true requires a rateLimiter " +
...
const requireFailMode = (deps) => {
    ...
    if (failMode !== "open" && failMode !== "closed") {
        throw new Error('federationGrantsModule: federationGrants.enabled = true requires rateLimit.failMode ("open" | "closed"). ' +
 ...
 */
const requireDelegatedCapability = (deps, connections) => {
    const providers = deps.federationProviders;
    for (const connection of connections.values()) {
        const provider = providers?.get(connection.federation);
        if (provider === undefined) {
            throw new Error(`federationGrantsModule: federationGrants.connections.${connection.name} names the ` +
        ...
        if (!supportsDelegatedAuthorization(provider)) {
            throw new Error(`federationGrantsModule: the federation "${connection.federation}", named by ` +
 ...
 */
const requireAuditDecision = (deps) => {
    if (deps.auditSink !== undefined)
        return;
    const declared = deps.config?.audit?.sink?.type;
    if (declared === AUDIT_SINK_ABSENCE_POLICY.absentValue)
        return;
    throw new Error("federationGrantsModule: federationGrants.enabled = true with no auditSink component. " +
...
const authorizerFor = (deps) => (federation) => {
    const providers = deps.federationProviders;
 ...
 */
const requireUserSessionStore = (deps) => {
    const store = deps.userSessionStore;
    if (store === undefined) {
        throw new Error("federationGrantsModule: federation grants are enabled and no userSessionStore is installed. " +
...
const refresherFor = (deps) => (connection) => {
    const providers = deps.federationProviders;
    const provider = providers?.get(connection.federation);
    if (!supportsDelegatedAuthorization(provider))
        return undefined;
    return {
        refreshDelegatedToken: (params) => provider.refreshDelegatedToken(params),
    };
};
/**
 ...
 */
const sessionsBoundaryFor = (revocation) => async (subject) => {
    const watermark = await revocation.revokedBefore(subject);
    if (watermark === null)
        return null;
    if (watermark instanceof Date && !Number.isNaN(watermark.getTime()))
        return watermark;
    throw new Error("federationGrantsModule: the subjectRevocation adapter answered something that is neither " +
        "a date nor null for the subject's sessions boundary. Fails closed (D13).");
};
const boundaryFor = (revocation) => async (subject) => {
    ...
    if (watermark instanceof Date && !Number.isNaN(watermark.getTime()))
        return watermark;
    throw new Error("federationGrantsModule: the subjectRevocation adapter answered something that is " +
...
export const federationGrantBackgroundModule = defineModule({
     ...
     */
    optional: ["federationGrantStore", "subjectRevocation", "auditSink"],
    provides: {
...
export const federationGrantsModule = defineModule({
    ...
    requires: ["config", "federationGrantBackground", "clientRepository"],
    optional: [
    ...
    contributes: {
        routes: [
            (deps) => {
                ...
                requireFederationGrantIdentityLookup(acquisition.identityLookup, deps.userRepository, connections);
