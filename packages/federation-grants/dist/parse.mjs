...
const single = (value, field) => {
    ...
    if (Array.isArray(value))
        return { ok: false, description: `duplicate_${field}` };
    if (typeof value !== "string")
        return { ok: false, description: `invalid_${field}` };
    return { ok: true, value };
};
export function parseFederationGrantTokenRequest(body) {
    ...
    for (const field of Object.keys(fields)) {
        ...
        if (!KNOWN.has(field))
            return refuse("unexpected_parameter");
    }
    const subRead = single(fields.sub, "sub");
    if (!subRead.ok)
        return refuse(subRead.description);
    ...
    if (sub === undefined || sub === "")
        return refuse("sub_required");
    const connectionRead = single(fields.connection, "connection");
    if (!connectionRead.ok)
        return refuse(connectionRead.description);
    const connection = connectionRead.value;
    if (connection === "")
        return refuse("invalid_connection");
    const resourceRead = single(fields.resource, "resource");
    if (!resourceRead.ok)
        return refuse(resourceRead.description);
    const resource = resourceRead.value;
    if (resource === "")
        return refuse("invalid_resource");
    const scopeRead = single(fields.scope, "scope");
    if (!scopeRead.ok)
        return refuse(scopeRead.description);
    ...
    if (rawMinTtl !== undefined) {
        if (Array.isArray(rawMinTtl))
            return refuse("duplicate_min_ttl");
 ...
 */
export function parseFederationGrantStatusRequest(body) {
 ...
 */
export function parseFederationGrantRevokeRequest(body) {
 ...
 * branch on.
 */
function parseWithout(body, known) {
    if (typeof body !== "object" || body === null || Array.isArray(body)) {
        return refuse("invalid_body");
    }
...
const lodgingRefuse = (description) => ({
    ok: false,
    description,
});
/**
...
function parseLodging(body, requireConnection) {
    if (typeof body !== "object" || body === null || Array.isArray(body)) {
    ...
    for (const field of Object.keys(fields)) {
        ...
        if (!LODGING_KNOWN.has(field))
            return lodgingRefuse("unexpected_parameter");
    }
    const read = (field) => single(fields[field], field);
    const sub = read("sub");
    if (!sub.ok)
        return lodgingRefuse(sub.description);
    if (sub.value === undefined || sub.value === "")
        return lodgingRefuse("sub_required");
    const connection = read("connection");
    if (!connection.ok)
        return lodgingRefuse(connection.description);
    if (connection.value === "")
        return lodgingRefuse("invalid_connection");
    if (requireConnection && connection.value === undefined)
        return lodgingRefuse("connection_required");
    const redirect = read("redirect_uri");
    if (!redirect.ok)
        return lodgingRefuse(redirect.description);
    if (redirect.value === undefined || redirect.value === "") {
        return lodgingRefuse("redirect_uri_required");
    }
    const state = read("state");
    if (!state.ok)
        return lodgingRefuse(state.description);
    ...
    if (state.value === undefined || state.value === "")
        return lodgingRefuse("state_required");
    const scope = read("scope");
    if (!scope.ok)
        return lodgingRefuse(scope.description);
    let scopes;
    if (scope.value !== undefined) {
        ...
        if (tokens.length === 0)
            return lodgingRefuse("invalid_scope");
        scopes = tokens;
    }
    let expiresInSeconds;
    if (fields.expires_in !== undefined) {
        if (Array.isArray(fields.expires_in))
            return lodgingRefuse("duplicate_expires_in");
        expiresInSeconds = seconds(fields.expires_in);
        if (expiresInSeconds === undefined)
            return lodgingRefuse("invalid_expires_in");
    }
    const upstream = read("upstream_sub");
    if (!upstream.ok)
        return lodgingRefuse(upstream.description);
    if (upstream.value === "")
        return lodgingRefuse("invalid_upstream_sub");
    return {
        ok: true,
        value: {
            subject: sub.value,
...
export function parseFederationGrantCreateRequest(body) {
 ...
 */
export function parseFederationGrantReauthorizeRequest(body) {
