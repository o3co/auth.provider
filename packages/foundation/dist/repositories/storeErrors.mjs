...
export class StoreCredentialRefusedError extends Error {
    ...
    constructor(url, status) {
        super(`HttpUserRepository: the Store at ${endpointForMessage(url)} refused this deployment's credential ` +
...
export function transportCode(err) {
...
const withCode = (message, code) => code === undefined ? message : `${message} (${code})`;
/**
 ...
 */
export function requestFailure(err, messages) {
    const code = transportCode(err);
    const reason = requestFailureReason(err);
    const message = reason === "malformed_response"
        ? messages.malformed
...
export function readFailure(err, message) {
    const code = transportCode(err);
