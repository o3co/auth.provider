...
describe("HttpUserRepository", () => {
    ...
    describe("authenticate", () => {
        ...
        it("sends email field (not username) in request body", async () => {
            ...
            server.use(http.post(`${BASE_URL}/user/authenticate`, async ({ request }) => {
        ...
        it("throws on unexpected HTTP status", async () => {
            server.use(http.post(`${BASE_URL}/user/authenticate`, () => {
                return new HttpResponse(null, { status: 500 });
            }));
            await expect(repo.authenticate("alice@example.com", "pass")).rejects.toThrow("Unexpected HTTP status 500");
        });
    });
