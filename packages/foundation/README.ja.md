...
**`Bearer` チャレンジ付きの `401` は資格情報の拒否である。** `bearerToken` が設定されているとき、四つのエンドポイントのどれかから `WWW-Authenticate: Bearer …`（RFC 6750 §3）を持つ `401` が返ると、この節が `401` に与える読み — 「ユーザーが居ない」、リンクの拒否、照会の `answered HTTP 401` — のどれでもなく、`HttpUserRepository: the Store at <url> refused this deployment's credential (HTTP 401 with a Bearer challenge) — …` という例外になる。呼び出し元は他の Store の障害と同じく扱い — `503 temporarily_unavailable` — 、それをログに出す。チャレンジは大文字小文字を問わず、他のチャレンジと並んでいても、独立したヘッダー行にあっても見つけ、別のチャレンジの引用符付きパラメーターの中にあるものは数えない。Store がスキームの後に書いたものは何も繰り返さない。`Bearer` チャレンジの無い `401`、およびトークンが設定されていないときのあらゆる `401` は、この節が与える意味を保つので、トークンを検査しない Store には影響しない。
