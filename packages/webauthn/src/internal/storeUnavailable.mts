...
export const storeUnavailableDescription = (store: WebAuthnStore): string => DESCRIPTIONS[store];
 ...
 */
export function refuseCeremonyStoreUnavailable(
	res: Response,
	logger: Pick<Logger, "error">,
	failure: CeremonyStoreFailure,
	cause: unknown,
): void {
	logger.error(
		{ site: failure.site, store: failure.store, step: failure.step, err: loggableError(cause) },
		"webauthn_ceremony_store_unavailable",
	);
	res.status(503).json({
