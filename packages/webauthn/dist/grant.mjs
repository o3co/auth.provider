 ...
 */
export const createWebAuthnGrant = (deps) => {
    ...
    return {
        async handle(ctx) {
            ...
            if (authenticatedClientForCheck &&
            ...
            // ------------------------------------------------------------------
            const ceremonyOutcome = await deps.challengeCeremony.consume("webauthn:authentication", challengeValue);
            if (ceremonyOutcome.outcome !== "consumed") {
                return {
                    result: {
                        status: 400,
                        error: "invalid_grant",
                        errorDescription: ceremonyOutcome.outcome === "replayed" ? "challenge_replayed" : "challenge_unknown",
                    },
                };
            }
            // ------------------------------------------------------------------
            ...
            let policyGrantedAudience = null;
            if (deps.grantPolicy) {
            ...
            const audience = policyGrantedAudience ??
 ...
 */
function parseAssertionBody(raw) {
    ...
    const responseObj = innerResponse;
    ...
    let challengeValue;
    try {
        const clientDataJSON = JSON.parse(Buffer.from(clientDataJSONBase64, "base64url").toString("utf8"));
