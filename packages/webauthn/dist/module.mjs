    ...
    contributes: {
        grants: {
            [WEBAUTHN_GRANT_TYPE]: (deps) => {
                ...
                if (!deps.grantPolicy) {
                    throw new Error("webauthn grant requires `grantPolicy` to be wired. " +
        ...
        routes: [
            ...
            (deps) => {
                ...
                router.post("/", createRegistrationOptionsHandler({
            ...
            (deps) => {
                const router = express.Router();
                router.use(express.json({ limit: "100kb" }));
                router.post("/", createRegistrationVerifyHandler({
                    config: deps.webauthnConfig,
                    challengeCeremony: deps.challengeCeremony,
                    credentialStore: deps.webauthnCredentialStore,
                }));
            ...
            (deps) => {
                const router = express.Router();
                router.use(express.json({ limit: "100kb" }));
                router.post("/", createAuthenticationOptionsHandler({
                    config: deps.webauthnConfig,
                    challengeStore: deps.challengeStore,
                    credentialStore: deps.webauthnCredentialStore,
                }));
