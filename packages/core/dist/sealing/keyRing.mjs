 ...
 */
export const isSealingKeyId = (id) => typeof id === "string" && KEY_ID_PATTERN.test(id);
/**
 ...
 */
export function checkSealingKeyRing(ring, setting) {
    const seen = new Set();
    ring.forEach((entry, index) => {
        if (!isSealingKeyId(entry.id)) {
            throw new RangeError(`${setting} has an encryption key id at index ${index} that does not match ${KEY_ID_PATTERN.source}`);
        }
        ...
        if (!Buffer.isBuffer(entry.key) || entry.key.length !== SEALING_KEY_BYTES) {
            throw new RangeError(`${setting} has an encryption key at index ${index} that is not a Buffer of ${SEALING_KEY_BYTES} bytes`);
        }
    });
}
/**
 ...
 */
export function decodeSealingKey(encoded) {
    if (typeof encoded !== "string" || /\s/.test(encoded))
        return undefined;
