 ...
 * @internal
 */
function checkMaterialisedRouteCollisions(routes) {
 ...
 */
function computeMountOrder(routes) {
    if (routes.length === 0) {
        return [];
    }
    ...
    const inDegree = new Array(routes.length).fill(0);
    // adjacency[i] = set of nodes that must come AFTER node i
    const adjacency = Array.from({ length: routes.length }, () => new Set());
    for (let i = 0; i < routes.length; i++) {
        const r = routes[i];
        if (r === undefined)
            continue;
    ...
    while (ready.length > 0) {
        ...
        if (newlyReady.length > 0) {
            ...
            newlyReady.sort((a, b) => (routes[a]?.declarationIndex ?? 0) - (routes[b]?.declarationIndex ?? 0));
...
function buildDispose(frozen) {
    ...
    return function dispose() {
        ...
        cachedPromise = (async () => {
            ...
            // rejection — see §6.3").
            const errorsWithOrigin = [];
            // Step 1: iterate cleanups in reverse order (§8.1 steps 1-2).
            const reversedCleanups = [...frozen.cleanups].reverse();
            ...
            const explicitCleanupKeys = new Set(frozen.cleanups.map((r) => r.componentKey));
            for (const [key, value] of Object.entries(frozen.components)) {
                if (explicitCleanupKeys.has(key)) {
                    // Explicit cleanup was declared for this key; skip Symbol.asyncDispose.
                    continue;
                }
                if (frozen.externalKeys.has(key)) {
            ...
            if (errorsWithOrigin.length > 0) {
                const originSummary = errorsWithOrigin
                    ...
                    .join(", ");
                throw new AggregateError(errorsWithOrigin.map((e) => e.error), `AppHandle.dispose: ${errorsWithOrigin.length} cleanup error${errorsWithOrigin.length === 1 ? "" : "s"} (${originSummary})`);
            }
        })();
        return cachedPromise;
    };
 ...
 * Per A2-β §5.6 + §6.3 + §8.1.
 */
export function assembleApp(frozen, options = {}) {
    ...
    try {
        ...
        const expressModule = req("express");
    ...
    if (RouterCtor === undefined) {
        throw new Error("assembleApp: cannot resolve express. Install `express` as a peer dependency or provide options.express.Router.");
    }
    const router = RouterCtor();
    // Mount each route contribution in mount-index order.
    for (const orderedRoute of ordered) {
        router.use(orderedRoute.contribution.mountPath, orderedRoute.contribution.handler);
    }
        ...
        listen(port) {
            return new Promise((resolve, reject) => {
                if (expressFactory === undefined) {
                    reject(new Error("assembleApp.listen: express factory unavailable; install `express` as a peer dependency."));
                    return;
                }
