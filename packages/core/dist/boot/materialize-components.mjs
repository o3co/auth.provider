...
function buildDeps(components, requires, optional) {
    ...
    for (const key of requires) {
        if (!(key in components)) {
            throw new Error(`invariant violated: missing required dep "${String(key)}" — stage 1/2 should have caught this`);
        }
 ...
 * Per A2-β §5.3.
 */
export async function materializeComponents(plan, bootstrapComponents, overrideComponents) {
    ...
    for (const activation of plan.providerActivations) {
        ...
        if (!validatedModule) {
            throw new Error(`invariant violated: module "${moduleName}" not found in validated manifests`);
        }
        ...
        if (!factory) {
            throw new Error(`invariant violated: module "${moduleName}" has no provider for "${String(componentKey)}"`);
        }
