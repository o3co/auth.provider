 ...
 */
const REPLICA_UNSAFE_MODULE_REASONS = {
    memorySessionStores: "user sessions, RP registrations and family indexes fork per replica — back-channel logout reaches only the replica that received it, so a logged-out session stays valid on the others",
    "core-rate-limiter-memory": "rate-limit counters fork per replica — every configured limit is effectively multiplied by the replica count, and resets on each deploy",
    "core-access-token-denylist-memory": "access-token revocation forks per replica — a revoked token keeps working on every replica that did not receive the revocation",
    "core-replay-seen-set-memory": "DPoP proof-replay detection forks per replica — a captured proof can be replayed once against each replica",
    "core-refresh-token-family-store-memory": "refresh-token families fork per replica — rotation replay detection and cascade revoke see only this replica's history",
    "core-challenge-store-memory": "WebAuthn challenges fork per replica — a ceremony started on one replica cannot be completed on another",
    "core-webauthn-credential-store-memory": "registered WebAuthn credentials fork per replica — a passkey registered on one replica does not exist on the others",
    "core-federation-token-store-memory": "upstream federation tokens fork per replica — a token stored on one replica is missing on the others",
};
/**
 ...
 */
export const REPLICA_UNSAFE_MODULES = Object.keys(REPLICA_UNSAFE_MODULE_REASONS);
/**
 ...
 */
export function checkReplicaSafety({ modules, config, logger }) {
    ...
    if (mode === "single")
        return;
    logger?.warn({ modules: offenders, reasons }, 
