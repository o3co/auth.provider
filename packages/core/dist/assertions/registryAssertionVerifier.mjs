 ...
 */
const idJagSubjectHandle = (issuer) => (claims) => {
    if (typeof claims.sub !== "string" || claims.sub.length === 0)
        return null;
    const tenant = typeof claims.tenant === "string" && claims.tenant.length > 0 ? claims.tenant : null;
...
const isRefusal = (err) => err instanceof errors.JOSEError && REFUSAL_CODES.has(err.code);
/**
 ...
 */
export function createRegistryAssertionVerifier(options) {
    ...
    if (audiences.length === 0 || audiences.some((a) => a.length === 0)) {
        throw new Error("createRegistryAssertionVerifier: audience is required — an assertion without " +
    ...
    return {
        ...
        async verify(assertion, context = {}) {
            ...
            if (entry === null)
                return null;
            if (entry.expiresAt !== undefined && entry.expiresAt.getTime() <= Date.now())
                return null;
            ...
            if (idJag) {
                ...
                if (issuerIdentifier === undefined) {
                    throw new Error(`createRegistryAssertionVerifier: entry ${entry.issuer} has profile "id-jag" ` +
            ...
            let claims;
            try {
                ({ payload: claims } = await jwtVerify(assertion, keyFor(entry), {
                    issuer: entry.issuer,
                    audience: idJag ? issuerIdentifier : audiences,
            ...
            if (idJag) {
                ...
                if (Array.isArray(claims.aud) && claims.aud.length !== 1)
                    return null;
                if (claims.client_id !== context.clientId)
                    return null;
                ...
                // issue in the store.
                const expiresAtMs = Math.max(claims.exp * 1000, Date.now() + 1_000);
                const fresh = await replaySeenSet?.markSeen(`jwt-bearer:id-jag:${entry.issuer}`, jti, expiresAtMs);
                if (fresh !== true)
                    return null;
            }
            if (entry.allowedSubjects !== undefined &&
                (typeof claims.sub !== "string" || !entry.allowedSubjects.includes(claims.sub))) {
                return null;
            }
            const readers = options.readersFor?.(entry);
            const readHandle = readers?.readSubjectHandle ??
            ...
            const scope = entry.allowedScopes === undefined
                ? claimed
                : claimed === undefined
                    ? entry.allowedScopes
            ...
            if (idJag) {
                ...
                if (resources !== undefined) {
                    const admitted = entry.allowedAudiences === undefined
                        ? resources
