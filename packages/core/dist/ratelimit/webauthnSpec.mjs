 ...
 */
export const resolveWebAuthnAuthenticationOptionsLimitSpec = (limits, config) => {
    const result = { ...limits };
    const given = config?.webauthn?.rateLimit?.authenticationOptions;
    if (given === undefined)
        return result;
    const spec = requireUsableConfiguredRateLimitSpec("webauthn.rateLimit.authenticationOptions", given);
    if (result[WEBAUTHN_AUTHENTICATION_OPTIONS_RATE_LIMIT_PREFIX] !== undefined)
        return result;
