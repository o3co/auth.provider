...
function structuredReason(error) {
    const e = error;
    ...
    if (e.error === "too_many_requests" || e.status === 429)
        return "rate_limited";
    ...
    if (extractNetworkCode(error) !== undefined)
        return "network";
    return undefined;
}
/**
 ...
 */
export function isKnownFederationRefreshErrorCode(code) {
    return typeof code === "string" && KNOWN_ERROR_CODES.has(code);
}
export function classifyFederationRefreshError(error) {
    const extras = {};
    if (error !== null && typeof error === "object") {
        const code = error.error;
        if (typeof code === "string" && KNOWN_ERROR_CODES.has(code))
            extras.upstreamCode = code;
        const retryAfter = retryAfterSeconds(error);
        if (retryAfter !== undefined)
            extras.retryAfterSeconds = retryAfter;
        const reason = structuredReason(error);
        if (reason !== undefined)
            return { reason, structured: true, ...extras };
    }
