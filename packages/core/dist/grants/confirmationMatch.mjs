...
export const CONFIRMATION_MEMBERS = Object.keys(BINDING_PROFILES);
/**
 ...
 * Important #2).
 */
export const matchConfirmation = (cnf, binding) => {
    ...
    const expected = readMember(cnf, member);
    const presented = binding && binding.kind === BINDING_PROFILES[member].kind
        ? readMember(binding.confirmation, member)
        : undefined;
    if (presented === undefined)
        return { status: "no-proof", member, expected };
    ...
    if (presented !== expected)
        return { status: "mismatch", member, expected };
 ...
 */
export const ownedConfirmation = (binding) => {
    if (!binding)
        return undefined;
    for (const member of CONFIRMATION_MEMBERS) {
        if (BINDING_PROFILES[member].kind !== binding.kind)
            continue;
 ...
 */
export const isCompoundConfirmation = (raw) => CONFIRMATION_MEMBERS.filter((member) => readMember(raw, member) !== undefined).length > 1;
