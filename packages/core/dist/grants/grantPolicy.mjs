 ...
 */
export function policyOutOfBounds(errorDescription) {
 ...
 */
export async function evaluateGrantPolicy(grantPolicy, request, context, effectiveScopes, scopeCeiling = { scopes: effectiveScopes, name: "requested scope" }) {
    let decision;
    try {
    ...
    if (exceeded.length > 0) {
        return {
            ok: false,
            result: policyOutOfBounds(`policy returned scopes exceeding ${scopeCeiling.name}: ${exceeded.join(" ")}`),
        };
    }
 ...
 */
export function boundPolicyAudience(decision, ceiling) {
    ...
    if (ceiling === undefined) {
        return {
            ok: false,
            result: policyOutOfBounds("policy returned an audience but no authenticated client supplies an allowedAudiences ceiling"),
        };
    }
    ...
    if (exceeded.length > 0) {
        return {
            ok: false,
            result: policyOutOfBounds(`policy returned audiences outside client allowedAudiences: ${exceeded.join(" ")}`),
        };
    }
