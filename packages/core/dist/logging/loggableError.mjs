 ...
 */
function printWhole(_depth, options, print) {
 ...
 */
const printedWhole = (draft) => Object.defineProperty(draft, inspect.custom, { value: printWhole, enumerable: false });
/**
 ...
 */
export const isLoggableReason = (value) => typeof value === "string" && value.length <= REASON_MAX_LENGTH && REASON_CODE.test(value);
...
const oneLine = (text) => text.replace(LINE_UNSAFE, "?");
/**
...
export function lineSafeText(text, maxLength = LOGGED_STRING_MAX_LENGTH) {
    if (!Number.isInteger(maxLength) || maxLength < LINE_SAFE_MIN_LENGTH) {
        throw new RangeError(`lineSafeText: maxLength must be an integer of at least ${LINE_SAFE_MIN_LENGTH} (got ${maxLength})`);
    }
...
const descriptionOf = (value) => {
    ...
    if (!OAUTH_ERROR_TEXT.test(firstLine))
        return undefined;
 ...
 */
const QUOTES_ITS_INPUT = new Set(["SyntaxError", "YAMLException"]);
/**
 ...
 */
const messageText = (name, message) => typeof message !== "string" || QUOTES_ITS_INPUT.has(name)
    ? undefined
...
const capped = (value) => cutAt(oneLine(value), LOGGED_STRING_MAX_LENGTH);
/**
 ...
 */
export const guardedRead = (target, key) => {
    try {
...
const read = (target, key) => guardedRead(target, key)?.value;
/**
 ...
 */
const headerEnd = (stack, name, code, message) => {
 ...
 */
const framesOf = (stack, name, code, message) => {
    ...
    for (const line of lines.slice(first)) {
        if (!FRAME.test(line) || frames.length === LOGGED_STACK_MAX_FRAMES)
            break;
        frames.push(line);
    }
...
const statusFieldsOf = (err) => {
    ...
    for (const key of keys) {
        if (count === MAX_STATUS_FIELDS)
            break;
        if (!STATUS_FIELD.test(key))
            continue;
...
const membersOf = (err) => {
    ...
    for (let index = 0; index < Math.min(total, LOGGED_AGGREGATE_MAX_ERRORS); index++) {
        const member = read(errors, String(index));
        if (isError(member))
 ...
 */
export function uncappedDetail(err) {
    if (!isError(err))
        return undefined;
 ...
 * throws.
 */
export function loggableError(err) {
    ...
    for (let next = 0; next < pending.length; next++) {
        const { err: node, depth, into } = pending[next];
        if (!isError(node))
            continue;
        ...
        if (members !== null) {
            ...
            if (kept.length > 0)
                into.aggregateErrors = kept;
            if (members.total > kept.length)
                into.aggregateErrorsOmitted = members.total - kept.length;
        }
    }
    return root;
}
