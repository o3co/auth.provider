 ...
 */
import { createApp, } from "../boot/index.mjs";
...
export async function createTestApp(options) {
    const bootstrapComponents = options?.bootstrapComponents ??
        { config: makeValidAppConfig(), pathResolver: (s) => s };
    ...
    const inspect = projectInspect(handle);
...
function projectInspect(handle) {
    ...
    const { grantHandlerResolver, tokenExchangeValidatorResolver, federationProviders } = handle.components;
