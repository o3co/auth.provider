...
function toPublicJwk(jwk) {
    ...
    for (const [member, value] of Object.entries(jwk)) {
        if (PRIVATE_JWK_MEMBERS.includes(member))
            continue;
        pub[member] = value;
    }
    return pub;
 ...
 */
export const createRouter = (express, keyStore, opts = {}) => {
    const path = opts.path ?? DEFAULT_JWKS_PATH;
    if (!isValidJwksPath(path)) {
        throw new Error(`createJwksRouter: path must be an absolute path beginning with "/" ` +
    ...
    if (!Number.isInteger(cacheMaxAgeSeconds) || cacheMaxAgeSeconds < 0) {
        throw new Error(`createJwksRouter: cacheMaxAgeSeconds must be a non-negative integer, got ${cacheMaxAgeSeconds}`);
    }
    ...
    let cached = null;
    const byKid = (a, b) => (a.kid < b.kid ? -1 : a.kid > b.kid ? 1 : 0);
    ...
    // the safe direction.
    const sameKeySet = (current, previous) => {
        ...
        return sortedCurrent.every((key, i) => key.kid === sortedPrevious[i]?.kid && key.publicKey === sortedPrevious[i]?.publicKey);
    };
    ...
    const refuseUnavailable = (res) => {
        res.setHeader("Cache-Control", "no-store");
        return res.status(503).json({
            error: "jwks_unavailable",
            error_description: "No public verification keys are currently available to publish. The signing " +
    ...
    router.get(path, async (req, res) => {
        if (keyStore.algorithm === "HS256") {
            ...
            return res.status(404).json({
                ...
                error_description: "This authorization server signs with HS256, a symmetric algorithm with no " +
        ...
        try {
            ...
            opts.logger?.error({ algorithm: keyStore.algorithm, err: loggableError(err) }, "jwks_unavailable");
            return refuseUnavailable(res);
        }
        if (cached === null || !sameKeySet(managedKeys, cached.keyRefs)) {
            const exported = await Promise.all(managedKeys.map(async (mk) => {
                const publicJwk = toPublicJwk((await exportJWK(mk.publicKey)));
                if (publicJwk === null)
                    return null;
                return { ...publicJwk, kid: mk.kid, use: "sig", alg: keyStore.algorithm };
            }));
            const keys = exported.filter((k) => k !== null);
            if (keys.length === 0) {
                ...
                opts.logger?.error({ algorithm: keyStore.algorithm, keys: managedKeys.length }, "jwks_unavailable");
                return refuseUnavailable(res);
            }
