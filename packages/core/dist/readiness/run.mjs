 ...
 */
function startOrJoin(probe, inFlight) {
    if (!inFlight)
        return probe.check();
    ...
    void started
        ...
        if (inFlight.get(probe.name) === started)
            inFlight.delete(probe.name);
    });
    return started;
}
async function runOne(probe, timeoutMs, inFlight) {
 ...
 */
export async function runReadinessProbes(probes, options) {
    const checks = await Promise.all(probes.map((probe) => runOne(probe, options.timeoutMs, options.inFlight)));
