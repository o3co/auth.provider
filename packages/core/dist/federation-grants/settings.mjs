...
import { assertFederationGrantRetrievalLimits, } from "./retrieve.mjs";
/**
 ...
 */
const MAXIMUM = { seconds: 31_536_000, milliseconds: 31_536_000_000 };
...
function setting(settings, key) {
    ...
    if (written === undefined)
        return FEDERATION_GRANT_SETTING_DEFAULTS[key];
    const unit = key.endsWith("Ms") ? "milliseconds" : "seconds";
    const refuse = () => {
        throw new RangeError(`federationGrants.${key} must be a whole number of ${unit} no greater than ` +
    ...
    // as a duration.
    const value = typeof written === "number"
        ? written
        : typeof written === "string" && DECIMAL.test(written.trim())
            ? Number(written.trim())
            : Number.NaN;
    if (!Number.isInteger(value) || value < 0 || value > MAXIMUM[unit])
        refuse();
    return value;
}
const settingsOf = (config) => (config?.federationGrants ?? {});
/** `maxExpiresIn`, in milliseconds, within the ceiling the code enforces above any setting. */
function resolveMaxExpiresInMs(settings) {
    ...
    if (maxExpiresInMs <= 0 || maxExpiresInMs > FEDERATION_GRANT_LIFETIME_CEILING_MS) {
        throw new RangeError("federationGrants.maxExpiresIn must be a positive number of seconds no greater than " +
 ...
 */
export function resolveFederationGrantAcquisitionLimits(config) {
    ...
    if (defaultLifetimeMs <= 0) {
        throw new RangeError("federationGrants.defaultExpiresIn must be a positive number of seconds: a grant needs a lifetime");
    }
    if (defaultLifetimeMs > maxLifetimeMs) {
        throw new RangeError(`federationGrants.defaultExpiresIn (${defaultLifetimeMs / 1000}) must not exceed ` +
 ...
 */
export function resolveFederationGrantRetrievalLimits(config) {
    const settings = settingsOf(config);
 ...
 * named.
 */
export function resolveFederationGrantKeepPolicy(config) {
    ...
    throw new RangeError('federationGrants.allowKeepOnSubjectRevocation must be one of "true", "false", "1" or "0" ' +
