...
export const memoryFederationGrantIntentStoreModule = defineModule({
    ...
    replicaSafety: {
        unsafe: true,
        reason: "federation grant acquisition forks per replica — an intent lodged on one replica is unknown to every other, so the consent page and the upstream callback answer as if the flow had expired whenever they land elsewhere, and the bound on live intents is counted per replica instead of per (client, subject)",
    },
    provides: {
...
export const memoryFederationGrantStoreModule = defineModule({
    ...
    replicaSafety: {
        unsafe: true,
        reason: "federation grants fork per replica — a grant lodged or authorized on one replica is unknown to every other, one revoked there still yields upstream tokens here, and a refresh token rotated on one replica leaves every other presenting the old one, which a reuse-detecting IdP answers by revoking the family",
    },
    ...
    provides: {
        federationGrantStore: (deps) => {
            const seconds = deps.config
                .federationGrants?.tombstoneRetention;
            return createMemoryFederationGrantStore(seconds === undefined ? {} : { tombstoneRetentionMs: seconds * 1000 });
        },
    },
});
