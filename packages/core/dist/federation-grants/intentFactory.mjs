 ...
 */
export function createFederationGrantIntentStoreFactory() {
    return createAdapterFactory("FederationGrantIntentStore");
}
/**
 ...
 */
export function registerBuiltinFederationGrantIntentStores(factory, logger = consoleLogger) {
    factory.register("memory", () => {
        logger.warn("federationGrantIntentStore: in-memory adapter is for dev/test only — a flow started on one instance cannot be finished on another (every callback answers as unknown), and a restart loses every connect in progress.");
