 ...
 */
import { classifyFederationRefreshError, isKnownFederationRefreshErrorCode, } from "../federation-tokens/refresh-error.mjs";
...
import { federationGrantIneligibilityRetry, federationGrantRefreshFailureStands, isFederationGrantInteractionCode, isUsableMaxUpstreamAccessTokenLifetime, judgeUpstreamAccessToken, scopesWithin, } from "./eligibility.mjs";
 ...
 */
export function assertFederationGrantRetrievalLimits(limits) {
    const timed = [
        "upstreamTimeoutMs",
    ...
    if (!(limits.refreshFailureBackoffMs <= limits.ineligibleRetryAfterMs)) {
        throw new RangeError("federationGrants: refreshFailureBackoffMs must not exceed ineligibleRetryAfterMs — the marker's interval is the ceiling on how long a failing upstream is not asked");
    }
    if (!(limits.upstreamTimeoutMs <= limits.upstreamHardTimeoutMs)) {
        throw new RangeError("federationGrants: upstreamTimeoutMs must not exceed upstreamHardTimeoutMs — the soft deadline only answers the caller, and the hard one is where the request is aborted");
    }
    if (!(limits.upstreamHardTimeoutMs +
        limits.persistRetryBudgetMs +
        FEDERATION_GRANT_REFRESH_LOCK_MARGIN_MS <=
        limits.refreshLockTtlMs)) {
        throw new RangeError("federationGrants: upstreamHardTimeoutMs + persistRetryBudgetMs must leave a second of refreshLockTtlMs — the refresh lock has no renewal, and one that expires mid-refresh lets two replicas present the same refresh token");
    }
}
/**
...
function report(deps, request, during, error) {
    try {
        deps.report?.({
            during,
            error,
 ...
 */
function handOver(deps, request, work) {
    try {
...
const dateAllowanceMs = (limits) => Math.max(limits.refreshBufferMs, limits.revocationSkewMs);
/** RFC 6749 §4.1.2.1's names for an outage: not refusals, whatever status they came with. */
const UPSTREAM_OUTAGE_CODES = new Set([
    "server_error",
...
async function evaluate(deps, request, look = {}) {
    ...
    if (status.status === "connection_not_configured" ||
    ...
    if (status.status === "reauthorization_required") {
        ...
        if (status.reason === "credential_unreadable" &&
    ...
    if (typeof minTtlSeconds !== "number" ||
        ...
        if (eligible && believed && remainingMs > 0) {
            ...
            if (carries && (!refreshIt || notAsked !== undefined || look.attempted === true)) {
                ...
                return {
                    ...
                    expiresIn: Math.floor(Math.max(0, Math.min(tokenEndsAt, grantEndsAt.getTime()) - now.getTime()) / 1000),
                };
            }
    ...
    if (notAsked !== undefined)
        return { kind: "deny", denial: notAsked, grant };
    return {
        kind: "refresh",
        grant,
        connection,
...
async function audit(deps, request, type, outcome, grant) {
    ...
    if (told === "elapsed")
        report(deps, request, "audit", NOT_ANSWERED);
    else if (!told.ok)
        report(deps, request, "audit", told.error);
}
const outcomeOf = (denial) => "reason" in denial && denial.reason !== undefined
 ...
 */
function conclude(deps, request, evaluation, refreshed, otherwise) {
    if (evaluation.kind === "token") {
        // Best effort: a use that could not be recorded is still a use.
        handOver(deps, request, within(settle(() => deps.store.touch(evaluation.grant.id, deps.now())), SIDE_EFFECT_WAIT_MS).then((touched) => {
            if (touched === "elapsed")
                report(deps, request, "touch", NOT_ANSWERED);
            else if (!touched.ok)
                report(deps, request, "touch", touched.error);
        }));
        handOver(deps, request, audit(deps, request, "federation.grant.token.success", "success", evaluation.grant));
        return {
            ok: true,
    ...
    handOver(deps, request, audit(deps, request, "federation.grant.token.denied", outcomeOf(denial), evaluation.grant));
 ...
 * credential (D5).
 */
function readResponse(response, grant, storedRefreshToken) {
 ...
 * one.
 */
async function isStored(deps, grant, credentials, ineligible, budgetMs) {
    const read = await within(settle(() => deps.store.open(grant.id, deps.now())), budgetMs);
    if (read === "elapsed" || !read.ok || read.value === null)
        return false;
...
async function refreshUnderLock(deps, request, held, refresher, hardDeadline) {
    const { grant, connection } = held;
    const { limits } = deps;
    const failed = (during, error) => report(deps, request, during, error);
    ...
    const call = settle(() => refresher.refreshDelegatedToken({
    ...
    if (!settled.ok) {
        ...
        if (classified.reason === "invalid_grant" && classified.structured) {
            const marked = await within(settle(() => deps.store.requireReauthorization({
                grantId: grant.id,
        ...
        if (isFederationGrantInteractionCode(classified.upstreamCode)) {
            const reason = `upstream_${classified.upstreamCode}`;
            const noted = await stamp(deps, request, grant, { at: deps.now(), kind: "rejected", upstreamCode: classified.upstreamCode }, limits.persistRetryBudgetMs);
            switch (noted) {
                case "written":
    ...
    if (response.token === undefined) {
        ...
        const lifetime = expiresIn !== null && Number.isFinite(expiresIn) && !Number.isNaN(expiresAtMs)
            ? expiresIn
            : null;
    ...
    for (let attempt = 0; attempt < attempts; attempt++) {
        ...
        if (remaining <= 0)
            break;
        const result = await within(settle(() => deps.store.replaceCredentials({
            grantId: grant.id,
            expectedVersion: grant.version,
            credentials,
            ineligible,
    ...
    const left = persistDeadline - deps.now().getTime();
    if (left > 0)
        await stamp(deps, request, grant, { at: deps.now(), kind: "unavailable" }, left);
    return {
        kind: "denied",
        denial: unavailable("storage"),
 ...
 */
async function stamp(deps, request, grant, failure, budgetMs) {
    const noted = await within(settle(() => deps.store.noteRefreshFailure({
        grantId: grant.id,
        expectedVersion: grant.version,
        failure,
...
async function letGo(deps, request, lock) {
    const released = await within(settle(() => lock.release()), SIDE_EFFECT_WAIT_MS);
    // The lock has a TTL: one that could not be let go of runs out.
    if (released === "elapsed")
        report(deps, request, "release", NOT_ANSWERED);
    else if (!released.ok)
        report(deps, request, "release", released.error);
}
/**
 ...
 * else is.
 */
async function lastLook(deps, request, fallback, wrote = {}) {
    const { fetched } = wrote;
    ...
    return conclude(deps, request, stored, 
    ...
    fetched !== undefined && stored.kind === "token" && stored.token.value === fetched, replaced ? unavailable("concurrent_update") : fallback);
}
async function refresh(deps, request) {
    const askedAt = deps.now().getTime();
    const asking = settle(() => deps.store.acquireRefreshLock(request.grantId, {
    ...
    if (asked === "elapsed" || !asked.ok) {
        if (asked === "elapsed") {
            ...
            void asking.then((late) => {
                if (!late.ok || !late.value.acquired)
                    return;
    ...
    const release = () => letGo(deps, request, lock);
    let held;
    let refresher;
    let leaseStartedAt;
    let startedAt;
    try {
        ...
        if (typeof waitedMs !== "number" ||
            ...
            report(deps, request, "lock", new Error("the store did not say how long it waited for the lock"));
            return lastLook(deps, request, unavailable("storage"));
        }
        ...
        if (found === undefined) {
            handOver(deps, request, release());
            return conclude(deps, request, { kind: "deny", denial: NOT_PERMITTED, grant: again.grant }, false, NOT_PERMITTED);
        }
        ...
        if (!(startedAt < leaseStartedAt + deps.limits.upstreamTimeoutMs)) {
            ...
            handOver(deps, request, release());
            report(deps, request, "refresh", new Error("the look under the refresh lock used up the lease; the upstream was not asked"));
            return lastLook(deps, request, unavailable("storage"));
        }
        held = again;
        refresher = found;
    ...
    const work = refreshUnderLock(deps, request, held, refresher, leaseStartedAt + deps.limits.upstreamHardTimeoutMs).catch((error) => {
    ...
    handOver(deps, request, work.then(async (outcome) => {
        if (outcome.keepLock !== true)
            await release();
    ...
    if (first.kind === "denied")
        return lastLook(deps, request, first.denial);
    ...
    return lastLook(deps, request, unavailable(first.kind === "written" ? "upstream" : "concurrent_update"), first.kind === "written" ? first : {});
}
/**
 ...
 */
export async function retrieveFederationGrantToken(deps, request) {
