 ...
 */
export function scopesWithin(scopes, within) {
 ...
 */
export function isUsableMaxUpstreamAccessTokenLifetime(maxAccessTokenLifetime) {
...
export function judgeUpstreamAccessToken(token) {
    ...
    if (!isUsableMaxUpstreamAccessTokenLifetime(token.maxAccessTokenLifetime) ||
 ...
 */
export function federationGrantIneligibilityStands(marker, 
/** Seconds; the connection's current value. */
maxAccessTokenLifetime) {
    if (marker === undefined)
        return false;
    if (!isUsableMaxUpstreamAccessTokenLifetime(maxAccessTokenLifetime))
        return true;
 ...
 */
export function federationGrantIneligibilityRetry(marker, context) {
    if (marker === undefined)
        return { due: true };
    const at = marker.at.getTime();
    const now = context.now.getTime();
    if (!(at <= now + context.allowanceMs))
        return { due: true };
...
const INTERACTION_CODES = new Set(FEDERATION_GRANT_INTERACTION_CODES);
/**
 ...
 */
export function federationGrantInteractionCode(failure) {
    if (failure === undefined || failure.kind !== "rejected")
        return undefined;
...
export function isFederationGrantInteractionCode(code) {
 ...
 */
export function federationGrantRefreshFailureStands(failure, context) {
 ...
 */
export function resolveFederationGrantIntentScopes(requested, connection) {
    const full = [...new Set(connection.scopes)];
    const asked = requested === undefined ? new Set(full) : new Set(requested);
    if (!scopesWithin([...asked], full))
        return { ok: false, reason: "outside_connection" };
    ...
    if (!scopesWithin(required, scopes))
