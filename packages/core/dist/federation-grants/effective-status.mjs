 ...
 */
import { federationGrantIneligibilityStands, federationGrantInteractionCode, isUsableMaxUpstreamAccessTokenLifetime, } from "./eligibility.mjs";
 ...
 */
export function coveredByRevocationBoundary(instant, boundary, skewMs) {
 ...
 */
export function effectiveFederationGrantStatus(grant, context) {
    if (grant.status === "revoked")
        return { status: "revoked", reason: grant.revocation.by };
    if (grant.status === "pending")
        return { status: "pending" };
    if (coveredByRevocationBoundary(grant.consent.at, context.grantsBoundary, context.revocationSkewMs)) {
        return { status: "revoked", reason: "backstop" };
    }
    const expiry = federationGrantExpiryState(grant, context.now, context.maxExpiresInMs);
    if (expiry !== "live")
        return { status: "expired", reason: expiry };
    const connection = context.connection;
    if (connection === undefined)
        return { status: "connection_not_configured" };
