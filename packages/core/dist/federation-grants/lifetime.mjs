...
export function resolveFederationGrantLifetimeMs(limits) {
    ...
    if (limits.requestedMs !== undefined)
        assertPositiveFinite("requestedMs", limits.requestedMs);
    return Math.min(limits.requestedMs ?? limits.defaultMs, limits.maxMs, FEDERATION_GRANT_LIFETIME_CEILING_MS);
}
/**
 ...
 */
export function federationGrantExpiresAt(consentAt, lifetimeMs) {
 ...
 */
export function withinFederationGrantLifetimeCeiling(consentAt, expiresAt) {
 ...
 */
export function federationGrantEffectiveExpiry(grant, maxMs) {
 ...
 */
export function federationGrantExpiryState(grant, now, maxMs) {
    if (!(now.getTime() < grant.expiresAt.getTime()))
        return "consented_lifetime";
    if (!(now.getTime() < grant.consent.at.getTime() + maxMs))
