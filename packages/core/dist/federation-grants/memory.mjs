 ...
 */
function instant(date, name) {
    const ms = date.getTime();
    if (Number.isNaN(ms))
        throw new RangeError(`FederationGrantStore: ${name} is not a valid date`);
    return ms;
}
const isDate = (date) => !Number.isNaN(date.getTime());
function copyAuthorization(from) {
...
const credentialDatesAreDates = (credentials) => credentials.accessToken === undefined || isDate(credentials.accessToken.obtainedAt);
/**
 ...
 */
export function createMemoryFederationGrantStore(options = {}) {
    const retentionMs = options.tombstoneRetentionMs ?? DEFAULT_FEDERATION_GRANT_TOMBSTONE_RETENTION_MS;
    if (!Number.isFinite(retentionMs) || retentionMs < 0) {
        throw new RangeError("createMemoryFederationGrantStore: tombstoneRetentionMs must be a non-negative finite number");
    }
    ...
    const visible = (grantId, nowMs) => {
        const entry = entries.get(grantId);
        if (entry === undefined || !reclaim(grantId, entry, Date.now()))
            return undefined;
     ...
     */
    const mayActivate = (entry, handle, nowMs) => {
        const grant = entry.grant;
        ...
        if (grant.status !== "pending" && !(nowMs < grant.expiresAt.getTime()))
            return false;
     ...
     */
    const credentialsAt = (entry, nowMs) => {
        const grant = entry.grant;
        if (grant.status !== "active" || !(nowMs < grant.expiresAt.getTime()))
            return null;
        return entry.credentials;
    };
    const written = (entry, grant) => {
        entry.grant = grant;
        return { ok: true, grant: structuredClone(grant) };
    };
    const tryLock = (grantId, ttlMs) => {
        const held = locks.get(grantId);
        const startedAt = Date.now();
        if (held !== undefined && held.expiresAt > startedAt)
            return null;
    ...
    return {
        ...
        async createPending(input) {
            ...
            if (!(nowMs < input.intent.expiresAt.getTime()))
                return failed();
            ...
            if (resident !== undefined && reclaim(input.id, resident, Date.now()))
                return failed();
        ...
        async nameIntent(input) {
            ...
            if (entry === undefined)
                return failed();
            const grant = entry.grant;
            if (grant.status !== "active" && grant.status !== "reauthorization_required")
                return failed();
            if (!(nowMs < grant.expiresAt.getTime()))
                return failed();
            entry.intent = {
        ...
        async retireIntent(input) {
            const nowMs = instant(input.now, "now");
            const entry = visible(input.grantId, nowMs);
            if (entry === undefined || entry.intent === null)
                return failed();
            const grant = entry.grant;
        ...
        async inspect(grantId, now) {
            ...
            if (entry === undefined)
                return null;
            const credentials = credentialsAt(entry, nowMs) === null ? "absent" : "ok";
        ...
        async activate(input) {
            ...
            if (entry === undefined)
                return failed();
            if (!mayActivate(entry, input.intentHandle, nowMs))
                return failed();
            ...
            if (!(authorization.authorizedAt.getTime() <= nowMs))
                return failed();
            if (!credentialDatesAreDates(input.credentials))
                return failed();
        ...
        async noteRefreshFailure(input) {
            ...
            if (previous !== undefined && atMs < previous.at.getTime())
                return failed();
            ...
            if (federationGrantInteractionCode(previous) !== undefined)
                return failed();
        ...
        async acquireRefreshLock(grantId, { ttlMs, waitForMs }) {
            ...
            while (taken === null) {
                ...
                await new Promise((resolve) => setTimeout(resolve, Math.min(LOCK_POLL_INTERVAL_MS, remaining)));
            ...
            return {
                ...
                release: async () => {
                    ...
                    if (locks.get(grantId)?.token === held)
                        locks.delete(grantId);
                },
            };
        },
    };
