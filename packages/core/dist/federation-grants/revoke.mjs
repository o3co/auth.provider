 ...
 * the one ID.
 */
export function federationGrantCorrelationId(given) {
 ...
 */
export async function revokeFederationGrant(deps, grantId, by) {
    const written = await deps.store.revoke(grantId, by, deps.now());
    if (written.ok)
        await tell(deps, written.grant, by);
    return written;
}
/**
