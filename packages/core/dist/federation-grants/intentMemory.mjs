 ...
 */
import { FEDERATION_GRANT_FIRST_INTENTS_PER_CLIENT_SUBJECT_LIMIT, federationGrantConsentExpiry, } from "./intentStore.mjs";
/**
...
const pairKey = (clientId, subject) => 
...
function sameIntent(a, b) {
    const asKey = (record) => JSON.stringify([
        record.handle,
        record.kind,
        record.grantId,
        record.clientId,
        ...
        Object.entries(record.authorizationParams).sort(([left], [right]) => left < right ? -1 : left > right ? 1 : 0),
        record.redirectUri,
        record.clientState,
...
const bindingsMatch = (a, b) => a.sessionId === b.sessionId && a.sid === b.sid && a.subject === b.subject;
export function createMemoryFederationGrantIntentStore() {
    ...
    const sweep = () => {
        ...
        for (const [handle, entry] of entries) {
            if (entry.record.expiresAt.getTime() <= now)
    ...
    const countReservations = (clientId, subject) => {
        ...
        for (const [handle, entry] of entries) {
            ...
            if (pairKey(entry.record.clientId, entry.record.subject) === key)
                held += 1;
        }
        return held;
    };
    /** Ends an intent, whichever way it ended: the marker stays, everything else goes. */
    const close = (entry, options) => {
        entry.live = false;
        if (entry.challenge !== null) {
            consents.delete(entry.challenge);
    ...
    return {
        ...
        async getConsent(challenge, now) {
            ...
            if (record === undefined)
                return null;
            if (resident(record.intentHandle) === undefined)
                return null;
        ...
        async answerConsent({ challenge, binding, answer, now, }) {
            ...
            if (!(nowMs < record.expiresAt.getTime()))
                return { outcome: "empty" };
            if (!bindingsMatch(record.binding, binding))
                return { outcome: "empty" };
        ...
        async consumeTransaction({ state, connection, now }) {
            ...
            if (transaction === undefined)
                return null;
            if (resident(transaction.intent.handle) === undefined)
                return null;
            // Hidden, not reclaimed: a caller's clock does not delete a record.
            if (!(nowMs < transaction.expiresAt.getTime()))
                return null;
            // A callback on another connection's path spends nothing.
            if (transaction.intent.connection !== connection)
                return null;
