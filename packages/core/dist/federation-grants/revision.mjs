 ...
 * pin a known answer.
 */
function fingerprint(tag, fields) {
 ...
 */
export function federationGrantIdentityRevision(connection) {
 ...
 */
export function federationGrantAuthorizationRevision(connection) {
    const scopes = [...new Set(connection.scopes)].sort();
    const params = Object.entries(connection.authorizationParams ?? {}).sort(([a], [b]) => a < b ? -1 : a > b ? 1 : 0);
