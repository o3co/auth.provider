 ...
 */
export function usableMaxEntries(maxEntries, owner) {
    if (!Number.isInteger(maxEntries) || maxEntries <= 0) {
        throw new RangeError(`${owner}: maxEntries must be a positive whole number (got ${String(maxEntries)})`);
    }
    if (maxEntries > MAX_MEMORY_STORE_ENTRIES) {
        throw new RangeError(`${owner}: maxEntries must be at most ${MAX_MEMORY_STORE_ENTRIES}, the most entries a Map holds (got ${String(maxEntries)})`);
    }
    return maxEntries;
}
/** The store's options for the value at `key`: `{}` when absent, `{ maxEntries }` when usable. */
export function configuredMaxEntries(value, key) {
    ...
    if (count > MAX_MEMORY_STORE_ENTRIES) {
        throw new RangeError(`${key} must be at most ${MAX_MEMORY_STORE_ENTRIES}, the most entries a Map holds (got ${JSON.stringify(value)})`);
    }
