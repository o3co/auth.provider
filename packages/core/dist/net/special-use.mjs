...
export function isSpecialUseAddress(address) {
    const family = isIP(address);
    if (family === 4)
        return SPECIAL_USE.check(address, "ipv4");
    if (family === 6) {
        // The IPv4 half is what the packet reaches, whichever way the address
        // was written.
        const embedded = embeddedIpv4(address);
        if (embedded !== null)
            return SPECIAL_USE.check(embedded, "ipv4");
