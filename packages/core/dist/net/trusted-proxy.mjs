...
export const TRUSTED_PROXY_NAMED_RANGES = Object.keys(NAMED_RANGES);
/**
 ...
 */
export function checkTrustedProxyEntry(value) {
    if (typeof value !== "string")
        return "not-a-string";
    const entry = value.trim();
    if (entry === "")
        return "empty";
    if (Object.hasOwn(NAMED_RANGES, entry.toLowerCase()))
        return null;
    ...
    if (family === 0)
        return "not-an-address-or-keyword";
    ...
    if (isIP(rest) !== 0)
        return "netmask-notation";
    if (!DECIMAL_PREFIX.test(rest))
        return "bad-prefix-length";
...
export function isTrustedProxyEntry(value) {
    return checkTrustedProxyEntry(value) === null;
}
/** Operator-facing explanation for each rejection reason. */
export function describeTrustedProxyEntryRejection(reason) {
    ...
    switch (reason) {
        ...
        case "not-an-address-or-keyword":
            return (`is not an IP address, a CIDR range, or one of the named ranges (${named}). ` +
        ...
        case "bad-prefix-length":
            return ("has a malformed CIDR prefix length. Write a plain decimal number, " +
                "0-32 for IPv4 and 0-128 for IPv6 (e.g. 10.0.0.0/8, fc00::/7).");
        case "netmask-notation":
            return ("uses dotted-netmask notation, which is not supported. Write the " +
 ...
 */
export const createTrustedProxyMatcher = (entries, options = {}) => {
    ...
    entries.forEach((rawEntry, index) => {
        ...
        if (rejection !== null) {
            throw new Error(`createTrustedProxyMatcher: ${label}[${index}] = ${JSON.stringify(rawEntry)} ` +
        ...
        blockList.addSubnet(address, Number(entry.slice(slashIdx + 1)), isIP(address) === 6 ? "ipv6" : "ipv4");
        ruleCount += 1;
    });
