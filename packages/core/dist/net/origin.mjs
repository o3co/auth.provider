 ...
 */
export function checkSerializedOrigin(raw) {
    ...
    if (url.origin === "null")
        return { reason: "opaque-origin", scheme };
    if (raw !== url.origin)
        return { reason: "not-serialized", serialized: url.origin };
    if (scheme === "https")
        return null;
    if (scheme === "http" && isLoopbackHostname(url.hostname))
        return null;
    return { reason: "insecure-scheme", scheme, hostname: url.hostname };
}
/** Operator-facing wording for one {@link SerializedOriginRejection}. */
export function describeSerializedOriginRejection(rejection) {
    switch (rejection.reason) {
        case "unparsable":
            return 'must be an absolute origin, e.g. "https://app.example.com"';
        case "wildcard":
            return ("must not contain a wildcard — matching against the Origin header is exact string " +
                "equality, so a wildcard entry matches nothing; list each origin in full");
        case "opaque-origin":
            return (`scheme ${JSON.stringify(rejection.scheme)} has no tuple origin — a browser sends the ` +
        ...
        case "insecure-scheme":
            return (`http:// is accepted for loopback hosts only (localhost, 127.0.0.0/8, [::1]); got ` +
                `scheme ${JSON.stringify(rejection.scheme)} on host ${JSON.stringify(rejection.hostname)}`);
        case "not-serialized":
            return ("must be a bare origin — scheme, host, and a port only when it is not the scheme's " +
