...
export function createMemoryConsentStore() {
    ...
    const key = (sub, clientId) => JSON.stringify([sub, clientId]);
    return {
        kind: "memory",
        get size() {
            return records.size;
        ...
        async grant(record) {
            ...
            const live = current !== undefined && (current.expiresAt === undefined || current.expiresAt > Date.now())
                ? current.scopes
                : [];
            records.set(id, {
                ...record,
 ...
 */
export function createMemoryPendingConsentStore() {
    const records = new Map();
    /**
    ...
    return {
        ...
        async set(record) {
            ...
            const mine = bySession.get(record.sessionId) ?? new Set();
