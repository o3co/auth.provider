...
export const memoryConsentStoreModule = defineModule({
    ...
    replicaSafety: {
        unsafe: true,
        reason: "consent records fork per replica — a consent granted on one replica is asked for again on every other, one revoked there stays granted here, and a consent challenge parked on one replica is unknown to every other",
    },
    provides: {
