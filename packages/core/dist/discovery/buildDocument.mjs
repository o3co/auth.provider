 ...
 */
function looksIssuerRelative(field, value) {
    if (field.endsWith("_endpoint") || ISSUER_RELATIVE_FIELD_NAMES.has(field))
        return true;
 ...
 */
export function buildDiscoveryDocument(items, opts) {
    ...
    // malformed document.
    if (issuer === "") {
        throw new DiscoveryDocumentError(`discovery issuer must not be empty after trailing-slash normalization (got ${JSON.stringify(opts.issuer)})`);
    }
    ...
    if (opts.signingAlgs.length === 0) {
        throw new DiscoveryDocumentError("discovery document requires at least one id_token signing algorithm " +
    ...
    for (const item of items) {
        for (const [field, path] of Object.entries(item.endpoints ?? {})) {
            if (RESERVED_FIELDS.has(field)) {
                throw new DiscoveryDocumentError(`discoveryMetadata may not contribute the reserved field "${field}" (owned by the aggregator)`);
            }
            if (typeof path !== "string" || !path.startsWith("/")) {
                throw new DiscoveryDocumentError(`discoveryMetadata endpoint "${field}" must be an absolute path beginning with "/", got ${JSON.stringify(path)}`);
            }
            const url = `${issuer}${path}`;
            if (field in doc && doc[field] !== url) {
                throw new DiscoveryDocumentError(`discoveryMetadata field "${field}" was contributed with conflicting values (${JSON.stringify(doc[field])} vs ${JSON.stringify(url)})`);
            }
            doc[field] = url;
        }
        for (const [field, value] of Object.entries(item.metadata ?? {})) {
            ...
            if (looksIssuerRelative(field, value)) {
                throw new DiscoveryDocumentError(`discoveryMetadata field "${field}" looks issuer-relative and must be contributed via ` +
            ...
            if (Array.isArray(value)) {
                ...
                if (existing !== undefined && !Array.isArray(existing)) {
                    throw new DiscoveryDocumentError(`discoveryMetadata field "${field}" was contributed as both an array and a scalar`);
                }
                doc[field] = dedupe([...(existing ?? []), ...value]);
            }
            else {
                if (field in doc && doc[field] !== value) {
                    throw new DiscoveryDocumentError(`discoveryMetadata field "${field}" was contributed with conflicting values (${JSON.stringify(doc[field])} vs ${JSON.stringify(value)})`);
                }
                doc[field] = value;
            }
        }
    ...
    if (missing.length > 0) {
        throw new DiscoveryDocumentError(`discovery document is missing OIDC-required field(s): ${missing.join(", ")}. ` +
    ...
    const invalidArrays = REQUIRED_NONEMPTY_ARRAY_FIELDS.filter((f) => !Array.isArray(doc[f]) || doc[f].length === 0);
    if (invalidArrays.length > 0) {
        throw new DiscoveryDocumentError(`discovery document field(s) must be a non-empty array: ${invalidArrays.join(", ")}. ` +
