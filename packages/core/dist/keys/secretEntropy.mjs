...
function base64ByteLength(value) {
    ...
    if (!/^[A-Za-z0-9+/]+$/.test(body) && !/^[A-Za-z0-9_-]+$/.test(body))
        return undefined;
    ...
    if (padding > 0 && remainder !== 4 - padding)
        return undefined;
 ...
 */
export function measureSecretEntropyBytes(secret) {
    const candidates = [
 ...
 */
export function describeWeakSecret(actualBytes, requirement) {
    const minBytes = requirement.minBytes ?? MIN_SECRET_ENTROPY_BYTES;
    return (`${requirement.configKey} must carry at least ${minBytes} bytes ` +
 ...
 */
export function assertSecretEntropy(secret, requirement) {
    const minBytes = requirement.minBytes ?? MIN_SECRET_ENTROPY_BYTES;
