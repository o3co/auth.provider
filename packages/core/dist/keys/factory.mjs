...
export function createKeyStoreFactory() {
    return createAdapterFactory("KeyStore");
}
/**
 ...
 */
function readKeyValue(pemString, filePath) {
 ...
 */
function narrowPreviousKeysArray(value) {
...
export function registerBuiltinKeyStores(factory) {
    factory.register("local", async (config) => {
        const rawAlgorithm = config.algorithm;
        const algorithm = typeof rawAlgorithm === "string" && rawAlgorithm.length > 0 ? rawAlgorithm : "HS256";
        if (algorithm === "HS256") {
