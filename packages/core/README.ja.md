...
- **`error_description`** は意図して残す唯一の相手側の文字列 — 失効したグラントと壊れたクライアントを見分けるため: 最初の行だけ（Azure AD の AADSTS の行で、Trace ID の行は含まない）を、その行が RFC 6749 §5.2 の文字集合に収まるとき、トークンになりうる文字（`[A-Za-z0-9._~+/=-]`）が 20 文字以上続く最初の箇所の手前で切り、前後の空白を除いて残す — 古い Spring の "Invalid refresh token: <トークン>" は "Invalid refresh token:" を、Azure AD の AADSTS700016 はアプリケーション ID の手前までを残す — 何も残らなければ省く。
