...
export function createInMemorySessionFederationIndex(): SessionFederationIndex {
	...
	return {
		kind: "memory",
		async addFederation(sid: string, federationName: string, expiresAt: Date): Promise<void> {
			set.add(sid, federationName, expiresAt);
		},
		async listFederations(sid: string): Promise<ReadonlyArray<string>> {
			return set.list(sid);
		},
		async removeFederation(sid: string, federationName: string): Promise<void> {
