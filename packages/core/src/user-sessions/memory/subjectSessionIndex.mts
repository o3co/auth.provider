...
export function createInMemorySubjectSessionIndex(): SubjectSessionIndex {
	...
	const prune = (subject: string): Map<string, number> | undefined => {
		const sids = bySubject.get(subject);
		if (sids === undefined) return undefined;
		const now = Date.now();
		for (const [sid, expiresAtMs] of sids) {
			if (expiresAtMs <= now) sids.delete(sid);
		}
		if (sids.size === 0) {
	...
	return {
		...
		async addSid(subject, sid, expiresAt) {
			...
			const sids = bySubject.get(subject) ?? new Map<string, number>();
		...
		async removeSid(subject, sid) {
			...
			sids.delete(sid);
			if (sids.size === 0) bySubject.delete(subject);
		},
