...
- `adapters/`, `logging/`, `errors/`, `net/` and `security/` are leaves: they import nothing else in core, except that `adapters/` takes types from `readiness/` and types and `loggableError` from `logging/`.
...
| `logging/` | `Logger` (pino-compatible), `EventLogger`, `consoleLogger`, and `loggableError` — what a log line may carry of a library's or a store's error (a message a parser or a peer wrote is not trusted). | — | A leaf. |
