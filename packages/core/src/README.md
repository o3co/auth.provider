...
| `logging/` | `Logger` (pino-compatible), `EventLogger`, `consoleLogger`, and `loggableError` — what a log line may carry of a library's error. | — | A leaf. |
...
`testing/` is the `@o3co/auth-provider-core/testing` subpath: `createTestApp`, `TestInspect`, the config fixtures, the `GrantRegistry` re-export for tests that build a registry by hand, and `createFakeIdp`, the fake OpenID Provider every federation adapter's tests run the real `openid-client` against — with hand-named endpoints for Google and Apple, or discovered for the generic OIDC adapter. It is separate so that test helpers ship on their own subpath and product code never loads them. `__tests__/` at this level holds the package-wide tests: the barrel, the drift guards, and the compile-time contracts that span directories.
