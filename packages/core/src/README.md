...
| `modules/` | `PathResolver` and `FederationProviderHandle`, the structural narrowing of the `federationProviders` synthetic key; re-exports `manifest/`. | Import the concrete `FederationProvider` from `packages/session` (a package cycle). |
