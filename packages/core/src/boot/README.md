...
- A `grantPolicy` from any source is refused unless `oauth.jwt.issuer` is a non-empty string — [`composition-root-invariants.test.mts`](./__tests__/composition-root-invariants.test.mts); the federation stores are wired whole or not at all — [`federation-stores-incomplete.test.mts`](./__tests__/federation-stores-incomplete.test.mts).
