...
- Imports from core: `modules/manifest` (types, `SYNTHETIC_COMPONENT_KEYS`), `config/application.schema` (`composeConfigSchema`), `grants/registry` (the `GrantRegistry` behind the `grants` collector), `middleware/{cors,tokenBinding,protectedResourceBinding}`, `discovery/planRoute`, `net/origin`, `adapters/AdapterFactory`, `readiness/*`, `logging/*`; `replica-safety.mts` imports every bundled `memory…Module` to read its declaration. `express` is an optional peer, resolved lazily with `createRequire`.
- Imported by: `../app.mts`, `../index.mts`, `../discovery/planRoute.mts`, `../testing/`. No other directory in core depends on boot; packages reach it only through `createApp`.
...
- Stage-1 rows run in registry order, the first violation wins, each row has a unique id and a spec pointer, and the registry is frozen rows included — [`check-registry.test.mts`](./__tests__/check-registry.test.mts). The pairing check is the row `federation-redirect-policy-pairing` ([`validate-manifests.mts`](./validate-manifests.mts) line 1603) calling `checkFederationRedirectPolicyPairing` (line 775) — [`federation-pairing.test.mts`](./__tests__/federation-pairing.test.mts).
