...
- A `LifecycleRegistrar` cleanup that throws is logged once, at error, object-first as `adapter_lifecycle_cleanup_failed` with `phase` (`dispose` or `boot_failure`), its `cleanupIndex` and the `loggableError` projection, and the drain goes on. At `dispose()` the line goes to the `logger` component; when boot failed, to the `logger` bootstrap component the composition root passed in; either way to `consoleLogger` when there is none — [`create-app.test.mts`](./__tests__/create-app.test.mts).
