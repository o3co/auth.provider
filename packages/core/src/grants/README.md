...
It is separate because every grant package — `oauth`, `device-grant`, `oauth-token-exchange`, `webauthn` — implements this one contract and mints through these helpers, and they do not all depend on one another — `oauth-token-exchange` and `webauthn` depend only on core — so a rule two grants share (the policy ceilings, confirmation matching, the claim filter) has one home here. `GrantRegistry` is the exception to "shared by grants": in product code only `boot/` uses it, as the `grants` collector (`testing/` re-exports it for tests), which makes it assembly code kept here beside the type it holds — a judgement call.
...
- Token minting is [`token.mts`](./token.mts); every other shared rule (id_token, logout_token, the claim filter, policy evaluation, confirmation matching, token binding, sender constraint, authentication claims, the email-verified gate) is one file each, named for what it holds.
