...
- [`types.mts`](./types.mts) — the records. Each field's semantics are documented once, on the field: deny-by-absence for `allowedGrantTypes`, `defaultScopes`, `allowedFederationGrantConnections`, `federationGrantRedirectUris` and `allowedAzpForFederationToken`; strict `=== true` for `firstParty` and `allowPlainPkce`; the `allowedAudiences` fallbacks.
...
- Every registered redirect-URI list is held to `../net/redirect-uri` at registration; a `federationGrantRedirectUris` entry that already carries the flow's result parameter is refused (`../federation-grants/lodge.mts`).
