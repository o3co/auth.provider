...
export function checkCanonicalIssuer(value: unknown): IssuerRejection | null {
	...
	if (url.hostname === "") return "not-absolute-url";
	if (url.protocol !== "https:" && url.protocol !== "http:") return "unsupported-scheme";
	if (url.protocol === "http:" && !LOOPBACK_HOSTNAMES.has(url.hostname)) return "insecure-scheme";
...
export function describeIssuerRejection(reason: IssuerRejection): string {
	switch (reason) {
		case "not-a-string":
