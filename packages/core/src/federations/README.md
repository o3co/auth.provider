...
| [`scope.mts`](./scope.mts) | contract | RFC 6749 §3.3's scope grammar: [`isScopeToken`](./scope.mts), [`parseScopeTokens`](./scope.mts) and [`canonicalScope`](./scope.mts), the form a scope is written and compared in. Three packages had a copy of this before #647 and all three were wrong about whitespace the same way. No state. |
| [`token-type.mts`](./token-type.mts) | contract | RFC 6749's `token_type`: [`BEARER_TOKEN_TYPE`](./token-type.mts), [`canonicalTokenType`](./token-type.mts) (is this a `token-type` at all, per §A.13 — a `type-name` or a URI reference, checked against RFC 3986's grammar including its structure) and [`isBearerTokenType`](./token-type.mts) (§5.1's case-insensitive comparison). What an upstream token may be handed on as, asked by both routes that hand one on (#645). No state. |
...
`types` → `response-mode` (the `responseMode` field) · `response-mode` → `types` (`Pick<FederationProvider, "responseMode">`), type-only in both directions · `token-type` → `node:net` (`isIPv6`, for the inside of an IP-literal) and nothing in core. It once borrowed `isScopeToken` from `scope` as a bound on what a token type could be, then used a character-set check, and neither was enough: the first read `"Bearer^"` as a type name, the second `"https://["`, and both sent an adapter's garbage down the refusal meant for a real type the upstream issued (#649). It checks RFC 3986's grammar, structure included, now. Nothing else: no store, no config, no logger. Imported inside core by `../modules/manifest/contributes-map` (which is how `../modules/manifest/synthetic-keys` and `../boot/` reach it), `../federation-grants/retrieve`, `../federation-grants/eligibility` and the package root; outside core, by the session router, `oauth`'s logout and federation-token routes, `federation-grants` and the four adapters.
...
| [`__tests__/scope.test.mts`](./__tests__/scope.test.mts) | the grammar, the whitespace readings the three copies got wrong, and the canonical form. |
| [`__tests__/token-type.test.mts`](./__tests__/token-type.test.mts) | §A.13's grammar — URNs, relative and network-path references, IPv6 and IPvFuture literals and pct-encoding in; characters no URI may contain, and legal characters in an illegal structure (an unclosed IP-literal, a bracket in a path, a non-numeric port), out — the case-insensitive comparison, and that a value which merely contains `bearer` is refused. |
