...
| [`token-type.mts`](./token-type.mts) | contract | RFC 6749's `token_type`: [`BEARER_TOKEN_TYPE`](./token-type.mts), [`canonicalTokenType`](./token-type.mts) (is this a name at all) and [`isBearerTokenType`](./token-type.mts) (§5.1's case-insensitive comparison). What an upstream token may be handed on as, asked by both routes that hand one on (#645). No state. |
...
`types` → `response-mode` (the `responseMode` field) · `response-mode` → `types` (`Pick<FederationProvider, "responseMode">`), type-only in both directions · `token-type` → `scope`, for a character class that is a superset of what it needs: §A.13's `token-type = type-name / URI-reference` is not §3.3's `scope-token = 1*NQCHAR`, but NQCHAR admits everything both alternatives can contain, so `isScopeToken` rejects nothing a token type may be. It is a bound, not a parse — `isBearerTokenType` is what decides. Nothing else: no store, no config, no logger. Imported inside core by `../modules/manifest/contributes-map` (which is how `../modules/manifest/synthetic-keys` and `../boot/` reach it), `../federation-grants/retrieve`, `../federation-grants/eligibility` and the package root; outside core, by the session router, `oauth`'s logout and federation-token routes, `federation-grants` and the four adapters.
...
| [`__tests__/token-type.test.mts`](./__tests__/token-type.test.mts) | what is a type name and what is not, the case-insensitive comparison, and that a value which merely contains `bearer` is refused. |
