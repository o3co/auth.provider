...
`types` → `response-mode` (the `responseMode` field) · `response-mode` → `types` (`Pick<FederationProvider, "responseMode">`), type-only in both directions · `token-type` depends on nothing. It once borrowed `isScopeToken` from `scope` as a bound on what a token type could be, and a bound was not enough: it read `"Bearer^"` as a type name and sent an adapter's garbage down the refusal meant for a real type the upstream issued (#649). It checks §A.13's own grammar now. Nothing else: no store, no config, no logger. Imported inside core by `../modules/manifest/contributes-map` (which is how `../modules/manifest/synthetic-keys` and `../boot/` reach it), `../federation-grants/retrieve`, `../federation-grants/eligibility` and the package root; outside core, by the session router, `oauth`'s logout and federation-token routes, `federation-grants` and the four adapters.
...
| [`__tests__/token-type.test.mts`](./__tests__/token-type.test.mts) | §A.13's grammar — URNs and pct-encoding in, printable characters no URI may contain out — the case-insensitive comparison, and that a value which merely contains `bearer` is refused. |
