...
11. **A token response has one reading.** `expires_in` as sent, `expiresAt` dated from when the answer arrived, and `null` on both when none was sent — no lifetime is assumed; `tokenType` as the library reported it; `scope` present exactly when sent, an empty one included; refresh and id tokens only when non-empty. Every bundled adapter's login and refresh go through `federationTokenSnapshot` — [`token-snapshot.test.mts`](./__tests__/token-snapshot.test.mts).
