 ...
 */
export function createFederationGrantStoreFactory(): FederationGrantStoreFactory {
	return createAdapterFactory<FederationGrantStore>("FederationGrantStore");
}
 ...
 */
export function registerBuiltinFederationGrantStores(
	factory: FederationGrantStoreFactory,
	logger: Logger = consoleLogger,
): void {
	factory.register("memory", () => {
		logger.warn(
			"federationGrantStore: in-memory adapter is for dev/test only — do not use in production (grants and their upstream refresh tokens are held unsealed, lost on restart, and not shared between instances).",
		);
