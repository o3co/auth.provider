...
- Two contribution value types are `unknown` today: `FederationProvider` ([`contributes-map.mts`](./contributes-map.mts) line 62) and `ExchangeTokenValidator` (line 70). Their concrete types live in `packages/session` and `packages/oauth-token-exchange`, and consumers cast the registry value back (F1 / F2 in o3co/auth.provider#626). P1 of that issue tracks carrying the concrete type from registration to use; nothing here does so yet.
