...
A store that keeps a secret at rest seals it in the `v2` key-ring envelope ([`src/sealing/envelope.mts`](src/sealing/envelope.mts)). `sealWithKeyRing(plaintext, ring, { purpose, record })` returns `v2.<key id>.<iv>.<ciphertext>.<tag>`, AES-256-GCM under the ring's first key. `openWithKeyRing(envelope, ring, { purpose, record })` answers an `OpenedSeal`: `ok` with the value; `key_unavailable` when the envelope names a key the ring no longer holds, which an operator undoes by putting the key back; or `unreadable` for anything else, a value bound to another purpose or record included. It never throws for the envelope. The purpose label (1 to 64 printable ASCII characters, no space) and the record's bytes are authenticated and not stored, so a value copied into another record, or read by another caller sealing under the same ring, does not open.

A ring (`SealingKeyRing`: `SealingKey` entries `{ id, key }`, in [`src/sealing/keyRing.mts`](src/sealing/keyRing.mts)) seals with its first key and opens with any. An ID is 1 to 64 characters of `A-Za-z0-9_-` and a key is `SEALING_KEY_BYTES` (32) bytes. Both functions throw a `RangeError` on a ring that breaks the rule or a purpose that breaks its own, and sealing does on an empty ring, so a store can refuse a bad ring at construction by sealing an empty value once. `decodeSealingKey` reads a configured key: canonical base64 of exactly 32 bytes with no whitespace, or `undefined` for the caller to refuse, naming its configuration key. `@o3co/auth-provider-redis`'s federation grant store seals its credentials this way.
