...
- **`error_description`** is the one peer-written string kept on purpose, because it tells a revoked grant from a broken client: its first line only (Azure AD's AADSTS line, not its Trace ID lines), when that line is within RFC 6749 §5.2's character set and carries no run of twenty or more token characters (`[A-Za-z0-9._~+/=-]` — legacy Spring echoes "Invalid refresh token: <the token>").
...
- Every string is capped at 256 characters. It never throws. Under pino, `type` reads as the error's name.
