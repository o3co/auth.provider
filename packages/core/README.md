...
| `endpoints` | `login.url`: the deployment's login page. `consent.url`: its consent page for clients that are not first-party (default `/consent`). `mfa.url`: its page for a second factor at `/authorize` (default `/mfa`); nothing sends a browser there yet |
| `mfa.mode` | Whether a password login asks for a second factor — see [MFA](#mfa). `"off"` (the default) is the only value this release accepts: no module here asks for or verifies a second factor, so any other value refuses boot, naming the key. The rest of the `mfa` section belongs to the package that reads it and passes through |
| `mfaFactorStore.adapter`, `mfaTransactionStore.adapter` | Which store keeps enrolled factors (`memory`, `redis` or `store`) and which keeps MFA transactions and the lock state (`memory` or `redis`); both `memory` by default. Read only by a composition that installs MFA, which none does while `mfa.mode` is `"off"` |
...
- `MfaCoordinator`, what the login route and `/authorize` consult about MFA, filling the `mfaCoordinator` slot, with `PrimaryAuthentication` — [`src/mfa/coordinator.mts`](src/mfa/coordinator.mts). `decideAfterPrimary` reads the subject's factors before anything is written and throws on an outage; `openLoginTransaction` binds the transaction to the session id the browser will hold after regeneration. `MFA_ABSENCE_POLICY` makes an unfilled slot a statement, `mfa.mode = "off"`, for a module that reads it; no bundled module attaches it yet.
- The enrollment witness, which keeps a lost factor store from reading as "never enrolled" (D12): `User.mfaEnrolled`, which the Store answers on `authenticate`, and the optional `UserRepository.markMfaEnrolled(subject, enrolled)`, detected by `supportsMfaEnrollmentWitness()` — [`src/repositories/UserRepository.mts`](src/repositories/UserRepository.mts). `InMemoryUserRepository` has no `markMfaEnrolled`.
- `MfaFactorStore`, where a subject's enrolled factors are kept, and `MfaFactorRecord` — [`src/mfa/factorStore.mts`](src/mfa/factorStore.mts). A record's `data` is sealed before it reaches the store, which keeps it byte for byte; `update` is a compare-and-set on `version`; a store that cannot answer throws rather than answering "no factors". The in-process adapter `createMemoryMfaFactorStore()` and its module `memoryMfaFactorStoreModule` are for development and a single replica: a restart empties them, which the module warns about once (`mfa_factor_store_in_memory`), and `deployment.mode = "multi"` refuses the module. `createMfaFactorStoreFactory()` / `registerBuiltinMfaFactorStores()` build it by name. Every adapter runs [`src/mfa/__tests__/factorStore.contract.mts`](src/mfa/__tests__/factorStore.contract.mts).
- `MfaTransactionStore`, where MFA transactions and the subject lock state are kept, with `MfaTransaction`, `MfaTransactionPatch` and `MfaLockoutPolicy` — [`src/mfa/transactionStore.mts`](src/mfa/transactionStore.mts). A transaction is the single-use record of one second-factor ceremony; `reserveAttempt`, `takeChallenge` and `consume` are atomic, so attempts in flight are each spent, a challenge is answered once and one verification consumes the transaction. `create` refuses a record that does not start from a fresh count (`checkNewMfaTransaction`), and `update` writes only its patch keys: a value sets, `null` clears a field that may be empty, `undefined` is absent, and anything else is a `RangeError` (`mfaTransactionPatchWrites`, which every adapter calls first). The subject state is the lock on guessable proofs, judged on the time each caller passes, so the callers' clocks must agree: the consecutive run (a short backoff from `threshold` failures, doubling to `maxSeconds` and forgotten `memorySeconds` after the last lock ends — or, before any lock, after the previous failure; a hold at `hardLimit` until an exempt success; a success ends the run up to its own reservation), the weekly budget of failures in any rolling seven days (`MFA_WEEKLY_WINDOW_MS`) that no success refunds, and the browsers an exempt success trusts against the weekly hold — renewed, not added, when the success came from a browser already trusted. A store keeps a failure `MFA_CLOCK_SKEW_ALLOWANCE_MS` (a day) after it stops counting, so a caller whose clock runs ahead erases nothing another still counts. `clearSubjectState` — the operator reset and a password change — clears the run, the week and the trusts; the week clearing on a password change is deliberate. Beside the lock, the store keeps the operator reset's email-proof requirement until the subject's next first binding consumes it (`requireEmailProofAtNextBinding`, `emailProofRequiredAtNextBinding`, `consumeEmailProofRequirement`); `clearSubjectState` leaves it. `checkMfaLockoutPolicy()` refuses a policy a store cannot apply with a `RangeError`, among them a `threshold` above `hardLimit` and a `hardLimit` above NIST's 100 (`MFA_LOCKOUT_MAX_HARD_LIMIT`). The in-process adapter `createMemoryMfaTransactionStore()` and `memoryMfaTransactionStoreModule` are for development and a single replica, refused under `deployment.mode = "multi"`; the adapter holds at most `mfaTransactionStore.memory.maxEntries` transactions (`DEFAULT_MEMORY_MFA_TRANSACTION_STORE_MAX_ENTRIES`, a hundred thousand) and at the cap refuses a new one with `MfaTransactionStoreFullError`, a store fault, rather than evict a live one; `createMfaTransactionStoreFactory()` / `registerBuiltinMfaTransactionStores()` build it by name. Every adapter runs [`src/mfa/__tests__/transactionStore.contract.mts`](src/mfa/__tests__/transactionStore.contract.mts).
