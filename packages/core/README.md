...
- `jwks_uri` — always advertised (contributed by `jwksModule`); an issuer-configured composition MUST install `jwksModule` or boot fails fast with `DiscoveryDocumentError`. The route never publishes an empty key set: an HS256 deployment answers `404 jwks_not_published`, and an asymmetric keystore that yields no exportable public key answers `503 jwks_unavailable`, both with `Cache-Control: no-store`; the `503` is logged at error level as `jwks_unavailable` when the router has a logger (`jwksModule` passes the composition's). The symmetric secret is never published either way. A `200` from this route always carries at least one key.
