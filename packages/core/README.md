...
- **The command a store's error answered**, by name alone: `command: { name }` from ioredis's `command: { name, args }`, when the name is a token of at most 32 letters, digits and `_` — which Redis command failed (`set`, `evalsha`, `hello`), never its arguments. It stays at ioredis's own path, so a query on `err.command.name` reads a raw and a projected line alike; a `command` that is a string (execa's shell line) is not kept.
- **A budget for one line**: at most sixteen projections (`LOGGED_MAX_PROJECTIONS`) — the error, its causes and its members together — taken nearest first, so the error's own cause and members come before any of theirs. A member the budget leaves out counts in `aggregateErrorsOmitted`; a cause it leaves out leaves `causeOmitted: true`. With every string capped, a line stays under about 64 KB.
...
- Every string is capped at 256 characters. It never throws. `consoleLogger` hands the projection to `console.*` as it is, and has the console print it eight levels deep instead of its default two. That is past the deepest level a projection nests to, so its causes and members print whole rather than as `[Object]`. It does this through a non-enumerable `util.inspect.custom`, so a spy or aggregator reading the object sees only its fields.
