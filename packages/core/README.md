...
- Never kept: a cause or a member that is not an Error (where openid-client puts the answer it refused), any other field (`command`, `body`, `buffer`), and anything of a thrown non-Error but its `typeof`, as `thrown`.
