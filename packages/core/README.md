...
- Every string is capped at 256 characters. It never throws. `consoleLogger` hands the projection to `console.*` as it is; the console's own inspection prints objects two levels deep and folds deeper ones to `[Object]`, which is its formatting, not a field the projection lacks.
