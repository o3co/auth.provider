...
- **`message`** is kept, with the two known quoting shapes removed: a `SyntaxError`'s message is dropped — V8's `JSON.parse` and body-parser quote the input — keeping only ` at position N` (at most ten digits) as `position`; and Redis's `, with args beginning with: …` is cut from every message, whichever client's class carries it. Other text a peer wrote into a message is kept: the projection cannot tell it from this process's own.
- **`error_description`** is the one peer-written string kept on purpose, because it tells a revoked grant from a broken client: its first line only (Azure AD's AADSTS line, not its Trace ID lines), when that line is within RFC 6749 §5.2's character set, cut before its first run of twenty or more token characters (`[A-Za-z0-9._~+/=-]`) and trimmed — legacy Spring's "Invalid refresh token: <the token>" keeps "Invalid refresh token:", Azure AD's AADSTS700016 the text up to the application id — and omitted when nothing is left.
- **`stack`** keeps the frames and never the header line that carries the message: at most ten frames, 2048 characters, at every cause.
...
- Every string is capped at 256 characters. It never throws. Under pino, `type` reads as the error's name: each projection has a non-enumerable own `constructor` of `undefined`, invisible to JSON and to `util.inspect` — so in a test, compare a projection with `toEqual`; `toStrictEqual` fails on the hidden constructor.
