...
- a revoked grant whose lifetime exceeds the refresh-token watermark retention
  ...
  again (D13) — each ending in a `/token` call after the pre-#593 horizon has
  elapsed (`subjectRevocation.acceptance.test.mts`, "a revocation outlives the
  watermark retention sized before #593");
