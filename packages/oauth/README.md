...
**Error descriptions.** Every `error_description` that `/oauth/token` sends, that `/oauth/authorize` puts in an error redirect, and that client authentication sends — on `/oauth/token`, `/oauth/introspect` and `/oauth/revoke`, whose errors use the same format (RFC 7662 §2.3, RFC 7009 §2.2.1) — is held to the characters RFC 6749 allows in one (§5.2, §4.1.2.1): printable ASCII without `"` and `\`. Any other character — including in a value the client sent and a description quotes, such as a grant type, scope, audience, token type or `response_type`, and in a configured value such as a client's `tokenEndpointAuthMethod` — is replaced with `?` ([`errorDescription.mts`](./src/errorDescription.mts)), whichever grant, route or authentication step wrote the description. Descriptions quote a value with `'`.
