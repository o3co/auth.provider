...
- Each module under `/oauth` parses its own body, and the order they are listed in does not matter. `oauthModule`'s router parses JSON and form bodies (Express's default limits) for the routes in the [Endpoints](#endpoints) table only — `OAUTH_ROUTE_PATHS` in [`routes.mts`](src/routes.mts); a request to any other path under `/oauth` — the device grant's, federation grants', WebAuthn's, a deployment's own — reaches its route with the body unread. A module that mounts a route there and reads `req.body` mounts a parser of its own.
