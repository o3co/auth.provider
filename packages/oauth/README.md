...
The ask is deliberately not a parameter on the URL the browser carries. It was, until v0.13.0, and a request supplying `reauth_after=0` itself satisfied the check for any live session — skipping the round trip the parameter exists to force. It also meant a login page that rebuilt the authorize URL rather than returning `redirect_to` verbatim dropped the ask and looped. Being the server's own record, it survives whatever the page does to the URL, and it is spent when it is read: one ask, one answer.
