...
| 410 | `re_authentication_required` | The IdP rejected the refresh token: `invalid_grant` / `invalid_token` in the `error` of what its library raised, under a status that is not a 5xx — the session's federation is cleared; the user must re-authenticate with the IdP. A 5xx is never this, whatever its body names, and neither is an error whose message merely contains the code: those keep the stored tokens (`503` and `500` below) |
