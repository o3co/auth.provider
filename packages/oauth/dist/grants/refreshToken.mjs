...
export const createRefreshTokenGrant = (deps) => {
    const { config, keyStore } = deps;
    return {
        async handle(ctx) {
            const { body, issuer } = ctx;
            const { refresh_token: refreshTokenValue, client_id, scope: requestedScope, } = body;
            ...
            const legacyType = tokenPayload.type;
            ...
            const subjectStr = typeof tokenPayload.sub === "string"
                ? tokenPayload.sub
                : typeof claims.user?.id === "string"
                    ? claims.user.id
                    : undefined;
            const azpStr = typeof claims.azp === "string"
                ? claims.azp
                : typeof claims.client?.id === "string"
                    ? claims.client.id
                    : undefined;
            const scopeStr = typeof claims.scope === "string"
                ? claims.scope
            ...
            let finalAudience = tokenAud ?? client_id ?? null;
            if (deps.grantPolicy) {
            ...
            const tokenPayloadClaims = tokenPayload;
            ...
            const previousJti = typeof tokenPayloadClaims.jti === "string" ? tokenPayloadClaims.jti : null;
            ...
            const newAccessToken = await generateToken({ family_id: newFamilyId, ...(sid ? { sid } : {}) }, {
            ...
            const newRefreshToken = await generateToken({ family_id: newFamilyId, ...(sid ? { sid } : {}) }, {
            ...
            if (deps.refreshTokenStore && previousJti !== null) {
                ...
                const newJti = newRefreshPayload.jti;
                const newExp = newRefreshPayload.exp;
                if (typeof newJti === "string" && typeof newExp === "number") {
                    ...
                    let rotateResult;
                    try {
                        rotateResult = await deps.refreshTokenStore.rotate(previousJti, newJti, newFamilyId, new Date(newExp * 1000));
                    }
                    catch {
                        return {
                            result: {
