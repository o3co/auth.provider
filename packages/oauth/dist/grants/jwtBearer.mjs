 ...
 */
export const createJwtBearerGrant = (deps) => {
    ...
    return {
        ...
        async handle(ctx) {
            ...
            if (deps.grantPolicy) {
                const policy = await evaluateGrantPolicy(deps.grantPolicy, {
                    grantType: JWT_BEARER_GRANT_TYPE,
                    clientId,
                    subject,
                    requestedScope: effectiveScopes.length > 0 ? [...effectiveScopes] : undefined,
                }, { ip: ctx.ip, userAgent: ctx.userAgent, issuer: ctx.issuer ?? "" }, effectiveScopes);
                if (!policy.ok)
            ...
            const accessToken = await generateToken({ ...(clientId ? { client_id: clientId } : {}) }, {
 ...
 */
function resolveScope(ctx, assertionScope) {
    ...
    const ceilings = [assertionScope, client?.allowedScopes].filter((c) => c !== undefined);
    const within = (s) => ceilings.every((c) => c.includes(s));
    ...
    if (ceilings.length === 0) {
        return {
            status: 400,
            error: "invalid_scope",
            errorDescription: "scope was requested but nothing bounds it: the assertion names no scope and " +
