...
import { generateIdToken, generateToken, generateTokenResponse, } from "@o3co/auth-provider-core";
import { decodeJwtPayload } from "./_jwtPayload.mjs";
export const createAuthorizationGrant = (deps) => {
    ...
    return {
        async handle(ctx) {
            ...
            const { code, code_verifier = null, client_id, client_secret = null, redirect_uri = null, } = body;
            ...
            const grantedScopes = codeData.grantedScope ?? session.granted_scopes;
            ...
            if (deps.userSessionStore && !sid) {
                return {
                    result: {
                        status: 400,
                        error: "invalid_grant",
                        errorDescription: "code record is missing session identifier (sid) — ensure login wiring records sid at authorize time",
                    },
                };
            }
            ...
            const audience = grantedAudiencesFromCode && grantedAudiencesFromCode.length > 0
            ...
            const accessToken = await generateToken({ family_id: familyId, ...(sid ? { sid } : {}) }, {
            ...
            const refreshToken = await generateToken({ family_id: familyId, ...(sid ? { sid } : {}) }, {
            ...
            if (deps.userSessionStore && sid) {
                ...
                try {
                    ...
                    await deps.userSessionStore.registerRP(sid, {
                        clientId: client_id,
                        backchannelLogoutUri: clientRecord
                            ?.backchannelLogoutUri,
                        backchannelLogoutSessionRequired: clientRecord
                            ?.backchannelLogoutSessionRequired,
                        frontchannelLogoutUri: clientRecord
                            ?.frontchannelLogoutUri,
                        frontchannelLogoutSessionRequired: clientRecord
