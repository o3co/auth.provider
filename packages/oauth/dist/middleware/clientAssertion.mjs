 ...
 */
import { consoleLogger, createRemoteKeySetCache, } from "@o3co/auth-provider-core";
import { createLocalJWKSet, decodeJwt, errors, jwtVerify, } from "jose";
/**
...
export const hasClientAssertion = (body) => body !== undefined &&
    (body.client_assertion !== undefined || body.client_assertion_type !== undefined);
export function createClientAssertionVerifier(options) {
    const logger = options.logger ?? consoleLogger;
    const clockTolerance = options.clockToleranceSeconds ?? 30;
    const now = options.now ?? Date.now;
    const audiences = [options.issuer, options.tokenEndpoint].filter((value) => typeof value === "string" && value.length > 0);
    ...
    const refuse = (status, error, description, reason, context = {}) => {
    ...
    return {
        async verify(body, findClient) {
            ...
            if (typeof assertion !== "string" ||
                ...
                return refuse(400, "invalid_request", "client_assertion and client_assertion_type are required together", "half_present");
            }
            if (assertionType !== JWT_BEARER_CLIENT_ASSERTION_TYPE) {
                return refuse(401, "invalid_client", `Unsupported client_assertion_type; only ${JWT_BEARER_CLIENT_ASSERTION_TYPE} is accepted`, "unsupported_type", { assertionType });
            }
            if (audiences.length === 0) {
                return refuse(500, "server_error", "private_key_jwt client authentication needs a configured issuer to check the assertion audience against", "no_audience");
            }
            ...
            if (typeof iss !== "string" || iss.length === 0 || unverified.sub !== iss) {
                return refuse(401, "invalid_client", "client assertion iss and sub must both be the client_id (RFC 7523 §3)", "iss_sub_mismatch");
            }
            const bodyClientId = body?.client_id;
            if (bodyClientId !== undefined && bodyClientId !== iss) {
                return refuse(401, "invalid_client", "client_id does not match the client assertion", "client_id_mismatch", { clientId: iss });
            }
            let client;
            try {
            ...
            if (client.tokenEndpointAuthMethod !== "private_key_jwt") {
                return refuse(401, "invalid_client", `tokenEndpointAuthMethod mismatch: client is configured for "${client.tokenEndpointAuthMethod}"`, "method_mismatch", { clientId: iss });
            }
            let keys;
            try {
            ...
            if (!keys) {
                return refuse(500, "server_error", "client registration carries no usable keys for private_key_jwt", "no_keys", { clientId: iss });
            }
            let payload;
            try {
            ...
            if (exp - nowSeconds > MAX_CLIENT_ASSERTION_LIFETIME_SECONDS) {
                return refuse(401, "invalid_client", `client assertion exp is too far ahead (at most ${MAX_CLIENT_ASSERTION_LIFETIME_SECONDS} seconds)`, "lifetime", { clientId: iss });
            }
            ...
            if (iat !== undefined) {
                if (iat - nowSeconds > clockTolerance) {
                    return refuse(401, "invalid_client", "client assertion iat is in the future", "iat_future", { clientId: iss });
                }
                if (nowSeconds - iat > MAX_CLIENT_ASSERTION_LIFETIME_SECONDS + clockTolerance) {
                    return refuse(401, "invalid_client", `client assertion iat is too old (at most ${MAX_CLIENT_ASSERTION_LIFETIME_SECONDS} seconds)`, "iat_stale", { clientId: iss });
                }
            }
            const jti = payload.jti;
            if (typeof jti !== "string" || jti.length === 0) {
                return refuse(401, "invalid_client", "client assertion jti must be a non-empty string", "jti_shape", { clientId: iss });
            }
            ...
            if (!options.replaySeenSet) {
                return refuse(500, "server_error", "private_key_jwt client authentication needs a replaySeenSet to record each assertion's jti; none is wired", "replay_store_missing", { clientId: iss });
            }
            let fresh;
            try {
                ...
                fresh = await options.replaySeenSet.markSeen(`${CLIENT_ASSERTION_REPLAY_SCOPE_PREFIX}${iss}`, jti, (exp + clockTolerance) * 1000);
            }
            catch (err) {
                return refuse(503, "temporarily_unavailable", "Client authentication is temporarily unavailable", "replay_store_unavailable", { err, clientId: iss });
            }
            if (!fresh) {
                return refuse(401, "invalid_client", "client assertion jti has already been used", "replay", { clientId: iss });
            }
