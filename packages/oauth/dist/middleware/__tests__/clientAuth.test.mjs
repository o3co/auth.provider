...
const fakePublicClient = (clientId) => ({
    clientId,
    allowedRedirectUris: [],
    allowedScopes: [],
});
const fakeRepo = (creds) => ({
    findById: async () => null,
    authenticate: async (id, secret) => {
        if (creds[id] === secret)
            return fakePublicClient(id);
        return null;
    },
});
describe("createClientAuthMiddleware", () => {
    ...
    it("splits on first colon only — client_id containing colon uses remainder as secret", async () => {
        const authenticateSpy = vi.fn().mockResolvedValue(null);
        const spyRepo = { authenticate: authenticateSpy, findById: vi.fn() };
        const app = express().use(express.urlencoded({ extended: false }));
