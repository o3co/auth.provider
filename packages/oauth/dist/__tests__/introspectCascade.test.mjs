...
async function buildApp(refreshTokenStore, auditSink) {
    const app = express();
...
async function introspect(app, token) {
    return request(app)
...
describe("/introspect — family revoke cascade (TODO-F-3 task 5)", () => {
    ...
    it("emits introspect.store_unavailable audit event when isFamilyRevoked throws", async () => {
        ...
        expect(storeEvent?.details?.family_id).toBe(familyId);
        expect(storeEvent?.details?.error).toContain("backend down");
    });
    it("emits introspect.family_revoked audit event when family is revoked", async () => {
        ...
        expect(revokedEvent?.details?.family_id).toBe(familyId);
    });
