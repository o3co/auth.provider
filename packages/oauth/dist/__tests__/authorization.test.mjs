...
function makeDeps(consumeByCodeImpl, clientRepository) {
    return {
        config: mockConfig,
    ...
    describe("handle", () => {
        ...
        it("returns 200 with access and refresh tokens on valid code exchange (no PKCE)", async () => {
            ...
            if ("tokens" in result) {
                ...
                expect(decoded.azp).toBe("client1");
            }
        ...
        it("registers initial rt+jwt via refreshTokenStore.rotate(null, ...) (CP-2)", async () => {
            const rotateSpy = vi.fn(async (_prev, _next, _fam, _exp) => ({ outcome: "rotated" }));
            ...
            const [previousJti, newJti, familyId, expiresAt] = rotateSpy.mock.calls[0];
            expect(previousJti).toBeNull();
            expect(typeof newJti).toBe("string");
            expect(newJti.length).toBeGreaterThan(0);
            expect(familyId).toMatch(/^[0-9a-f]{8}-[0-9a-f]{4}-4[0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/);
        ...
        it("issues an initial rt+jwt carrying a new family_id (C-3)", async () => {
            ...
            if (typeof refreshToken !== "string")
                throw new Error("expected refresh_token string");
            const decoded = decodeJwt(refreshToken);
            expect(typeof decoded.family_id).toBe("string");
            // UUID v4 shape: 8-4-4-4-12 hex, version nibble = 4
            expect(decoded.family_id).toMatch(/^[0-9a-f]{8}-[0-9a-f]{4}-4[0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/);
        });
        ...
        describe("A-3: client secret verification", () => {
            ...
            it("returns 200 when client_secret is correct", async () => {
                ...
                const deps = makeDeps(vi.fn().mockResolvedValue({ code: "abc", sid: "test-sid-1" }), clientRepo);
                const handler = createAuthorizationGrant(deps);
                const ctx = {
        ...
        describe("TODO-F-4: id_token issuance on openid scope", () => {
            ...
            function makeDepsWithIssuer(consumeByCodeImpl, clientRepository) {
                return {
            ...
            function makeUserSessionStore(session) {
                return {
                    kind: "spy",
                    linkFamily: vi.fn(async (_sid, _fam) => { }),
                    registerRP: vi.fn(async (_sid, _rp) => { }),
                    async create() { },
                    async get(querySid) {
                        if (querySid !== session.sid)
                            return null;
                        return {
                            sid: session.sid,
                            sub: session.sub,
            ...
            it("includes id_token in response when scope contains 'openid' and userSessionStore is wired (F-4-1)", async () => {
                ...
                const deps = {
                    ...makeDepsWithIssuer(vi.fn().mockResolvedValue({
                        code: "c1",
                        sid: "sid-1",
                        nonce: "client-nonce",
                ...
                if (typeof idTokenStr !== "string")
                    throw new Error("expected id_token string");
                const idPayload = decodeJwt(idTokenStr);
            ...
            it("does NOT include id_token when userSessionStore is not wired (backward compat, F-4-3)", async () => {
                ...
                const deps = makeDepsWithIssuer(vi.fn().mockResolvedValue({
                    code: "c3",
                    sid: "sid-3",
                    grantedScope: ["openid"],
                }));
        ...
        describe("TODO-F-3: family_id + sid claims, RP registration", () => {
            it("happy path: access_token and refresh_token both carry family_id and sid claims (F-3-1)", async () => {
                ...
                if (!("tokens" in result))
                    throw new Error("expected tokens");
                const decodedAt = decodeJwt(result.tokens.access_token);
                expect(typeof decodedAt.family_id).toBe("string");
                expect(decodedAt.family_id).toMatch(/^[0-9a-f]{8}-[0-9a-f]{4}-4[0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/);
                expect(decodedAt.sid).toBe("session-abc");
                const refreshToken = result.tokens.refresh_token;
                if (typeof refreshToken !== "string")
                    throw new Error("expected refresh_token string");
                const decodedRt = decodeJwt(refreshToken);
            ...
            it("returns 400 invalid_grant when code record has no sid and userSessionStore IS wired (F-3-2)", async () => {
                ...
                expect(result.errorDescription).toMatch(/sid/);
            });
            ...
            it("calls linkFamily and registerRP when userSessionStore is wired (F-3-3)", async () => {
                const linkFamilySpy = vi.fn(async (_sid, _fam) => { });
                const registerRPSpy = vi.fn(async (_sid, _rp) => { });
                const userSessionStore = {
                    kind: "spy",
                ...
                const [sidArg, familyIdArg] = linkFamilySpy.mock.calls[0];
                expect(sidArg).toBe("session-xyz");
                expect(familyIdArg).toMatch(/^[0-9a-f]{8}-[0-9a-f]{4}-4[0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/);
                expect(registerRPSpy).toHaveBeenCalledTimes(1);
                const [rpSid, rpData] = registerRPSpy.mock.calls[0];
            ...
            it("returns 503 temporarily_unavailable when clientRepository.findById throws (F-3-I2)", async () => {
                ...
                const deps = {
                    ...makeDeps(vi.fn().mockResolvedValue({ code: "abc", sid: "session-abc" }), throwingClientRepo),
                    userSessionStore,
                };
                const handler = createAuthorizationGrant(deps);
                const { result } = await handler.handle({
