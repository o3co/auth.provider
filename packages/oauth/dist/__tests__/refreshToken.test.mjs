...
async function makeRefreshToken(overrides = {}) {
...
describe("createRefreshTokenGrant", () => {
    ...
    describe("family_id and refreshTokenStore integration", () => {
        function createStubRefreshTokenStore(onRotate) {
            return {
                kind: "stub",
        ...
        it("emits family_id in the new rt+jwt (generated when absent from input)", async () => {
            ...
            expect(result.status).toBe(200);
            if ("tokens" in result) {
                const rtToken = result.tokens.refresh_token;
                ...
                expect(payload.family_id).toMatch(/^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/);
            }
            else {
        ...
        it("returns invalid_grant/replay_detected when the store reports 'replayed'", async () => {
            ...
            const depsWithStore = { ...mockDeps, refreshTokenStore: stub };
        ...
        it("returns 503 temporarily_unavailable when refreshTokenStore.rotate throws (CP-17)", async () => {
            ...
            const depsWithStore = { ...mockDeps, refreshTokenStore: throwingStore };
            const token = await new SignJWT({ sub: "u1", scope: "read write" })
                .setProtectedHeader({ alg: "HS256", kid: "v0", typ: "rt+jwt" })
                .setExpirationTime("24h")
                .setJti("prev-jti-503")
    ...
    describe("refresh_token grant — grantPolicy hook", () => {
        function createStubPolicy(evaluate) {
        ...
        it("narrows scope when policy returns allow with grantedScope", async () => {
            ...
            const depsWithPolicy = { ...mockDeps, grantPolicy: policy };
    ...
    describe("sid claim propagation and userSessionStore integration (TODO-F-3 task 4)", () => {
        ...
        function createStubUserSessionStore(get) {
            return {
                kind: "stub",
                get,
                async create() { },
        ...
        it("preserves family_id and sid on both minted tokens (F-3-4-1)", async () => {
            ...
            const deps = { ...mockDeps, userSessionStore: store };
            ...
            if (!("tokens" in result))
                expect.fail("Expected tokens in result");
            const atPayload = decodeTokenPayload(result.tokens.access_token);
            expect(atPayload.family_id).toBe("fam-1");
            expect(atPayload.sid).toBe("sid-1");
            const rtPayload = decodeTokenPayload(result.tokens.refresh_token);
