 ...
 */
import { generateLogoutToken } from "@o3co/auth-provider-core";
 ...
 */
export async function broadcastBackchannelLogout(opts) {
    const fetchImpl = opts.fetchImpl ?? fetch;
    const timeoutMs = opts.timeoutMs ?? DEFAULT_TIMEOUT_MS;
    const logger = opts.logger ?? console;
    const tasks = opts.rps
        .filter((rp) => typeof rp.backchannelLogoutUri === "string" && rp.backchannelLogoutUri.length > 0)
        .map(async (rp) => {
        try {
            ...
            try {
                ...
                if (!res.ok) {
                    logger.warn(`broadcastBackchannelLogout: RP ${rp.clientId} returned ${res.status} ${res.statusText ?? ""}`.trim());
                }
            }
            catch (err) {
                logger.warn(`broadcastBackchannelLogout: RP ${rp.clientId} POST to ${rp.backchannelLogoutUri} failed:`, err);
            }
            finally {
                clearTimeout(timer);
            }
