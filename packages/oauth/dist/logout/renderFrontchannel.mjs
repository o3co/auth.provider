 ...
 */
function safeJsStringLiteral(s) {
 ...
 */
function buildIframeUrl(baseUri, issuer, sid) {
 ...
 */
export function renderFrontchannelLogoutHtml(opts) {
    const logger = opts.logger ?? console;
    const iframes = opts.rps
        .filter((rp) => typeof rp.frontchannelLogoutUri === "string" && rp.frontchannelLogoutUri.length > 0)
        .map((rp) => {
        ...
        try {
            ...
            const iframeSrc = buildIframeUrl(rp.frontchannelLogoutUri, opts.issuer, includeSid ? opts.sid : undefined);
            ...
            logger.warn(`renderFrontchannelLogoutHtml: failed to build iframe for RP ${rp.clientId} (skipping):`, err);
