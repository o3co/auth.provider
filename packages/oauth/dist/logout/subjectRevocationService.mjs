 ...
 */
import { createSubjectRevocationService, defineModule, fullSectionsSchema, requireFederationGrantSubjectRevocation, resolveFederationGrantKeepPolicy, resolveSubjectRevocationHorizonMs, } from "@o3co/auth-provider-core";
import { z } from "zod";
...
const grantsEnabled = (deps) => deps.config.federationGrants?.enabled === true;
/**
...
const auditor = (sink, logger) => {
    return (event) => {
        ...
        void Promise.resolve()
            ...
            .catch((err) => {
            logger?.error({ err, grantId: event.grantId, correlationId: event.correlationId }, "federation_grant_audit_failed");
        });
    };
};
/**
...
export const subjectRevocationServiceModule = defineModule({
    ...
    provides: {
        subjectRevocationService: (deps) => {
            const enabled = grantsEnabled(deps);
            const store = deps.federationGrantStore;
            if (enabled && store === undefined) {
                throw new Error(`${NAME}: federationGrants.enabled = true requires a federationGrantStore ` +
