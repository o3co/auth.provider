 ...
 */
import { emitAuditEvent, formatObject, } from "@o3co/auth-provider-core";
...
export const createOAuthRouter = async (express, { registry, config, clientRepository, codeRepository, keyStore, rateLimiter, auditSink, grantPolicy, refreshTokenStore, userSessionStore, federationTokenStore, getFederationProviders = () => undefined, }) => {
    ...
    async function checkRateLimit(req, res, tag) {
        if (!rateLimiter)
            return true;
    ...
    router
        ...
        if (!(await checkRateLimit(req, res, "token")))
            return;
        ...
        if (result.errorDescription)
            errorBody.error_description = result.errorDescription;
        ...
        .post("/introspect", async (req, res, next) => {
        if (!(await checkRateLimit(req, res, "introspect")))
            return;
        ...
        try {
            ...
            const rawFamilyId = payload.family_id;
            const familyId = typeof rawFamilyId === "string" && rawFamilyId.length > 0 ? rawFamilyId : null;
            ...
            return res.status(200).json(formatObject({
                active: true,
                exp,
                iat,
                iss,
        ...
        if (!(await checkRateLimit(req, res, "authorize")))
            return;
        if (!req.session.isAuthenticated) {
            return res.redirect(`${config.endpoints.login.url}?redirect_to=${encodeURIComponent(`${req.protocol}://${req.get("host")}${req.originalUrl}`)}`);
        }
        ...
        const redirectError = (redirectUri, error, errorDescription, state) => {
        ...
        if ([req.query.response_type].flat().includes("code")) {
            const { client_id = null, scope = null, state = null, code_challenge = null, code_challenge_method = null, redirect_uri = null, } = req.query;
            ...
            const authorizationConfig = grantsConfig?.authorization;
            const pkceConfig = authorizationConfig?.pkce;
            const pkceRequired = pkceConfig?.required === true;
            const defaultMethod = typeof pkceConfig?.defaultMethod === "string" ? pkceConfig.defaultMethod : "plain";
            const supportedMethods = Array.isArray(pkceConfig?.supportedMethods)
                ? pkceConfig.supportedMethods
                : ["S256", "plain"];
            ...
            const allowedFilteredScopes = requestedScopes.length > 0
            ...
            let grantedAudience;
            const subjectForPolicy = typeof req.session.user?.id === "string"
                ? req.session.user.id
                : undefined;
            if (grantPolicy) {
                ...
                try {
                    ...
                    return redirectError(redirect_uri, "temporarily_unavailable", "policy evaluation unavailable", toStr(state));
                }
                if (decision.outcome === "deny") {
                    return redirectError(redirect_uri, decision.error, decision.errorDescription ?? "policy denied", toStr(state));
                }
                if (decision.grantedScope) {
                    ...
                    if (invalidFromPolicy.length > 0) {
                        return redirectError(redirect_uri, "invalid_scope", `policy returned scopes outside client allowance: ${invalidFromPolicy.join(" ")}`, toStr(state));
                    }
            ...
            const audienceForPersist = grantedAudience && grantedAudience.length > 0 ? grantedAudience : undefined;
            let issue;
            try {
    ...
    const logoutSupported = !!userSessionStore && !!federationTokenStore && !!refreshTokenStore && hasIssuer;
    // Federation-token endpoint forwards upstream; does NOT need our issuer.
    const federationTokenSupported = !!userSessionStore && !!federationTokenStore && !!refreshTokenStore;
    if (logoutSupported) {
        router.use(logoutRoute.createRouter(express, {
            keyStore,
            issuer: issuer,
            userSessionStore,
    ...
    if (federationTokenSupported) {
        router.use(federationTokenRoute.createRouter(express, {
            keyStore,
            refreshTokenStore,
            userSessionStore,
            federationTokenStore,
