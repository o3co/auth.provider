 ...
 */
import { buildCanonicalRequestUrl, emitAuditEvent, isEmailVerified, isGrantTypeAllowed, } from "@o3co/auth-provider-core";
import { deriveAudienceFromResources, extractResourceParam, unrepresentedResources, } from "../grants/_resourceIndicator.mjs";
import { PKCE_METHOD_ABSENT_DEFAULT, PKCE_METHOD_S256, pkceMethodsForClient, } from "../grants/pkce.mjs";
...
const redirectError = (ctx, error, errorDescription) => {
 ...
 */
const auditFailure = (ctx, details) => emitAuditEvent(ctx.opts.auditSink, {
    timestamp: new Date(),
 ...
 */
const resolveClientAndRedirectUri = async (req, res, opts) => {
...
const checkResponseTypeIsCode = (ctx) => {
    ...
    if (toStr(raw) !== "code") {
        ...
        const description = raw === undefined
...
const checkAuthorizationCodeGrantAllowed = async (ctx, client) => {
    ...
    redirectError(ctx, "unauthorized_client", "client is not authorized for the authorization_code grant");
    return false;
};
...
// changes the former.
const checkFirstPartyInvariant = async (ctx, client) => {
    ...
    redirectError(ctx, "unauthorized_client", "client is not authorized for the authorization endpoint");
    return false;
};
...
const checkEmailVerified = async (ctx) => {
    if (!(ctx.opts.oauth.requireEmailVerified && !isEmailVerified(ctx.req.session.user)))
        return true;
 ...
 */
const checkPkce = (ctx, client, codeChallenge, codeChallengeMethod) => {
    ...
    if (!pkceMethodsForClient(policy, client).includes(method)) {
        redirectError(ctx, "invalid_request", requestedMethod === undefined
 ...
 * whole class.
 */
const checkSingleValuedParams = (ctx) => {
...
const checkNonce = (ctx) => {
    const nonceMaxLength = ctx.opts.oauth.nonceMaxLength;
    if (ctx.req.query.nonce === undefined)
        return true;
...
const resolveScopes = (ctx, scope, client) => {
    ...
    if (ctx.opts.oauth.oidcMode === "oidc-required" &&
        ...
        redirectError(ctx, "invalid_scope", "openid scope is required when server is acting as an OIDC OP");
        return null;
    }
...
const applyGrantPolicy = async (ctx, inputs) => {
    const { requestedScopes, allowedFilteredScopes, originalScope, authorizeResource } = inputs;
    let grantedScopes = allowedFilteredScopes;
    let grantedAudience;
    const sessionUser = ctx.req.session.user;
    const subjectForPolicy = typeof sessionUser?.id === "string" ? sessionUser.id : undefined;
    ...
    if (grantPolicy) {
        ...
        if (decision.grantedScope) {
            ...
            const invalidFromPolicy = decision.grantedScope.filter((s) => !allowedFilteredScopes.includes(s));
            if (invalidFromPolicy.length > 0) {
                redirectError(ctx, "invalid_scope", `policy returned scopes outside client allowance: ${invalidFromPolicy.join(" ")}`);
                return null;
            }
 ...
 * sent.
 */
const resolveAudienceForPersist = (ctx, client, authorizeResource, grantedAudience) => {
    ...
    if (ctx.opts.oauth.resourceIndicatorEnabled && authorizeResource && !effectiveGrantedAudience) {
        const derived = deriveAudienceFromResources(authorizeResource, new Set([...(client.allowedAudiences ?? []), ctx.clientId]));
        if (derived !== undefined)
            effectiveGrantedAudience = [derived];
    }
    const audienceForPersist = effectiveGrantedAudience && effectiveGrantedAudience.length > 0
    ...
    if (ctx.opts.oauth.resourceIndicatorEnabled && authorizeResource) {
        ...
        if (unrepresented.length > 0) {
            redirectError(ctx, "invalid_target", `requested_resources_not_in_audience: ${unrepresented.join(" ")}`);
            return null;
        }
    }
...
const mintCode = async (ctx, params) => {
    let issue;
    try {
...
const redirectWithCode = async (ctx, code) => {
    const url = new URL(ctx.redirectUri);
 ...
 * same audit payloads.
 */
export const createAuthorizeHandler = (opts) => {
    ...
    return async (req, res) => {
        if (!req.session.isAuthenticated) {
            ...
            return res.redirect(`${loginUrl}${joiner}redirect_to=${encodeURIComponent(buildCanonicalRequestUrl(issuerOrigin, req.originalUrl))}`);
        }
        ...
        const { scope = null, state = null, code_challenge = null, code_challenge_method = null, } = req.query;
        ...
        if (!checkResponseTypeIsCode(ctx))
            return;
        ...
        // policy hook.
        if (!checkSingleValuedParams(ctx))
            return;
        if (!(await checkAuthorizationCodeGrantAllowed(ctx, client)))
            return;
        if (!(await checkFirstPartyInvariant(ctx, client)))
            return;
        if (!(await checkEmailVerified(ctx)))
            return;
        ...
        const authorizeResource = opts.oauth.resourceIndicatorEnabled
            ? extractResourceParam(req.query)
            : null;
        ...
        const audience = resolveAudienceForPersist(ctx, client, authorizeResource, policy.grantedAudience);
        if (!audience)
            return;
