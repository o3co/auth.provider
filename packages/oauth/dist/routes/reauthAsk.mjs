...
const readRecord = (value) => {
    if (value == null || typeof value !== "object")
        return null;
    const record = value.reauth;
    if (record == null || typeof record !== "object")
        return null;
    const { askedAt, request } = record;
    if (typeof askedAt !== "number" || !Number.isFinite(askedAt))
        return null;
 ...
 */
export const createReauthAskStore = (store) => {
    const key = (id) => `${REAUTH_ASK_KEY_PREFIX}${id}`;
    return {
        async ask(record) {
        ...
        async consume(id, request) {
            const value = await new Promise((resolve, reject) => {
                store.get(key(id), (err, record) => err ? reject(err) : resolve(record));
            });
            ...
            if (record.request !== request)
                return null;
            if (record.askedAt + REAUTH_ASK_TTL_MS <= Date.now())
                return null;
            return record;
        },
    };
 ...
 */
export const reauthAskStoreFor = (req) => {
    ...
    return createReauthAskStore(candidate);
};
