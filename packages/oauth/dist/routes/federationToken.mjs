 ...
 */
function supportsRefresh(provider) {
    if (provider == null)
        return false;
    return (typeof provider.refreshFederationToken === "function");
}
/**
...
export function createRouter(express, opts) {
    ...
    router.post("/federation/:name/token", async (req, res) => {
        ...
        try {
            ...
            logger.warn(`POST /oauth/federation/${name}/token: jwtVerify failed:`, error instanceof Error ? error.message : String(error));
        ...
        if (!familyId) {
            res.setHeader("WWW-Authenticate", 'Bearer error="invalid_token", error_description="missing family_id claim"');
            return res
                .status(401)
        ...
        if (!azp) {
            res.setHeader("WWW-Authenticate", 'Bearer error="invalid_token", error_description="missing azp claim"');
            return res
                .status(401)
        ...
        try {
            ...
            logger.warn(`POST /oauth/federation/${name}/token: isFamilyRevoked failed (refresh store outage):`, error);
            res.setHeader("WWW-Authenticate", 'Bearer error="invalid_token", error_description="revocation check unavailable"');
            return res.status(401).json({
                error: "invalid_token",
        ...
        if (!tokens) {
            ...
            try {
                ...
                logger.warn(`POST /oauth/federation/${name}/token: removeFederation self-heal failed:`, error);
        ...
        if (tokens.expiresAt === null || tokens.expiresAt.getTime() > Date.now() + refreshBufferMs) {
            ...
            const expiresIn = tokens.expiresAt === null
                ? undefined
        ...
        if (!supportsRefresh(provider)) {
            logger.warn(`POST /oauth/federation/${name}/token: provider does not support refresh or not found`);
            return res.status(503).json({
        ...
        try {
            ...
            if (release !== undefined) {
                ...
                try {
                    ...
                    logger.warn(`POST /oauth/federation/${name}/token: federationTokenStore.get (post-lock re-read) failed:`, error);
                    return res.status(503).json({
                        error: "temporarily_unavailable",
                        error_description: "federation token store unavailable",
                    });
                ...
                if (freshTokens &&
                    ...
                    // without calling IdP.
                    const expiresIn = freshTokens.expiresAt === null
                        ? undefined
            ...
            try {
                ...
                if (msg.includes("invalid_grant")) {
                    ...
                    try {
                        ...
                        logger.warn(`POST /oauth/federation/${name}/token: federationTokenStore.delete cleanup failed:`, cleanupErr);
                    }
                    try {
                        await opts.userSessionStore.removeFederation(sid, name);
                    }
                    catch (cleanupErr) {
                        logger.warn(`POST /oauth/federation/${name}/token: removeFederation cleanup failed:`, cleanupErr);
                    }
                    emitAuditEvent(opts.auditSink, {
                        timestamp: new Date(),
            ...
            try {
                ...
                logger.warn(`POST /oauth/federation/${name}/token: federationTokenStore.update failed:`, error);
                return res.status(503).json({
                    error: "temporarily_unavailable",
                    error_description: "federation token store unavailable",
                });
            ...
            const expiresIn = refreshed.expiresAt === null
                ? undefined
