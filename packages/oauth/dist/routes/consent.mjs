...
export const newConsentChallenge = () => randomBytes(32).toString("base64url");
const jsonError = (res, status, error, description) => res
    .status(status)
...
export function createConsentRouter(express, opts) {
    const { consentStore, pendingConsentStore, clientRepository, auditSink, logger, userSessionStore, } = opts;
    /**
     ...
     */
    const clientFor = async (res, pending) => {
        let client;
        try {
    ...
    router.post("/consent", async (req, res) => {
        ...
        if ((await clientFor(res, peeked)) === null)
            return;
        ...
        if (decision === "deny") {
            ...
            if (pending.state !== undefined)
                url.searchParams.append("state", pending.state);
