 ...
 */
function supportsEndSession(provider) {
    if (provider == null)
        return false;
...
export function createRouter(express, opts) {
    ...
    router.post("/federation/:name/logout", express.urlencoded({ extended: false }), async (req, res) => {
        const { name } = req.params;
        const { post_logout_redirect_uri: postLogoutRedirectUri, state } = req.body;
        ...
        try {
            ...
            logger.warn(`/oauth/federation/${name}/logout: jwtVerify failed:`, error instanceof Error ? error.message : String(error));
        ...
        if (familyId !== null) {
            ...
            try {
                ...
                logger.warn(`/oauth/federation/${name}/logout: isFamilyRevoked failed (refresh store outage):`, error);
                return res.status(401).json({
                    error: "invalid_token",
        ...
        try {
            ...
            logger.warn(`/oauth/federation/${name}/logout: userSessionStore.removeFederation failed:`, error);
            return res.status(503).json({
                error: "temporarily_unavailable",
                error_description: "session store unavailable",
            });
        ...
        if (supportsEndSession(provider)) {
            try {
                const endSessionResult = await provider.endSession({
                    ...
                    postLogoutRedirectUri: typeof postLogoutRedirectUri === "string" ? postLogoutRedirectUri : undefined,
                ...
                logger.warn(`/oauth/federation/${name}/logout: provider.endSession failed (orphan IdP session):`, error);
                emitAuditEvent(opts.auditSink, {
                    timestamp: new Date(),
    ...
    router.post("/logout", express.urlencoded({ extended: false }), async (req, res) => {
        ...
        res.setHeader("Cache-Control", "no-store");
        res.setHeader("Pragma", "no-cache");
        const { id_token_hint: idTokenHint, post_logout_redirect_uri: postLogoutRedirectUri, state, } = req.body;
        ...
        const aud = typeof rawAud === "string"
            ? rawAud
        ...
        let validatedPostLogoutRedirectUri;
        ...
        const hasFrontchannel = session.activeRPs.some((rp) => typeof rp.frontchannelLogoutUri === "string" && rp.frontchannelLogoutUri.length > 0);
