 ...
 */
export const parseProof = async (raw) => {
    ...
    let jkt;
    try {
        jkt = await computeJkt(jwk);
    }
    catch (err) {
        throw new DPoPError("malformed_proof", `invalid JWK: ${err.message}`);
    }
    return {
        jwk: jwk,
        alg: header.alg,
