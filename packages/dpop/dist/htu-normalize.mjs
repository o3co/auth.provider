 ...
 */
const decodeUnreservedPercent = (s) => {
 ...
 */
const removeDotSegments = (path, originalHadTrailingSlash) => {
...
export const normalizeHtu = (raw) => {
    ...
    if (url.username !== "" || url.password !== "") {
        throw new Error(`normalizeHtu: htu must not contain userinfo (got "${url.username}:***" prefix)`);
    }
    ...
    const normalizedPath = decodedPath === "" ? "/" : removeDotSegments(decodedPath, originalHadTrailingSlash);
