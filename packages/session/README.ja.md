...
- **読めないレコードは障害ではなく「無い」ものとして扱う。** Redis ストアが返したレコードが JSON でない、またはセッションのレコード（`cookie` オブジェクトを持つオブジェクト）でないときは、セッションが無いものとして読む: express-session は新しいセッションを始め、ユーザーはもう一度サインインする。読むたびに warn 1 行、`session_cookie_record_unreadable`（`store: "cookie_session"`）としてログに出て、レコードのテキストは出さない。障害として答えれば、レコードが期限切れになるまでそのブラウザからのすべてのリクエストが失敗することになる（[`src/store/factory.mts`](src/store/factory.mts)）。
