...
import { consoleLogger, errorEnvelope } from "@o3co/auth-provider-core";
...
const deriveSigningKey = (secret) => Buffer.from(hkdfSync("sha256", secret, "", CSRF_KEY_INFO, 32));
const sign = (key, payload) => createHmac("sha256", key).update(payload, "utf8").digest("base64url");
/**
 ...
 * sidesteps both.
 */
const constantTimeEquals = (a, b) => timingSafeEqual(createHash("sha256").update(a, "utf8").digest(), createHash("sha256").update(b, "utf8").digest());
 ...
 */
const assertValidTtlSeconds = (ttlSeconds) => {
    if (!Number.isInteger(ttlSeconds) || ttlSeconds <= 0 || ttlSeconds > MAX_CSRF_TTL_SECONDS) {
        throw new Error(`csrf: ttlSeconds must be an integer between 1 and ${MAX_CSRF_TTL_SECONDS} seconds, received ${String(ttlSeconds)}`);
    }
};
export const createCsrfProtection = (options) => {
    ...
    const cookie = options.cookie ?? { secure: true, sameSite: "lax" };
    ...
    const isWellSigned = (token) => {
        const match = TOKEN_SHAPE.exec(token);
        if (!match)
            return false;
        const [, expires, nonce, signature] = match;
        if (!constantTimeEquals(signature ?? "", sign(key, `${expires}.${nonce}`)))
            return false;
        return Number(expires) * 1000 > now();
    };
    const readSubmitted = (req) => {
    ...
    return {
        ...
        verify(req) {
            ...
            if (fromCookie === undefined && submitted === undefined)
                return "absent";
            if (fromCookie === undefined || submitted === undefined)
                return "invalid";
            if (!constantTimeEquals(fromCookie, submitted))
                return "invalid";
 ...
 */
export const checkRequestOrigin = (req, trustedOrigins = []) => {
    ...
    if (server !== undefined && candidate === server)
        return "same-origin";
    for (const trusted of trustedOrigins) {
        if (normalizeOrigin(trusted) === candidate)
            return "trusted";
    }
    return "foreign";
};
 ...
 */
export const createCsrfGuard = ({ csrf, trustedOrigins = [], logger = consoleLogger, }) => {
    return (req, res, next) => {
        const originVerdict = checkRequestOrigin(req, trustedOrigins);
        if (originVerdict === "foreign") {
            const rawOrigin = req.headers?.origin;
            const origin = Array.isArray(rawOrigin) ? rawOrigin[0] : rawOrigin;
            logger.warn({ origin: (origin ?? "").slice(0, MAX_LOGGED_ORIGIN_LENGTH), path: req.path }, "csrf_origin_rejected");
        ...
        if (originVerdict === "absent") {
            ...
            if (tokenVerdict !== "valid") {
                ...
                res
                    .status(403)
                    .json(errorEnvelope("access_denied", `CSRF check failed: send a same-origin Origin or Referer header, or a double-submit CSRF token (the ${csrf.cookieName} cookie echoed in the ${csrf.headerName} header)`));
                return;
            }
        }
        next();
 ...
 */
export const createCsrfIssueHandler = (csrf) => {
    return (_req, res) => {
 ...
 */
export const createCsrfProtectionFromConfig = (session, overrides = {}) => createCsrfProtection({
