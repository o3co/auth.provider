 ...
 */
function isNotInstalled(reason, name) {
    const { code, message } = (reason ?? {});
    return (code === "ERR_MODULE_NOT_FOUND" && typeof message === "string" && message.includes(`'${name}'`));
}
/**
 ...
 */
export async function loadRedisStoreLibraries(imports = IMPORTS) {
    ...
    ].flatMap(({ name, result }) => result.status === "rejected"
        ? [
            {
                name,
    ...
    if (firstMissing !== undefined) {
        ...
        throw new Error(`${names} ${notInstalled.length === 1 ? "is" : "are"} not installed: run ${INSTALL_COMMAND}${otherFailure === undefined ? "" : `; "${otherFailure.name}" failed to load as well, for another reason, which is this error's cause`} (session.storage.type is "redis", and redis and connect-redis are optional peer dependencies of @o3co/auth-provider-session, which does not install them)`, { cause: otherFailure === undefined ? firstMissing.reason : otherFailure.reason });
    }
    if (other.length > 1) {
        throw new AggregateError(other.map(({ reason }) => reason), `${other.map(({ name }) => `"${name}"`).join(" and ")} both failed to load, for a reason other than not being installed; the two failures are this error's errors`);
    }
