 ...
 */
import { defineModule, fullSectionsSchema, } from "@o3co/auth-provider-core";
    ...
    contributes: {
        routes: [
            async (deps) => {
                ...
                const ctx = { lifecycle: deps.lifecycleRegistrar };
                const factory = createSessionStoreFactory(ctx);
                registerBuiltinSessionStores(factory);
                const storageSlice = config.session.storage;
                if (config.session.name.startsWith("__Host-") &&
                    (config.session.secure !== true || config.session.domain !== null)) {
                    throw new Error("session.name with __Host- prefix requires session.secure=true and session.domain=null");
                }
                const store = await factory.create({
                    type: storageSlice.type,
                    ...(storageSlice[storageSlice.type] ?? {}),
                });
