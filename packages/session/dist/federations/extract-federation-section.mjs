 ...
 */
export function extractFederationSection(federations, name) {
    ...
    const isNested = subSection != null && typeof subSection === "object" && !Array.isArray(subSection);
    if (isNested) {
        // Reject mixed shape: nested sub-section + top-level credential fields.
        const flatCredentialFields = ["clientId", "clientSecret", "callbackURL"].filter((k) => k in section);
        if (flatCredentialFields.length > 0) {
            throw new Error(`federations.${name}: mixed shape — remove top-level ${flatCredentialFields.join("/")} OR the ${type} { ... } sub-section`);
        }
