...
export function createGithubProvider(config) {
    if (!config.clientId || !config.clientSecret || !config.callbackURL) {
        throw new Error(`GitHub federation "${config.name}" requires clientId, clientSecret, and callbackURL`);
    }
        ...
        async endSession(req) {
            if (config.endSessionEndpoint) {
                ...
                try {
                    ...
                    throw new Error(`GitHub federation "${config.name}" has an invalid endSessionEndpoint: ${config.endSessionEndpoint}`);
                }
                if (req.idTokenHint)
            ...
            try {
                ...
                throw new Error(`GitHub federation "${config.name}" received an invalid postLogoutRedirectUri: ${base}`);
            }
            if (req.state)
                url.searchParams.set("state", req.state);
            return { url, method: "GET" };
