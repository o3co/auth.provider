...
import { supportsClaimMapping } from "../federations/types.mjs";
const DEFAULT_SESSION_TTL_MS = 86_400_000; // 24 h
export const createRouter = (express, { config: _config, federationProviders, providerCallbackUrls, userRepository, userSessionStore, federationTokenStore, sessionTtlMs = DEFAULT_SESSION_TTL_MS, }) => {
    if (!userSessionStore)
        throw new Error("federation routes require userSessionStore");
    if (!federationTokenStore)
        throw new Error("federation routes require federationTokenStore");
    if (!userRepository)
        throw new Error("federation routes require userRepository");
    if (!providerCallbackUrls)
        throw new Error("federation routes require providerCallbackUrls");
    ...
    router
        ...
        // ------------------------------------------------------------------
        .get("/oauth/federation/:name", (req, res) => {
        ...
        // ------------------------------------------------------------------
        .get("/oauth/federation/:name/callback", async (req, res) => {
        const provider = federationProviders.get(String(req.params.name));
        if (!provider) {
            return res.status(404).json({ message: "NotFound" });
        }
        ...
        const reusePrevSaveErr = await new Promise((resolve) => {
            req.session.save((err) => resolve(err ?? null));
        });
        if (reusePrevSaveErr) {
            console.warn({ err: reusePrevSaveErr, provider: provider.name }, "reuse-prevention session save failed");
            return res.status(500).json({
                error: "server_error",
        ...
        const regenerateErr = await new Promise((resolve) => {
        ...
        if (regenerateErr) {
            ...
            console.error({ err: regenerateErr, sid, provider: provider.name }, "session regeneration failed after userSessionStore.create");
            return res.status(500).json({
