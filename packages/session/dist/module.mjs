...
import { createFederationProviderFactory, registerBuiltinFederations, } from "./federations/factory.mjs";
 ...
 */
export const _sessionModuleImpl = (params) => ({
    name: "session",
    ...
    async init(context) {
        ...
        if (!context.userSessionStore || !context.federationTokenStore) {
            throw new Error("session module requires userSessionStore and federationTokenStore in ModuleContext");
        }
        // Build federation provider factory (or use injected stub in tests).
        const factory = params._federationFactory ??
            (() => {
        ...
        const federationProviders = new Map();
        ...
        for (const [name, section] of Object.entries(config.federations)) {
            ...
            const isNested = typeof subSection === "object" && subSection !== null && !Array.isArray(subSection);
            // Reject mixed shape: both top-level credential fields and a nested sub-section present.
            if (isNested) {
                const flatFieldsPresent = ["clientId", "clientSecret", "callbackURL"].filter((k) => k in section);
                if (flatFieldsPresent.length > 0) {
                    throw new Error(`federations.${name}: mixed shape — remove top-level ${flatFieldsPresent.join("/")} OR the ${type} { ... } sub-section`);
                }
            }
            const rawBuilderConfig = isNested
                ? (() => {
                    ...
                    const { enabled: _e, type: _t, [type]: _sub, ...topLevel } = section;
                    return { type, ...topLevel, ...subSection };
                })()
                : { type, ...section };
            ...
            const { enabled: _enabled, type: _type, ...flatConfig } = rawBuilderConfig;
            ...
            if (provider.name !== name) {
                throw new Error(`federations.${name}: provider builder returned name="${provider.name}", expected "${name}". ` +
        ...
        context.router.use("/session", _csr(express, {
        ...
        context.router.use("/session", _cfr(express, {
            config,
            federationProviders,
 ...
 */
export const sessionModule = (opts) => _sessionModuleImpl(opts);
