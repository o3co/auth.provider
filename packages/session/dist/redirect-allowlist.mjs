 ...
 * debugging.
 */
export function describeRedirectRejection(reason, options = {}) {
    ...
    switch (reason) {
        ...
        case "not-absolute-url":
            return ("must be an absolute URL with a host (e.g. https://app.example.com/welcome), " +
        ...
        case "insecure-scheme":
            return ("must use https — http is accepted only for a loopback host " +
        ...
        case "outside-session-domain":
            return ("must be inside the configured session cookie domain, or name a loopback host — " +
                "a target the session cookie cannot reach would land the user logged out");
        case "no-allowlist":
            return (`is refused because no redirect allowlist is configured: set ${allowlistConfigKey} ` +
 ...
 */
export function checkRedirectShape(value) {
...
function isInsideSessionDomain(hostname, sessionDomain) {
 ...
 */
function normalizeAllowlist(entries, sessionDomain, factoryName, allowlistConfigKey) {
    ...
    for (const [index, entry] of entries.entries()) {
        const reject = (reason) => {
            throw new Error(`${factoryName}: redirectAllowlist[${index}] ` +
        ...
        if (sessionDomain !== undefined && sessionDomain !== null && sessionDomain !== "") {
            if (!isInsideSessionDomain(url.hostname, sessionDomain))
                reject("outside-session-domain");
        }
 ...
 */
export function createRedirectAllowlistValidator(options) {
    const allowlistConfigKey = options.allowlistConfigKey ?? DEFAULT_ALLOWLIST_CONFIG_KEY;
    ...
    const allowlist = normalizeAllowlist(options.redirectAllowlist, options.sessionDomain, options.factoryName, allowlistConfigKey);
    const refuse = (reason) => ({
        ok: false,
        status: 400,
    ...
    return Object.freeze({
        validateRedirect(url) {
            ...
            if (allowlist.size === 0)
                return refuse("no-allowlist");
            if (!allowlist.has(new URL(url).href))
                return refuse("not-allowlisted");
