...
2. It starts the federation with `?link=1`: `GET /session/oauth/federation/<name>?link=1`, **from a link or a form on the deployment's own site**. A start the browser reports as navigated from another site (`Sec-Fetch-Site: cross-site`) is `403 link_requires_same_site`: the start is a GET and the session cookie is `SameSite=Lax`, so without this any page could send a signed-in user there, and paired with a login CSRF at the IdP the attacker's identity would be linked to the victim's account. `same-site` (an account page on a sibling host), `none` (a typed URL) and an absent header (a non-browser client) are accepted. Without an authenticated session it is `401 login_required`; when the Store's repository does not implement `linkFederatedIdentity`, `400 link_unsupported` — both before the browser is sent anywhere.
